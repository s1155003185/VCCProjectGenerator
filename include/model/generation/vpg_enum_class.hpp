...
class VPGEnumClassAttribute : public vcc::BaseObject {
   private:
    mutable VPGEnumClassAttributeAccessMode _AccessMode =
    ...
    GETCUSTOM(bool, IsType1Custom,
              return vcc::isCapital(vcc::splitString(getType1(), {L"::"}).back());)
    GETSET(std::wstring, Type2, L"")
    GETCUSTOM(bool, IsType2Custom,
              return vcc::isCapital(vcc::splitString(getType2(), {L"::"}).back());)
    GETSET(std::wstring, PropertyName, L"")
    GETSET(std::wstring, Validate, L"")
    GETCUSTOM(std::wstring, DefaultValue, if (_InitializeProperties.empty()) return _DefaultValue;
              return vcc::concat(_InitializeProperties, L", ");)
    ...
    GETCUSTOM(bool, IsCustom,
              return vcc::isContain(getMacro(), L"(")
                         ? vcc::isContain(_Macro.substr(0, _Macro.find(L"(")), L"CUSTOM")
                         : false;)
    GETCUSTOM(bool, IsObject,
              return vcc::isContain(getMacro(), L"(")
                         ? vcc::isContain(_Macro.substr(0, _Macro.find(L"(")), L"_SPTR")
                         : false;)
    ...
    GETCUSTOM(bool, IsSet,
              return vcc::isContain(
                  std::vector<VPGEnumClassMacroType>{
    ...
    GETCUSTOM(bool, IsCollection,
              return getIsVector() || getIsMap() || getIsOrderedMap() || getIsSet();)
    GETCUSTOM(bool, IsHavingValidate,
              return vcc::isContain(getMacro(), L"(")
                         ? vcc::isContain(_Macro.substr(0, _Macro.find(L"(")), L"VALIDATE")
                         : false;)
    GETCUSTOM(
        VPGEnumClassAttributeAccessMode, AccessMode, switch (_MacroType) {
    ...
    GETCUSTOM(bool, IsInitializeInClassConstructorNeeded,
              return vcc::isContain(
                         std::vector<VPGEnumClassMacroType>{VPGEnumClassMacroType::GetsetSptr,
                                                            VPGEnumClassMacroType::ManagerSptr},
                         _MacroType) ||
                     (!vcc::isContain(
                          std::vector<VPGEnumClassMacroType>{VPGEnumClassMacroType::Getcustom,
                                                             VPGEnumClassMacroType::GetcustomSptr},
                          _MacroType) &&
