...
inline std::wstring getIndentStringWithNamespaceLevel(const size_t& namespaceLevel) {
    TRY std::wstring indent = L"";
    for (size_t i = 0; i < namespaceLevel; i++) indent += L"    ";  // 4 spaces
    return indent;
    CATCH
    return L"";
}
...
inline size_t getNamespaceLevel(const std::wstring& namespaceStr) {
    TRY if (namespaceStr.empty()) return 0;
    return vcc::splitString(namespaceStr, {L"::"}).size();
    CATCH
    return 0;
}

inline std::wstring getNamespaceCommonPrefix(const std::wstring& namespace1,
                                             const std::wstring& namespace2) {
    TRY if (namespace1.empty() || namespace2.empty()) return L"";
...
inline std::wstring generateCodeWithNamespace(
    const std::map<std::wstring, std::vector<std::wstring>>& namespaceClassMapping) {
    TRY std::wstring content = L"";
    ...
    for (auto const& pair : namespaceClassMapping) {
        ...
        if (!previousNamespace.empty() && previousNamespace != commonPrefix) {
            ...
            int64_t diff =
                std::abs((int64_t)namespaceLevel - (int64_t)getNamespaceLevel(commonPrefix));
            while (diff > 0) {
                namespaceLevel--;
        ...
        for (auto& classContent : pair.second) {
            ...
            for (auto& line : lines) {
                ...
                vcc::rTrim(line);
                if (vcc::isBlank(content)) content += L"\r\n";
                if (!vcc::isBlank(line))
