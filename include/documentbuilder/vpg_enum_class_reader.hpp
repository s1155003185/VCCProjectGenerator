...
class VPGEnumClassReader {
   ...
   private:
    std::wstring _getErrorMessage(const std::wstring& str, const size_t& pos,
    ...
    std::wstring _getCommand(const std::wstring& cppCode, const bool& isClassCommand,
                             size_t& pos) const;

    void _parseProperties(const std::wstring& cppCode, size_t& pos,
                          std::shared_ptr<VPGEnumClass> enumClass) const;
    bool _parseClass(const std::wstring& cppCode, size_t& pos,
                     std::shared_ptr<VPGEnumClass> enumClass) const;

    void parseCustom(const std::wstring& cppCode, const std::wstring& currentNamespace,
   ...
   public:
    ...
    std::shared_ptr<vcc::Json> getJsonAttributes(const std::wstring& command,
                                                 const std::wstring& attributeName) const;

    // all attribute start with @@
    std::vector<std::wstring> getAttribute(const std::wstring& str) const;
    void parse(const std::wstring& cppCode,
               std::vector<std::shared_ptr<VPGEnumClass>>& results) const;
};