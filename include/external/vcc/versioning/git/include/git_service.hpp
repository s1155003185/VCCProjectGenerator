...
enum class GitRemoteMirror { NA, Fetch, Push };
...
enum class GitPullOptionRecurseSubmodulesMode { Default, Yes, OnDemand, No };

class GitPullOption : public BaseObject {
    GETSET(bool, IsQuite, false);
    GETSET(GitPullOptionRecurseSubmodulesMode, RecurseSubmodules,
           GitPullOptionRecurseSubmodulesMode::Default);
...
enum class GitLogOrderBy { NA, Date, AuthorDate, Topo, Reverse };
...
enum class GitTagCreateTagOptionSignMode { Default, NoSign, Sign, LocalUser };
...
enum class GitBranchCreateBranchOptionTrackMode { Default, No, Direct, Inherit };

class GitBranchCreateBranchOption : public BaseObject {
    GETSET(bool, IsForce, false);
    GETSET(GitBranchCreateBranchOptionTrackMode, Track,
           GitBranchCreateBranchOptionTrackMode::Default);
...
enum class GitResetMode { NA, Soft, Mixed, Hard, Merge, Keep };
...
class GitService : public BaseService {
   public:
    ...
    static std::shared_ptr<GitStatus> getStatus(
        const LogConfig* logConfig, const std::wstring& workspace,
        const GitStatusSearchCriteria* searchCriteria = nullptr);

    // Initialize
    static void initializeGitResponse(const LogConfig* logConfig, const std::wstring& workspace);
    static void cloneGitResponse(const LogConfig* logConfig, const std::wstring& workspace,
                                 const std::wstring& url, const GitCloneOption* option = nullptr);

    /*-----------------------------------*
     * ----------- Remote     -----------*
     * ----------------------------------*/
    // remote
    static std::vector<std::shared_ptr<GitRemote>> getRemote(const LogConfig* logConfig,
                                                             const std::wstring& workspace);
    static void AddRemote(const LogConfig* logConfig, const std::wstring& workspace,
                          const std::wstring& name, const std::wstring& url,
                          const GitRemoteMirror& mirror = GitRemoteMirror::NA);
    static void RenameRemote(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& oldName, const std::wstring& newName);
    static void RemoveRemote(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& name);
    // fetch
    static void FetchAll(const LogConfig* logConfig, const std::wstring& workspace);
    // pull
    static void Pull(const LogConfig* logConfig, const std::wstring& workspace,
                     const GitPullOption* option = nullptr);
    // push
    static void Push(const LogConfig* logConfig, const std::wstring& workspace,
                     const GitPushOption* option = nullptr);
    ...
    // To draw graph, mark the point by column index first, then link nodes if having same
    // ParentHashID and HashID
    static std::vector<std::shared_ptr<GitLog>> getLogs(
        const LogConfig* logConfig, const std::wstring& workspace,
        const GitLogSearchCriteria* searchCriteria = nullptr);
    static std::shared_ptr<GitLog> getCurrentLog(const LogConfig* logConfig,
                                                 const std::wstring& workspace);
    // static void getLog(const LogConfig *logConfig, const std::wstring &workspace, const
    // std::wstring &hashID, std::shared_ptr<GitLog> log);

    /*-----------------------------------*
     * -----------    Tag     -----------*
     * ----------------------------------*/
    static std::vector<std::wstring> getTags(const LogConfig* logConfig,
                                             const std::wstring& workspace,
                                             const GitTagSearchCriteria* searchCriteria = nullptr);
    // Note: There is bug for Git, if using process, return string does not have branch and tags "
    // (HEAD -> main, tag: v0.0.1)" after commit Hash ID. But it is normal if using terminal
    // static void getTag(const LogConfig *logConfig, const std::wstring &workspace, const
    // std::wstring &tagName, std::shared_ptr<GitLog> log);
    static std::shared_ptr<GitTagCurrentTag> getCurrentTag(const LogConfig* logConfig,
                                                           const std::wstring& workspace);
    static void CreateTag(const LogConfig* logConfig, const std::wstring& workspace,
                          const std::wstring& tagName,
                          const GitTagCreateTagOption* option = nullptr);
    // Window behavior and Linux Behavior different, Window throw exception (tag will detach branch)
    // while Linux will not Can use GitService::SwitchReverse to switch back
    static void Switch(const LogConfig* logConfig, const std::wstring& workspace,
                       const std::wstring& tagName, bool isForce = false);
    // Undo Switch
    static void SwitchReverse(const LogConfig* logConfig, const std::wstring& workspace);
    static void DeleteTag(const LogConfig* logConfig, const std::wstring& workspace,
                          const std::wstring& tagName);
    ...
    // static void getCurrentBranch(const LogConfig *logConfig, const std::wstring &workspace,
    // std::shared_ptr<GitBranch> branch);
    static std::wstring getCurrentBranchName(const LogConfig* logConfig,
                                             const std::wstring& workspace);
    static std::vector<std::shared_ptr<GitBranch>> getBranches(const LogConfig* logConfig,
                                                               const std::wstring& workspace);
    static void CreateBranch(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& branchName,
                             const GitBranchCreateBranchOption* option = nullptr);
    static void SwitchBranch(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& branchName,
                             const GitBranchSwitchBranchOption* option = nullptr);
    static void RenameBranch(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& oldBranchName, const std::wstring& newBranchName,
                             bool isForce = false);
    static void CopyBranch(const LogConfig* logConfig, const std::wstring& workspace,
                           const std::wstring& oldBranchName, const std::wstring& newBranchName,
                           bool isForce = false);
    static void DeleteBranch(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& branchName, bool isForce = false);
    ...
    static std::shared_ptr<GitDifferenceSummary> getDifferenceSummary(
        const LogConfig* logConfig, const std::wstring& workspace,
    ...
    static std::shared_ptr<GitDifference> getDifferenceIndexFile(
        const LogConfig* logConfig, const std::wstring& workspace,
        const GitDifferentSearchCriteria* searchCriteria = nullptr);
    static std::shared_ptr<GitDifference> getDifferenceWorkingFile(
        const LogConfig* logConfig, const std::wstring& workspace, const std::wstring& filePath,
        const GitDifferentSearchCriteria* searchCriteria = nullptr);
    static std::shared_ptr<GitDifference> getDifferenceFile(
        const LogConfig* logConfig, const std::wstring& workspace, const std::wstring& filePath,
        const GitDifferentSearchCriteria* searchCriteria = nullptr);
    static std::shared_ptr<GitDifference> getDifferenceCommit(
     ...
     * ----------- Commit     -----------*
     * ----------------------------------*/
    static void stage(const LogConfig* logConfig, const std::wstring& workspace,
                      const std::wstring& filePath);
    static void stageAll(const LogConfig* logConfig, const std::wstring& workspace);
    static void unstage(const LogConfig* logConfig, const std::wstring& workspace,
                        const std::wstring& filePath);
    static void unstageAll(const LogConfig* logConfig, const std::wstring& workspace);
    static void Commit(const LogConfig* logConfig, const std::wstring& workspace,
     ...
     * ----------Reset/Restore-----------*
     * ----------------------------------*/
    static void DiscardFile(const LogConfig* logConfig, const std::wstring& workspace,
                            const std::wstring& filePath);
    static void RestoreFileToParticularCommit(const LogConfig* logConfig,
                                              const std::wstring& workspace,
                                              const std::wstring& filePath,
                                              const std::wstring& hashID);
    static void ResetCommit(const LogConfig* logConfig, const std::wstring& workspace,
                            const GitResetMode& resetMode, const std::wstring& hashID);
    static void ReverseCommit(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& hashID);
     ...
     * ----------------------------------*/
    // Overall Config
    static bool IsConfigExists(const LogConfig* logConfig, const std::wstring& workspace,
                               const std::wstring& key);
    static void getConfig(const LogConfig* logConfig, const std::wstring& workspace,
                          std::shared_ptr<GitConfig> config);
    static std::wstring getConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                  const std::wstring& key);
    ...
    static void setGlobalConfig(const LogConfig* logConfig, const std::wstring& key,
    ...
    // Local Config
    static bool IsLocalConfigExists(const LogConfig* logConfig, const std::wstring& workspace,
                                    const std::wstring& key);
    static void getLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
                               std::shared_ptr<GitConfig> config);
    static std::wstring getLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                       const std::wstring& key);
    static void setLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
    ...
    static void setLocalUserName(const LogConfig* logConfig, const std::wstring& workspace,
                                 const std::wstring& value);
    static std::wstring getLocalUserEmail(const LogConfig* logConfig,
                                          const std::wstring& workspace);
    static void setLocalUserEmail(const LogConfig* logConfig, const std::wstring& workspace,
                                  const std::wstring& value);
};
}  // namespace vcc
