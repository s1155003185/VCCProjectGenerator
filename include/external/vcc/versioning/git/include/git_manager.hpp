...
class GitManager : public BaseManager {
   ...
   public:
    ...
    std::shared_ptr<GitStatus> getStatus(
        const GitStatusSearchCriteria* searchCriteria = nullptr) const;
    ...
    void AddRemote(const std::wstring& name, const std::wstring& url,
    ...
    void CreateBranch(const std::wstring& branchName,
                      const GitBranchCreateBranchOption* option = nullptr);
    void SwitchBranch(const std::wstring& branchName,
                      const GitBranchSwitchBranchOption* option = nullptr);
    void RenameBranch(const std::wstring& oldBranchName, const std::wstring& newBranchName,
                      bool isForce = false);
    void CopyBranch(const std::wstring& oldBranchName, const std::wstring& newBranchName,
                    bool isForce = false);
    ...
    std::shared_ptr<GitDifferenceSummary> getDifferenceSummary(
        const std::vector<std::wstring>& hashIDs);
    std::shared_ptr<GitDifference> getDifferenceIndexFile(
        const GitDifferentSearchCriteria* searchCriteria = nullptr);
    std::shared_ptr<GitDifference> getDifferenceWorkingFile(
        const std::wstring& filePath, const GitDifferentSearchCriteria* searchCriteria = nullptr);
    std::shared_ptr<GitDifference> getDifferenceFile(
        const std::wstring& filePath, const GitDifferentSearchCriteria* searchCriteria = nullptr);
    std::shared_ptr<GitDifference> getDifferenceCommit(
    ...
    // void setGlobalConfig(const LogConfig *logConfig, const std::wstring &key, const std::wstring
    // &value);
