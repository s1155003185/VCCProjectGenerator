...
namespace vcc {
enum class EscapeStringType { DoubleQuote, Regex, SingleQuote, XML };
...
bool isStartWith(const std::wstring& str, const std::wstring& prefix,
                 const size_t& startFromPos = 0, bool isIgnoreCase = false);
...
std::wstring convertNamingStyle(const std::wstring& str, const NamingStyle& from,
                                const NamingStyle& to, const std::wstring& seperator = L" ");

// search
size_t find(const std::wstring& str, const wchar_t& c, const size_t& pos = 0,
            const bool& isIgnoreCase = false);
size_t find(const std::wstring& str, const std::wstring& subStr, const size_t& pos = 0,
            const bool& isIgnoreCase = false);
...
std::wstring getNextString(const std::wstring& str, size_t& pos,
                           ...
                           const std::vector<std::wstring>& quoteCloseList = {},
                           const std::vector<std::wstring>& quoteEscapeList = {});
std::wstring getNextQuotedString(
    const std::wstring& str, size_t& pos, const std::vector<std::wstring>& delimiters,
...
bool isContain(const std::wstring& str, const std::wstring& subStr, const size_t& pos = 0,
               const bool& isIgnoreCase = false);
