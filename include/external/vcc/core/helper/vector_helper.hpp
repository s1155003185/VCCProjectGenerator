...
template <typename T>
void setIObject(std::vector<std::shared_ptr<T>>& sourceVector, std::shared_ptr<IObject>& obj,
...
template <typename T>
void insertIObject(std::vector<std::shared_ptr<T>>& sourceVector, std::shared_ptr<IObject> obj,
                   const int64_t& index = -1);
template <typename T>
void insertIObjects(std::vector<std::shared_ptr<T>>& sourceVector,
                    const std::vector<std::shared_ptr<T>>& objs, const int64_t& index = -1);
...
template <typename T>
bool isContain(const std::vector<T>& v, const std::vector<T>& value) {
    if (isEmpty(v) || v.size() < value.size()) return false;
    for (auto const& element : value) {
        if (!isContain(v, element)) return false;
    }
    return true;
}
...
int64_t find(const std::vector<T>& sourceVector, const T& value) {
    if (isEmpty(sourceVector)) return -1;
    auto it = find(sourceVector.begin(), sourceVector.end(), value);
    if (it != sourceVector.end()) return it - sourceVector.begin();
    return -1;
}
...
int64_t findIObject(std::vector<std::shared_ptr<T>>& sourceVector, const IObject* obj) {
    ...
    for (size_t i = 0; i < sourceVector.size(); i++) {
        if (sourceVector.at(i)->getObjectId() == obj->getObjectId()) return i;
    }
    return -1;
}
...
template <typename T>
void insertIObjects(std::vector<std::shared_ptr<T>>& sourceVector,
                    const std::vector<std::shared_ptr<T>>& objs, const int64_t& index) {
...
void remove(std::vector<T>& sourceVector, const T& obj) {
    ...
    if (it != sourceVector.end()) sourceVector.erase(it);
}

template <typename T>
void remove(std::vector<T>& sourceVector, const std::vector<T>& filters) {
    sourceVector.erase(
        std::remove_if(sourceVector.begin(), sourceVector.end(),
                       [&](const T& element) { return isContain(filters, element); }),
        sourceVector.end());
}

template <typename T>
void removeAtIndex(std::vector<T>& sourceVector, const int64_t& index) {
    if (index >= 0) sourceVector.erase(sourceVector.begin() + (size_t)index);
}

template <typename T>
void RemoveAll(std::vector<T>& sourceVector, const T& obj) {
    sourceVector.erase(std::remove_if(sourceVector.begin(), sourceVector.end(),
                                      [&](const T& element) { return element == obj; }),
                       sourceVector.end());
}
