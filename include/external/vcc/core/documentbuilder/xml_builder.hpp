...
class XmlBuilder : public BaseDocumentBuilder {
   protected:
    ...
    virtual void parseXMLHeader(const std::wstring& xmlData,
                                size_t& pos) const;  // TODO: handle <?...?> and <!...>
   ...
   public:
    ...
    // tag
    virtual bool parseXMLTagHeader(const std::wstring& xmlData, size_t& pos,
                                   std::shared_ptr<Xml> element) const;  // ture if have tag end
    virtual void parseXMLTagContent(const std::wstring& xmlData, size_t& pos,
                                    std::shared_ptr<Xml> element) const;
    virtual void removeXMLTagTail(const std::wstring& xmlData, size_t& pos,
                                  std::shared_ptr<Xml> element) const;

    // content
    virtual void parseXMLTag(const std::wstring& xmlData, size_t& pos,
                             std::shared_ptr<Xml> element) const;
    virtual void parseXml(const std::wstring& xmlData, size_t& pos,
                          std::shared_ptr<Xml> element) const;
