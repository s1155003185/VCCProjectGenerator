...
TEST_F(VPGFileGenerationServiceTest, Empty) {
    ...
    std::wstring result = VPGFileGenerationService::generateFileContent(
        code, tagName, generatedContent, commandDelimiter);
    EXPECT_EQ(result, L"");
}

TEST_F(VPGFileGenerationServiceTest, DEMAND_MODE_FORCE) {
    EXPECT_EQ(
        VPGFileGenerationService::generateFileContent(this->getDemandHeader() + this->getCodeA(),
                                                      L"vcc:tagA", this->getReplaceCode(), L"//"),
        this->getDemandHeader() +
            L"    a\r\n"
            "    // <vcc:tagA gen=\"REPLACE\">\r\n"
            "    line A\r\n"
            "    line B\r\n"
...
TEST_F(VPGFileGenerationServiceTest, DEMAND_MODE_SKIP) {
    EXPECT_EQ(
        VPGFileGenerationService::generateFileContent(this->getDemandHeader() + this->getCodeA(),
                                                      L"vcc:tagB", this->getReplaceCode(), L"//"),
        this->getDemandHeader() + this->getCodeA());
}

TEST_F(VPGFileGenerationServiceTest, REPLACE_MODE_FORCE) {
    EXPECT_EQ(
        VPGFileGenerationService::generateFileContent(this->getForceHeader() + this->getCodeA(),
                                                      L"vcc:tagA", this->getReplaceCode(), L"//"),
        this->getForceHeader() +
            L"    a\r\n"
            "    // <vcc:tagA gen=\"REPLACE\">\r\n"
...
TEST_F(VPGFileGenerationServiceTest, REPLACE_MODE_NONE) {
    EXPECT_EQ(VPGFileGenerationService::generateFileContent(
                  this->getForceHeader() + this->getCodeA(), L"vcc:tagNone", this->getReplaceCode(),
                  L"//"),
...
TEST_F(VPGFileGenerationServiceTest, RESERVE_MODE_FORCE) {
    EXPECT_EQ(
        VPGFileGenerationService::generateFileContent(this->getSkipHeader() + this->getCodeA(),
                                                      L"vcc:tagA", this->getReplaceCode(), L"//"),
