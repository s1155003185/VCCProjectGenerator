...
std::wstring getHppClass(const std::wstring& np, const std::wstring& actionName,
                         const std::vector<std::wstring>& properties,
                         const std::wstring& assignment) {
    TRY std::wstring propertyStr = L"";
    for (auto const& property : properties) propertyStr += INDENT + property + L"\r\n";
    ...
    std::wstring action =
        ...
        "        // <vcc:customVPGGitForm" +
        actionName +
        L"PrivateFunctions sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        // </vcc:customVPGGitForm" +
        actionName +
        L"PrivateFunctions>\r\n"
        "\r\n"
        ...
        "        // <vcc:customVPGGitForm" +
        actionName +
        L"ProtectedFunctions sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        // </vcc:customVPGGitForm" +
        actionName +
        ...
        L"parentForm);\r\n";
    if (!assignment.empty())
        action += L"        VPGGitForm" + actionName +
                  L"(std::shared_ptr<vcc::LogConfig> logConfig, std::shared_ptr<vcc::IObject> "
                  L"parentForm, " +
                  assignment + L");\r\n";
    action += L"        ~VPGGitForm" + actionName +
              ...
              "        // <vcc:customVPGGitForm" +
              actionName +
              L"PublicFunctions sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
              "        // </vcc:customVPGGitForm" +
              actionName +
              L"PublicFunctions>\r\n"
              "};\r\n";
...
std::wstring getCppClass(const std::wstring& np, const std::wstring& actionName,
                         const std::vector<std::wstring>& properties,
                         const std::wstring& assignment) {
    TRY std::wstring propertyStr =
        INDENT + L"_LogConfig = logConfig;\r\n" + INDENT + L"_ParentObject = parentForm;\r\n";
    ...
    action +=
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"GetRedoMessageStart sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        return L\"execute VPGGitForm" +
        actionName +
        L" start\";\r\n"
        "        // </vcc:VPGGitForm" +
        actionName +
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"GetRedoMessageComplete sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        return L\"execute VPGGitForm" +
        actionName +
        L" complete\";\r\n"
        "        // </vcc:VPGGitForm" +
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"GetUndoMessageStart sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        return L\"Undo VPGGitForm" +
        actionName +
        L" start\";\r\n"
        "        // </vcc:VPGGitForm" +
        actionName +
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"GetUndoMessageComplete sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        return L\"Undo VPGGitForm" +
        actionName +
        L" complete\";\r\n"
        "        // </vcc:VPGGitForm" +
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"OnRedo sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        // </vcc:VPGGitForm" +
        actionName +
        L"OnRedo>\r\n"
        ...
        "        // <vcc:VPGGitForm" +
        actionName +
        L"OnUndo sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "        // </vcc:VPGGitForm" +
        actionName +
        L"OnUndo>\r\n"
        "    CATCH_RETURN_RESULT(vcc::OperationResult)\r\n"
...
TEST_F(VPGActionFileGenerationServiceTest, NoFile) {
    ...
    std::vector<std::wstring> hppResult = {
        ...
        getHppClass(L"", L"DeleteWorkspace",
                    {L"GETSET_SPTR_NULL(VPGGitFormDeleteWorkspaceArgument, Argument)"},
                    L"std::shared_ptr<VPGGitFormDeleteWorkspaceArgument> argument")};

    std::vector<std::wstring> cppResult = {
        getCppClass(L"", L"AddWorkspace", {}, L""),
        getCppClass(L"", L"DeleteWorkspace", {L"_Argument = argument;"},
                    L"std::shared_ptr<VPGGitFormDeleteWorkspaceArgument> argument")};
    ...
    classPathMapping.insert(
        std::make_pair(L"VPGGitFormDeleteWorkspaceArgument", L"vpg_git_form.hpp"));
    classPathMapping.insert(
        std::make_pair(L"VPGGitFormDeleteWorkspaceArgumentProperty", L"vpg_git_form.hpp"));
    ...
    VPGActionFileGenerationService::generateHpp(nullptr, classPathMapping,
                                                enumClassList.at(0).get(), L"VPG", L"", hppClass,
                                                sytemIncludeFiles, customIncludeFiles);
    VPGActionFileGenerationService::generateCpp(nullptr, classPathMapping,
                                                enumClassList.at(0).get(), L"VPG", L"", cppClass,
                                                sytemIncludeFiles, customIncludeFiles);
...
TEST_F(VPGActionFileGenerationServiceTest, SeperateFile) {
    ...
    VPGActionFileGenerationService::generateHpp(
        nullptr, classPathMapping, enumClassList.at(0).get(), L"VPG", this->getWorkspace(),
        hppClass, sytemIncludeFiles, customIncludeFiles);
    VPGActionFileGenerationService::generateCpp(
        nullptr, classPathMapping, enumClassList.at(0).get(), L"VPG", this->getWorkspace(),
        cppClass, sytemIncludeFiles, customIncludeFiles);
    EXPECT_TRUE(hppClass.empty());
    EXPECT_TRUE(cppClass.empty());

    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_add_workspace.hpp"})));
    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_add_workspace.cpp"})));
    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_delete_workspace.hpp"})));
    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_delete_workspace.cpp"})));

    std::wstring hppHeader =
        L"// <vcc:vccproj sync=\"FULL\" gen=\"FULL\"/>\r\n"
        L"#pragma once\r\n"
        "\r\n";
    std::wstring cppHeader = L"// <vcc:vccproj sync=\"FULL\" gen=\"FULL\"/>\r\n";

    std::wstring customPropertyHeader =
        L"\r\n"
        "// <vcc:customHeader sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "// </vcc:customHeader>\r\n";
    std::wstring customFooter =
        L"\r\n"
        "// <vcc:customFunctions sync=\"RESERVE\" gen=\"RESERVE\">\r\n"
        "// </vcc:customFunctions>\r\n";

    EXPECT_EQ(
        vcc::readFile(vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_add_workspace.hpp"})),
        hppHeader +
            L"#include <memory>\r\n"
            "#include <string>\r\n"
            "\r\n"
            ...
            customPropertyHeader + getHppClass(L"", L"AddWorkspace", {}, L""));
    EXPECT_EQ(
        vcc::readFile(vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_add_workspace.cpp"})),
        cppHeader +
            ...
            customPropertyHeader + getCppClass(L"", L"AddWorkspace", {}, L"") + customFooter);
...
std::map<std::wstring, std::vector<std::wstring>> getNamespaceClassContentMap(
    const std::wstring& ns, const std::vector<std::wstring>& code) {
...
TEST_F(VPGActionFileGenerationServiceTest, SeperateFile_Namespace) {
    ...
    EXPECT_EQ(
        ...
        hppHeader +
            ...
            "#include \"log_config.hpp\"\r\n" +
            customPropertyHeader +
            generateCodeWithNamespace(getNamespaceClassContentMap(
                L"Namespace", {getHppClass(L"", L"AddWorkspace", {}, L"")})));
    EXPECT_EQ(
        vcc::readFile(vcc::concatPaths({this->getWorkspace(), L"vpg_git_form_add_workspace.cpp"})),
        cppHeader +
            L"#include \"vpg_git_form_add_workspace.hpp\"\r\n"
            ...
            customPropertyHeader +
            generateCodeWithNamespace(getNamespaceClassContentMap(
                L"Namespace", {getCppClass(L"", L"AddWorkspace", {}, L"")})) +
            customFooter);
