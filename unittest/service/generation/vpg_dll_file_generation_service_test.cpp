...
TEST_F(VPGDllFileGenerationServiceTest, Normal) {
    ...
    VPGDllFileGenerationService::generateHpp(this->getLogConfig().get(), this->getFilePathHpp(),
                                             option.get());
    VPGDllFileGenerationService::generateCpp(this->getLogConfig().get(), this->getFilePathCpp(),
                                             option.get());
    ...
    if (!vcc::isContain(contentHpp, L"// <vcc:dllInterfaceHeader gen=\"REPLACE\">") ||
        !vcc::isContain(contentHpp, L"// </vcc:dllInterfaceHeader>")) {
        EXPECT_EQ(contentHpp, L"Generate Hpp Not Contain: dllInterfaceHeader");
    }
    if (!vcc::isContain(contentHpp, L"// <vcc:dllInterface gen=\"REPLACE\">") ||
        !vcc::isContain(contentHpp, L"// </vcc:dllInterface>")) {
        EXPECT_EQ(contentHpp, L"Generate Hpp Not Contain: dllInterface");
    }

    std::wstring contentCpp = vcc::readFile(this->getFilePathCpp());
    if (!vcc::isContain(contentCpp, L"// <vcc:dllInterfaceHeader gen=\"REPLACE\">") ||
        !vcc::isContain(contentCpp, L"// </vcc:dllInterfaceHeader>")) {
        EXPECT_EQ(contentCpp, L"Generate Cpp Not Contain: dllInterfaceHeader");
    }
    if (!vcc::isContain(contentCpp, L"// <vcc:dllInterface gen=\"REPLACE\">") ||
        !vcc::isContain(contentCpp, L"// </vcc:dllInterface>")) {
