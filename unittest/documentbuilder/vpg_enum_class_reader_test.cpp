...
TEST(VPGEnumClassReaderTest, getJsonAttributes) {
    EXPECT_EQ(VPGGlobal::getEnumClassReader()->getJsonAttributes(L"@@A@@Json@@B", L"@@Json"),
              nullptr);

    std::vector<std::wstring> keys = {L"Number", L"String"};
    EXPECT_EQ(
        VPGGlobal::getEnumClassReader()
            ->getJsonAttributes(
                L"@@A@@Json { \"Number\" : 12, \"String\":\"abc\" } @@B { \"B\" : 123}", L"@@Json")
            ->getKeys(),
        keys);
    EXPECT_EQ(VPGGlobal::getEnumClassReader()
                  ->getJsonAttributes(L"@@A@@Json{\"Number\":12,\"String\":\"abc\"}@@B{\"B\":123}",
                                      L"@@Json")
                  ->getKeys(),
              keys);
}
...
void CheckVPGEnumClassReaderTestNormalEnumClassResult(const VPGEnumClassReader* reader,
...
void CheckVPGEnumClassReaderTestNormalResult(const VPGEnumClassReader* reader,
                                             const std::wstring& code) {
    std::vector<std::shared_ptr<VPGEnumClass>> results;
    reader->parse(code, results);
    EXPECT_EQ(results.size(), (size_t)3);
...
TEST(VPGEnumClassReaderTest, Normal1) {
    CheckVPGEnumClassReaderTestNormalResult(
        VPGGlobal::getEnumClassReader().get(),
        L"#pragma once\r\n"
        L"\r\n"
        L"#include <string>\r\n"
        L"#include <vector>\r\n"
...
TEST(VPGEnumClassReaderTest, VCCEnumClassProperty) {
    std::wstring code =
        ...
        L"    EnumValidate, // GETSET_VALIDATE(std::wstring, EnumValidate, L\"Default\", "
        L"validate)\r\n"
        // command
        L"    EnumD, // Command D\r\n"
        L"    EnumE,\r\n"
    ...
    EXPECT_EQ(element->getProperties().at(0)->getEnum(), L"EnumA");
    EXPECT_EQ(element->getProperties().at(0)->getMacro(),
              L"GETSET(EnumTypeA, EnumA, L\"Default\")");
    ...
    EXPECT_EQ(element->getProperties().at(2)->getEnum(), L"EnumC");
    EXPECT_EQ(element->getProperties().at(2)->getMacro(),
              L"GETSET_SPTR(EnumTypeC, EnumC, ArgumentA, ArgumentB, ArgumentC)");
    ...
    EXPECT_EQ(element->getProperties().at(2)->getDefaultValue(),
              L"ArgumentA, ArgumentB, ArgumentC");
    EXPECT_EQ(element->getProperties().at(2)->getCommand(), L"");
    ...
    EXPECT_EQ(element->getProperties().at(3)->getMacro(),
              L"GETSET_VALIDATE(std::wstring, EnumValidate, L\"Default\", validate)");
    ...
    EXPECT_EQ(element->getProperties().at(3)->getMacroType(),
              VPGEnumClassMacroType::GetsetValidate);
...
TEST(VPGEnumClassReaderTest, VCCEnumClassMultiMacro) {
    ...
    EXPECT_EQ(element->getProperties().at(0)->getEnum(), L"EnumA");
    EXPECT_EQ(element->getProperties().at(0)->getMacro(),
              L"GETCUSTOM(int64_t, EnumA, return 100;)");
    ...
    EXPECT_EQ(element->getProperties().at(1)->getEnum(), L"EnumB");
    EXPECT_EQ(element->getProperties().at(1)->getMacro(),
              L"SETCUSTOM(EnumB , int64_t, return 100;)");
    ...
    EXPECT_EQ(element->getProperties().at(2)->getEnum(), L"EnumC");
    EXPECT_EQ(element->getProperties().at(2)->getMacro(),
              L"GETCUSTOM(int64_t, EnumC, return 100;)");
    ...
    EXPECT_EQ(element->getProperties().at(3)->getMacro(),
              L"SETCUSTOM(EnumC, int64_t, _EnumC = value;)");
...
TEST(VPGEnumClassReaderTest, AccessMode) {
    ...
    EXPECT_EQ(element->getProperties().at(0)->getEnum(), L"EnumA");
    EXPECT_EQ(element->getProperties().at(0)->getCommand(), L"");
    EXPECT_EQ(element->getProperties().at(0)->getAccessMode(),
              VPGEnumClassAttributeAccessMode::ReadWrite);
    EXPECT_EQ(element->getProperties().at(1)->getEnum(), L"EnumB");
    EXPECT_EQ(element->getProperties().at(1)->getCommand(), L"");
    EXPECT_EQ(element->getProperties().at(1)->getAccessMode(),
              VPGEnumClassAttributeAccessMode::ReadOnly);
    EXPECT_EQ(element->getProperties().at(2)->getEnum(), L"EnumC");
    EXPECT_EQ(element->getProperties().at(2)->getCommand(), L"");
    EXPECT_EQ(element->getProperties().at(2)->getAccessMode(),
              VPGEnumClassAttributeAccessMode::WriteOnly);
    EXPECT_EQ(element->getProperties().at(3)->getEnum(), L"EnumD");
    EXPECT_EQ(element->getProperties().at(3)->getCommand(), L"");
    EXPECT_EQ(element->getProperties().at(3)->getAccessMode(),
              VPGEnumClassAttributeAccessMode::ReadWrite);
    EXPECT_EQ(element->getProperties().at(4)->getEnum(), L"EnumE");
    EXPECT_EQ(element->getProperties().at(4)->getCommand(), L"");
    EXPECT_EQ(element->getProperties().at(4)->getAccessMode(),
              VPGEnumClassAttributeAccessMode::NoAccess);
}
...
TEST(VPGEnumClassReaderTest, EnumClassWithManager) {
    std::wstring code =
        ...
        "    Action3 // ACTION_WITH_ARG_SPTR(ModifyGitLog, GitLog) @@ActionResult { \"Redo.Class\" "
        ": \"redoActionResult\", \"Undo.Class\" : \"undoActionResult\" }\r\n"
        "};\r\n";
    std::vector<std::shared_ptr<VPGEnumClass>> results;
    VPGGlobal::getEnumClassReader()->parse(code, results);
    EXPECT_EQ(results.size(), (size_t)1);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(0)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Property);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(1)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Manager);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(2)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Manager);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(3)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Manager);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(4)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Action);
    EXPECT_EQ(element->getProperties().at(4)->getType1(), L"");
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(5)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Action);
    ...
    EXPECT_EQ((int64_t)element->getProperties().at(6)->getPropertyType(),
              (int64_t)VPGEnumClassAttributeType::Action);
