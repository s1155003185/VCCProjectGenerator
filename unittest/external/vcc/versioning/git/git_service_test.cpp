...
#ifdef __WIN32
            // window git will hold some files in bin/Debug/Git folder, cannot delete whole folder
            // by c++ remove_all
            TerminalService::execute(
                this->getLogConfig().get(), L"",
                L"rmdir /s /q \"" +
                    concatPaths({std::filesystem::current_path().wstring(), this->getWorkspace()}) +
                    L"\"");
#else
...
//     EXPECT_EQ(remotes.at(0)->getURL(),
//     L"https://github.com/s1155003185/VCCProjectGenerator.git");
//     EXPECT_EQ(remotes.at(0)->getMirror(), GitRemoteMirror::Fetch);
//     EXPECT_EQ(remotes.at(1)->getName(), L"origin");
//     EXPECT_EQ(remotes.at(1)->getURL(),
//     L"https://github.com/s1155003185/VCCProjectGenerator.git");
...
TEST_F(GitServiceTest, parseGitLog) {
    ...
    EXPECT_EQ(log0->getAuthorDate(),
              GitService::parseGitLogDatetime(L"Tue Feb 13 18:11:56 2024 +0800"));
    ...
    // branch
    str =
        L"commit e6c413289f2c1fd6fc6377495926ec5e5644de98 (HEAD -> 20240114Git, "
        L"origin/20240114Git)\n";
    str += L"Author:     Test Tester <test@gmail.com>\n";
    ...
    EXPECT_EQ(log1->getAuthorDate(),
              GitService::parseGitLogDatetime(L"Thu Jan 25 22:47:35 2024 +0800"));
    ...
    EXPECT_EQ(log2->getAuthorDate(),
              GitService::parseGitLogDatetime(L"Thu Jan 25 21:30:19 2024 +0800"));
    ...
    EXPECT_EQ(log3->getAuthorDate(),
              GitService::parseGitLogDatetime(L"Mon Jan 22 20:41:42 2024 +0800"));
    ...
    // mixed
    str =
        L"commit a4da73bfdd353931c0146138d931f17c533e561a (HEAD -> main, tag: v0.0.2, tag: v0.0.1, "
        L"branch2, branch)\n";
    str += L"Author: Test Tester <test@gmail.com>\n";
    ...
    EXPECT_EQ(log5->getAuthorDate(),
              GitService::parseGitLogDatetime(L"Tue Feb 13 18:07:12 2024 +0800"));
...
TEST_F(GitServiceTest, Tag) {
    ...
    // try
    // {
    //     GitTagCurrentTag noTag = GitService::getCurrentTag(this->getLogConfig().get(),
    //     this->getWorkspace());
    ...
    std::vector<std::wstring> tags =
        GitService::getTags(this->getLogConfig().get(), this->getWorkspace());
    ...
    // while Linux will not Can use GitService::SwitchReverse to switch back As window will throw
    // exception, meaningless to have unit test for switching tag try {
...
TEST_F(GitServiceTest, Branch) {
    ...
    EXPECT_TRUE(GitService::getCurrentBranchName(this->getLogConfig().get(),
                                                 this->getWorkspace()) == L"main" ||
                GitService::getCurrentBranchName(this->getLogConfig().get(),
                                                 this->getWorkspace()) == L"master");

    // Current Branch
    // DECLARE_SPTR(GitBranch, currentbranch);
    // GitService::getCurrentBranch(this->getLogConfig().get(), this->getWorkspace(),
    // currentbranch); EXPECT_EQ(currentbranch->getName(), L"branch");
    ...
    GitService::SwitchBranch(this->getLogConfig().get(), this->getWorkspace(), L"branch",
                             &switchBranchOption);
    ...
    EXPECT_EQ(GitService::getCurrentBranchName(this->getLogConfig().get(), this->getWorkspace()),
              L"branch");
...
TEST_F(GitServiceTest, stageAndDifference) {
    ...
    GitService::setLocalUserEmail(this->getLogConfig().get(), this->getWorkspace(),
                                  L"test@test.com");
    EXPECT_EQ(GitService::getLocalUserName(this->getLogConfig().get(), this->getWorkspace()),
              L"test");
    EXPECT_EQ(GitService::getLocalUserEmail(this->getLogConfig().get(), this->getWorkspace()),
              L"test@test.com");
    EXPECT_EQ(GitService::getUserName(this->getLogConfig().get(), this->getWorkspace()), L"test");
    EXPECT_EQ(GitService::getUserEmail(this->getLogConfig().get(), this->getWorkspace()),
              L"test@test.com");
    EXPECT_TRUE(GitService::IsLocalConfigExists(this->getLogConfig().get(), this->getWorkspace(),
                                                L"user.name"));
    ...
    auto differentSummary =
        GitService::getDifferenceSummary(this->getLogConfig().get(), this->getWorkspace(), {});
    ...
    auto diff = GitService::getDifferenceWorkingFile(
        this->getLogConfig().get(), this->getWorkspace(), L"test.txt");  // output in parseGitDiff
    EXPECT_EQ(diff->getFilePathOld(), L"test.txt");
    EXPECT_EQ(diff->getFilePathNew(), L"test.txt");
    EXPECT_EQ(diff->getLineNumberOld()[0], (size_t)1);
    EXPECT_EQ(diff->getLineCountOld()[0], (size_t)0);
    ...
    EXPECT_EQ(diff->getLineCountNew()[0], (size_t)2);
    GitService::stage(this->getLogConfig().get(), this->getWorkspace(), L"test.txt");
    auto statusModifyState =
        GitService::getStatus(this->getLogConfig().get(), this->getWorkspace());
    ...
    EXPECT_EQ(statusModifyState2->getWorkingTreeFiles()[GitFileStatus::Modified].at(0),
              L"test.txt");
    ...
    // Case: Renamed
    std::filesystem::rename(concatPaths({this->getWorkspace(), L"test.txt"}),
                            concatPaths({this->getWorkspace(), L"test2.txt"}));
    ...
    EXPECT_EQ(statusRenameState->getIndexFiles()[GitFileStatus::Renamed].at(0),
    ...
    std::vector<std::shared_ptr<GitLog>> logs =
        GitService::getLogs(this->getLogConfig().get(), this->getWorkspace());
    EXPECT_EQ(logs.size(), (size_t)4);
