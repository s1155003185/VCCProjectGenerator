...
TEST_F(ActionManagerTest, RedoUndoTest) {
    ...
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(0))
                  ->getMessage(),
              L"0");
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(1))
                  ->getMessage(),
              L"1");
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(2))
                  ->getMessage(),
              L"2");
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(3))
                  ->getMessage(),
              L"3");
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(4))
                  ->getMessage(),
              L"4");
    EXPECT_EQ(std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(5))
                  ->getMessage(),
              L"10");
    // Redo Undo
    manager->Undo();
...
TEST_F(ActionManagerTest, ChopTest) {
    ...
    EXPECT_EQ(
        std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(firstSeqNo))
            ->getMessage(),
        L"3");
    EXPECT_EQ(
        std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(firstSeqNo + 1))
            ->getMessage(),
        L"4");
    ...
    EXPECT_EQ(
        ...
        L"2");
    EXPECT_EQ(
        std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(firstSeqNo + 2))
            ->getMessage(),
        L"3");
    EXPECT_EQ(
        std::dynamic_pointer_cast<ActionManagerTestClass>(manager->getActions().at(firstSeqNo + 3))
            ->getMessage(),
        L"4");
}
