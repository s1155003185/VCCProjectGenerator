...
TEST_F(ThreadManagerTest, SuspendAndQueue) {
    ...
    auto thread1 = std::make_shared<vcc::Thread>(
        getLogConfig(),
    ...
    auto thread2 = std::make_shared<vcc::Thread>(
        getLogConfig(),
        [&threadCnt, &callbackCnt](const vcc::Thread* /*thread*/) {
            EXPECT_EQ(threadCnt, (size_t)1);
    ...
    auto thread3 = std::make_shared<vcc::Thread>(
        getLogConfig(),
        [&threadCnt, &callbackCnt](const vcc::Thread* /*thread*/) {
            EXPECT_EQ(threadCnt, (size_t)2);
...
TEST_F(ThreadManagerTest, Stop) {
    auto thread1 = std::make_shared<vcc::Thread>(
        getLogConfig(), [](const vcc::Thread* /*thread*/) {}, [](const vcc::Thread* /*thread*/) {});
    getManager()->suspend();
    getManager()->queue(thread1);
    getManager()->stop();
