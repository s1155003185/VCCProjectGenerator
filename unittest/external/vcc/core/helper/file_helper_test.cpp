...
TEST_F(FileHelperTest, getRegexFromFileFilter) {
    ...
    EXPECT_TRUE(
        std::regex_match(L"abcdef.txt", std::wregex(vcc::getRegexFromFileFilter(L"*def*"))));
}

TEST_F(FileHelperTest, CopyDirectoryWithoutFilter) {
    std::filesystem::remove_all(PATH(this->getWorkspaceTarget()));
    vcc::createDirectory(this->getWorkspaceTarget());
    vcc::appendFileOneLine(vcc::concatPaths({this->getWorkspaceSource(), L"FolderA", L"FileA.txt"}),
                           L"File A", true);
    ...
    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderA", L"FileA.txt"})));
}
