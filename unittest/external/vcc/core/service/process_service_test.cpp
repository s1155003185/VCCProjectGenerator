...
TEST(ProcessServiceTest, Normal) {
    EXPECT_TRUE(
        vcc::ProcessService::execute(nullptr, L"", L"git --version").starts_with(L"git version"));
}

TEST(ProcessServiceTest, NormalWithWorkspace) {
    EXPECT_TRUE(vcc::ProcessService::execute(nullptr, L"", L"..", L"git --version")
                    .starts_with(L"git version"));
}
