...
TEST_F(VPGVccGenerationManagerTest, Add) {
    ...
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/application.hpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"include/type/exception_type.hpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"include/factory/object_factory.hpp"})));
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/factory/property_accessor_factory.hpp"})));
    EXPECT_TRUE(isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/application.cpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"src/factory/object_factory.cpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"src/factory/property_accessor_factory.cpp"})));

    EXPECT_TRUE(isDirectoryExists(concatPaths({this->getWorkspaceTarget(), L"unittest"})));
    EXPECT_TRUE(
        isDirectoryExists(concatPaths({this->getWorkspaceTarget(), L"include/external/vcc/core"})));
    EXPECT_FALSE(isDirectoryExists(
        concatPaths({this->getWorkspaceTarget(), L"include/external/vcc/versioning/Git"})));
    EXPECT_TRUE(isDirectoryExists(
        concatPaths({this->getWorkspaceTarget(), L"unittest/external/vcc/core"})));
...
TEST_F(VPGVccGenerationManagerTest, Update) {
    ...
    writeFile(concatPaths({this->getWorkspaceTarget(), L".vcc/vcc.json"}),
              jsonBuilder->serialize(this->_Option->toJson().get()));
...
TEST_F(VPGVccGenerationManagerTest, Update_Makefile) {
    ...
    std::wstring vccJsonFileContent =
        readFile(concatPaths({this->getWorkspaceTarget(), L".vcc/vcc.json"}));
    ...
    writeFile(concatPaths({this->getWorkspaceTarget(), L".vcc/vcc.json"}),
              jsonBuilder->serialize(json.get()));

    this->getManager()->update();
    std::wstring makefileFileContent =
        readFile(concatPaths({this->getWorkspaceTarget(), L"Makefile"}));
...
TEST_F(VPGVccGenerationManagerTest, Generate) {
    ...
    std::wstring codeEmpty =
        ...
        L"};\r\n"
        L"\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_empty_property.hpp"}),
                      codeEmpty, true);

    // Dependent class
    std::wstring codeDependent =
        L""
        "#pragma once\r\n"
        "\r\n"
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA/Dependent",
                                   L"vcc_object_dependent_property.hpp"}),
                      codeDependent, true);

    // copied from TEST_F(VPGPropertyAccessorFileGenerationServiceTest, Single)
    std::wstring codeSingle =
        L""
        "#pragma once\r\n"
        "\r\n"
    ...
    std::wstring codeNoAccessModeNormal =
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_access_mode_normal_property.hpp"}),
                      codeNoAccessModeNormal, true);

    // copied from TEST_F(VPGPropertyAccessorFileGenerationServiceTest, AccessMode_Vector)
    std::wstring codeNoAccessVector =
        L""
        "#pragma once\r\n"
        "\r\n"
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_access_mode_vector_property.hpp"}),
                      codeNoAccessVector, true);

    // copied from TEST_F(VPGPropertyAccessorFileGenerationServiceTest, Multi)
    std::wstring codeMulti =
        L""
        "#pragma once\r\n"
        "\r\n"
    ...
    std::wstring codeContainer =
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_container_property.hpp"}),
                      codeContainer, true);
    ...
    std::wstring codeMIx =
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_mix_property.hpp"}),
                      codeMIx, true);

    std::wstring codeJson =
        L""
        "#pragma once\r\n"
        "\r\n"
        "// @@Json\r\n"
        ...
        "};\r\n";
    appendFileOneLine(concatPaths({this->getWorkspaceTarget(), L"include/type/ClassA",
                                   L"vcc_object_json_property.hpp"}),
                      codeJson, true);
    ...
    // Empty Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_empty.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_empty_property_accessor.hpp"})));
    EXPECT_FALSE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_empty_property_accessor.cpp"})));

    // Dependent Class
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/model/ClassA/Dependent",
                                   L"vcc_object_dependent.hpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA/Dependent",
                     L"vcc_object_dependent_property_accessor.hpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA/Dependent",
                     L"vcc_object_dependent_property_accessor.cpp"})));

    // Single Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_single.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_single_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_single_property_accessor.cpp"})));

    // NoAccess Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_no_access.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_no_access_property_accessor.hpp"})));
    EXPECT_FALSE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_no_access_property_accessor.cpp"})));

    // Access Normal Class
    EXPECT_TRUE(isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/model/ClassA",
                                           L"vcc_object_access_mode_normal.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_access_mode_normal_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_access_mode_normal_property_accessor.cpp"})));

    // Access Vector Class
    EXPECT_TRUE(isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/model/ClassA",
                                           L"vcc_object_access_mode_vector.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_access_mode_vector_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_access_mode_vector_property_accessor.cpp"})));

    // Multi Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_multi.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_multi_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_multi_property_accessor.cpp"})));

    // Object Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_object.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_object_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_object_property_accessor.cpp"})));

    // Container Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_container.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_container_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_container_property_accessor.cpp"})));

    // Mix Class
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_mix.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_mix_property_accessor.hpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"src/propertyAccessor/ClassA",
                                   L"vcc_object_mix_property_accessor.cpp"})));

    // Mix Class
    EXPECT_TRUE(isFilePresent(concatPaths(
        {this->getWorkspaceTarget(), L"include/model/ClassA", L"vcc_object_json.hpp"})));
    EXPECT_TRUE(isFilePresent(
        concatPaths({this->getWorkspaceTarget(), L"src/model/ClassA", L"vcc_object_json.cpp"})));
    EXPECT_TRUE(
        isFilePresent(concatPaths({this->getWorkspaceTarget(), L"include/propertyAccessor/ClassA",
                                   L"vcc_object_mix_property_accessor.hpp"})));
    EXPECT_TRUE(
    ...
    std::wstring unittest =
        ...
        "}\r\n";
    appendFileOneLine(
        concatPaths({this->getWorkspaceTarget(), L"unittest/module/ClassA", L"class_test.cpp"}),
        unittest, true);
