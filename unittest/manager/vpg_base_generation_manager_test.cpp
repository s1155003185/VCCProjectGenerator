   ...
   public:
    ...
    std::wstring getResultStr(const VPGConfig* _Option) {
        ...
        std::wstring IsExcludeUnittest =
            _Option->getTemplate()->getIsExcludeUnittest() ? L" Y" : L" N";
        std::wstring makeFileStr = L"hi\r\n";
        makeFileStr += L"# <vcc:name sync=\"ALERT\" gen=\"ALERT\">\r\n";
        makeFileStr += L"#----------------------------------#\r\n";
        makeFileStr += L"#---------- Project Name ----------#\r\n";
    ...
    void CreateSyncWorkspaceFolderWithExcludeTestCase() {
        ...
        this->getManager()->syncWorkspace(this->getLogConfig().get(), this->getWorkspaceSource(),
                                          this->getWorkspaceTarget(), includeOnly, excludes);
    }
...
TEST_F(VPGBaseGenerationManagerTest, AdjustMakefile_SyncWorkspaceFolderWithExclude) {
    ...
    EXPECT_TRUE(
        vcc::isDirectoryExists(vcc::concatPaths({this->getWorkspaceTarget(), L"FolderAdd"})));
    EXPECT_TRUE(
        vcc::isDirectoryExists(vcc::concatPaths({this->getWorkspaceTarget(), L"FolderUpdate"})));
    EXPECT_FALSE(
        vcc::isDirectoryExists(vcc::concatPaths({this->getWorkspaceTarget(), L"FolderDelete"})));
    EXPECT_FALSE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderAddFilterOut"})));
    EXPECT_TRUE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderUpdateFilterOut"})));
    EXPECT_TRUE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderDeleteFilterOut"})));
}
...
TEST_F(VPGBaseGenerationManagerTest, AdjustMakefile_SyncWorkspaceFolderIncludeOnly) {
    ...
    EXPECT_TRUE(
        vcc::isDirectoryExists(vcc::concatPaths({this->getWorkspaceTarget(), L"FolderDelete"})));
    EXPECT_TRUE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderAddIncludeOnly"})));
    EXPECT_TRUE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderUpdateIncludeOnly"})));
    EXPECT_FALSE(vcc::isDirectoryExists(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FolderDeleteIncludeOnly"})));
}
...
TEST_F(VPGBaseGenerationManagerTest, AdjustMakefile_SyncWorkspaceFileWithExclude) {
    ...
    EXPECT_FALSE(
        vcc::isFilePresent(vcc::concatPaths({this->getWorkspaceTarget(), L"FileAddFilterOut"})));
    EXPECT_TRUE(
        vcc::isFilePresent(vcc::concatPaths({this->getWorkspaceTarget(), L"FileModifyFilterOut"})));
    EXPECT_TRUE(
        vcc::isFilePresent(vcc::concatPaths({this->getWorkspaceTarget(), L"FileDeleteFilterOut"})));
}
...
TEST_F(VPGBaseGenerationManagerTest, AdjustMakefile_SyncWorkspaceFileIncludeOnly) {
    ...
    EXPECT_TRUE(
        vcc::isFilePresent(vcc::concatPaths({this->getWorkspaceTarget(), L"FileAddIncludeOnly"})));
    EXPECT_TRUE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FileModifyIncludeOnly"})));
    EXPECT_FALSE(vcc::isFilePresent(
        vcc::concatPaths({this->getWorkspaceTarget(), L"FileDeleteIncludeOnly"})));
}
