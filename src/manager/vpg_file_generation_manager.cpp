...
void VPGFileGenerationManager::getClassMacroList(const std::wstring& projWorkspace) {
    TRY std::wstring filePath = vcc::concatPaths({projWorkspace, classMacroFilePath});
...
std::wstring VPGFileGenerationManager::getClassFilenameFromEnumClassFilename(
    const std::wstring& enumClassFileName) {
    TRY std::wstring tmpFileName = enumClassFileName;
...
void VPGFileGenerationManager::getFileList(const VPGEnumClassReader* reader,
                                           const std::wstring& directoryFullPath,
                                           const std::wstring& projectPrefix,
                                           const bool& isSeperateAction) {
    /****************************************************************************************************
    ****************************** All Generated File should be added here
    * ******************************
    ****************************************************************************************************/
    TRY _EnumClasses.clear();
    VPGIncludePathService::getWorkspaceIncludePath(_Workspace, this->_ClassMacros, _IncludeFiles,
                                                   _EnumClasses);
    ...
    for (auto const& filePath :
         std::filesystem::recursive_directory_iterator(PATH(directoryFullPath))) {
        TRY if (filePath.is_directory()) continue;
        ...
        for (auto const& enumClass : enumClassList) {
            ...
            if (enumClassFiles.count(enumClassName) > 0)
                THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                    L"Enum Class " + enumClassName + L" duplicated:\r\n" +
            ...
            // class
            if (isPropertyFile(fileName, projectPrefix) &&
                isPropertyClass(enumClassName, projectPrefix)) {
                ...
                if (classFiles.count(className) > 0)
                    THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                        L"Class " + className + L" duplicated:\r\n" +
                ...
                if (isSeperateAction) {
                    for (auto const& property : enumClass->getProperties()) {
                        if (property->getPropertyType() == VPGEnumClassAttributeType::Action) {
                            std::wstring actionClassName =
                                getActionClassName(enumClass.get(), property.get());
                            std::wstring actionFileName =
                                getActionFileNameWithoutExtension(actionClassName, projectPrefix) +
                                L".hpp";
                            if (_IncludeFiles.find(actionClassName) == _IncludeFiles.end())
                                _IncludeFiles.insert(
                                    std::make_pair(actionClassName, actionFileName));
                        }
                    }
                }
            }
...
std::wstring VPGFileGenerationManager::getConcatPath(const std::wstring& projWorkspace,
                                                     const std::wstring& objWorkspace,
                                                     const std::wstring& middlePath,
                                                     const std::wstring& fileName) const {
    std::vector<std::wstring> objectFilePaths = {projWorkspace, objWorkspace};
    if (!middlePath.empty()) objectFilePaths.push_back(middlePath);
    objectFilePaths.push_back(fileName);
    return vcc::concatPaths(objectFilePaths);
}

void VPGFileGenerationManager::generateProperty(const vcc::LogConfig* logConfig,
                                                const VPGConfig* option) {
    TRY std::wstring projPrefix = option->getProjectPrefix();
    std::wstring projWorkspace = _Workspace;
    std::wstring typeWorkspaceFullPath =
        vcc::concatPaths({projWorkspace, option->getInputTypeWorkspace()});
    ...
    std::wstring actionUnittestDirectoryCpp = option->getOutputUnittestActionDirectoryCpp();
    ...
    std::wstring propertyAccessorFactoryDirectoryHpp =
        option->getOutputPropertyAccessorFactoryDirectoryHpp();
    std::wstring propertyAccessorFactoryDirectoryCpp =
        option->getOutputPropertyAccessorFactoryDirectoryCpp();

    std::vector<std::wstring> actionUnittestNames;
    if (option->getOutput() != nullptr && option->getOutput()->getUnittest() != nullptr) {
        actionUnittestNames = option->getOutput()->getUnittest()->getUnittestNames();
    }
    ...
    getFileList(&enumClassReader, typeWorkspaceFullPath, projPrefix,
                !vcc::isBlank(actionDirectoryHpp));
    ...
    for (auto const& enumClassIncludeFilePair : _IncludeFiles) {
        if (includeFileEnumClassMap.find(enumClassIncludeFilePair.second) ==
            includeFileEnumClassMap.end()) {
            std::vector<std::wstring> emptyList;
            includeFileEnumClassMap.insert(
                std::make_pair(enumClassIncludeFilePair.second, emptyList));
        }
        includeFileEnumClassMap.find(enumClassIncludeFilePair.second)
            ->second.push_back(enumClassIncludeFilePair.first);
    }
    for (auto const& filePath :
         std::filesystem::recursive_directory_iterator(PATH(typeWorkspaceFullPath))) {
        if (filePath.is_directory()) continue;
        std::wstring fileName = filePath.path().filename().wstring();
        if (includeFileEnumClassMap.find(fileName) == includeFileEnumClassMap.end()) continue;

        std::wstring filePathLinuxPath = vcc::getLinuxPath(filePath.path().wstring());
        std::wstring relativePath =
            vcc::getRelativePath(filePathLinuxPath.substr(vcc::find(
                                     filePathLinuxPath, vcc::getLinuxPath(typeWorkspaceFullPath))),
                                 typeWorkspaceFullPath);
        relativePath = PATH(relativePath).parent_path().wstring();
        if (relativePath == L".") relativePath = L"";

        for (auto const& enumClassName : includeFileEnumClassMap.find(fileName)->second) {
            if (_EnumClasses.find(enumClassName) == _EnumClasses.end()) continue;
            if (_EnumClasses.at(enumClassName)->getType() == VPGEnumClassType::Form)
                typeWorkspaceClassRelativePathMapForm.insert(
                    std::make_pair(enumClassName, relativePath));
            else
                typeWorkspaceClassRelativePathMapObject.insert(
                    std::make_pair(enumClassName, relativePath));
        }
    }
    ...
    for (auto const& exportOption : option->getExports()) {
        if (exportOption->getInterface() == VPGConfigInterfaceType::Java) {
            VPGJavaGenerationService::generateOperationResult(
                logConfig, projPrefix, exportOption.get(), typeWorkspaceClassRelativePathMapObject,
                typeWorkspaceClassRelativePathMapForm);
        }
    }
    ...
    for (auto& filePath :
        ...
        std::wstring middlePath =
            vcc::getRelativePath(vcc::getLinuxPath(filePath.path().parent_path().wstring()),
                                 vcc::getLinuxPath(typeWorkspaceFullPath));
        if (middlePath == L".") middlePath = L"";

        if (!vcc::isBlank(projPrefix) && !vcc::isStartWith(fileName, filePrefix))
            vcc::LogService::logWarning(logConfig, logId,
                                        L"Class Prefix " + projPrefix + L" missing. Skip: " + path);
        ...
        // ------------------------------------------------------------------------------------------
        // //
        if (option->getBehavior()->getActionHistoryType() ==
            VPGConfigActionHistoryType::NoHistory) {
            for (auto enumClass : enumClassList)
                for (auto property : enumClass->getProperties()) property->setIsNoHistory(true);
        }
        // ------------------------------------------------------------------------------------------
        // //
        //                              Procedure Start //
        ...
        // //
        for (auto const& enumClass : enumClassList) {
            std::wstring propertyClassNameWithoutNamespace =
                getTypeOrClassWithoutNamespace(enumClass->getName());
            std::wstring classNameWithoutNamespace =
                getClassNameFromPropertyClassName(enumClass->getName());
            if (projPrefix.empty() ||
                isPropertyClass(propertyClassNameWithoutNamespace, projPrefix)) {
                objectTypes.insert(classNameWithoutNamespace.substr(projPrefix.size()));
                objectEnumClassList.push_back(enumClass);
            } else {
                std::wstring classPrefixStr =
                    !vcc::isBlank(projPrefix) ? (L"Prefix " + projPrefix + L" or ") : L"";
                vcc::LogService::logWarning(
                    logConfig, logId,
                    L"Class " + classPrefixStr + L"Suffix " + propertyClassNameSuffix +
                        L"missing. Not generate object for " + enumClass->getName());
            }
            ...
            for (auto const& javaOption : option->getExports()) {
                if (vcc::isBlank(javaOption->getWorkspace()) ||
                    javaOption->getInterface() != VPGConfigInterfaceType::Java)
                    continue;

                std::wstring workspace =
                    vcc::isAbsolutePath(javaOption->getWorkspace())
                        ? javaOption->getWorkspace()
                        : vcc::concatPaths({_Workspace, javaOption->getWorkspace()});

                if (!vcc::isBlank(javaOption->getTypeDirectory()))
                    VPGJavaGenerationService::generateEnum(
                        logConfig,
                        getConcatPath(workspace, javaOption->getTypeDirectory(), middlePath,
                                      javaEnumClassName + L".java"),
                        middlePath, enumClass.get(), option, javaOption.get());
                ...
                if (isPropertyClass(propertyClassNameWithoutNamespace, projPrefix)) {
                    ...
                    if (enumClass->getType() == VPGEnumClassType::Form &&
                        !vcc::isBlank(javaOption->getFormDirectory()))
                        objectDirectory = javaOption->getFormDirectory();
                    if (!vcc::isBlank(objectDirectory))
                        VPGJavaGenerationService::generateObject(
                            logConfig,
                            getConcatPath(workspace, objectDirectory, middlePath,
                                          classNameWithoutNamespace + L".java"),
                            middlePath, enumClass.get(), typeWorkspaceClassRelativePathMapObject,
                            typeWorkspaceClassRelativePathMapForm, option, javaOption.get());
                }
            }
        }
        ...
        if (isPropertyFile(fileName, filePrefix)) {
            ...
            // ------------------------------------------------------------------------------------------
            // //
            std::wstring objectFileName =
                fileName.substr(0, fileName.size() - propertyFileSuffix.size());
            if (objectFileName.ends_with(L"_")) objectFileName.pop_back();

            std::wstring propertyAccessorFileName =
                objectFileName + L"_" + propertyAccessorFileSuffixWithoutExtention;

            if (!vcc::isBlank(objectDirectoryHpp) && !vcc::isBlank(objectDirectoryCpp)) {
                objectFileNames.insert(objectFileName + L".hpp");
                std::wstring fileObjectDirectoryHpp =
                    !vcc::isBlank(formDirectoryHpp)
                        ? getConcatPath(projWorkspace, formDirectoryHpp, middlePath,
                                        objectFileName + L".hpp")
                        : L"";
                std::wstring fileObjectDirectoryCpp =
                    !vcc::isBlank(formDirectoryCpp)
                        ? getConcatPath(projWorkspace, formDirectoryCpp, middlePath,
                                        objectFileName + L".cpp")
                        : L"";
                std::wstring actionFolderHpp =
                    !vcc::isBlank(actionDirectoryHpp)
                        ? getConcatPath(projWorkspace, actionDirectoryHpp, middlePath, L"")
                        : L"";
                std::wstring actionFolderCpp =
                    !vcc::isBlank(actionDirectoryCpp)
                        ? getConcatPath(projWorkspace, actionDirectoryCpp, middlePath, L"")
                        : L"";
                std::wstring actionUnittestFolderPathCpp =
                    !vcc::isBlank(actionUnittestDirectoryCpp)
                        ? getConcatPath(projWorkspace, actionUnittestDirectoryCpp, middlePath, L"")
                        : L"";

                VPGObjectFileGenerationService::generateHpp(
                    logConfig, option, _IncludeFiles, _EnumClasses,
                    getConcatPath(projWorkspace, objectDirectoryHpp, middlePath,
                                  objectFileName + L".hpp"),
                    fileObjectDirectoryHpp, actionFolderHpp, objectEnumClassList);
                VPGObjectFileGenerationService::generateCpp(
                    logConfig, projPrefix, _IncludeFiles, _EnumClasses,
                    getConcatPath(projWorkspace, objectDirectoryCpp, middlePath,
                                  objectFileName + L".cpp"),
                    fileObjectDirectoryCpp, actionFolderCpp, objectEnumClassList);
            }
            if (!propertyAccessorDirectoryHpp.empty() && !propertyAccessorDirectoryCpp.empty()) {
                ...
                VPGPropertyAccessorGenerationService::generateHpp(
                    logConfig, projPrefix,
                    getConcatPath(projWorkspace, propertyAccessorDirectoryHpp, middlePath,
                                  propertyAccessorFileName + L".hpp"),
                    objectEnumClassList);
                VPGPropertyAccessorGenerationService::generateCpp(
                    logConfig, projPrefix, _IncludeFiles,
                    getConcatPath(projWorkspace, propertyAccessorDirectoryCpp, middlePath,
                                  propertyAccessorFileName + L".cpp"),
                    objectEnumClassList);
            }
            // ------------------------------------------------------------------------------------------
            // //
            ...
            // ------------------------------------------------------------------------------------------
            // //
            if (!vcc::isBlank(actionUnittestDirectoryCpp) && !actionUnittestNames.empty()) {
                // if actionDirectoryHpp is not empty, then sepearte files
                VPGUnittestFileGenerationService::generateCppAction(
                    logConfig, projPrefix, fileName,
                    getConcatPath(projWorkspace, actionUnittestDirectoryCpp, middlePath, L""),
                    objectEnumClassList, actionUnittestNames, !actionDirectoryHpp.empty());
            }
        }
    ...
    // ------------------------------------------------------------------------------------------ //
    VPGObjectTypeFileGenerationService::generate(
        logConfig, vcc::concatPaths({projWorkspace, objectTypeDirectory, objectTypeHppFileName}),
        objectTypes);
    ...
    if (!vcc::isBlank(objectFactoryDirectoryHpp) && !vcc::isBlank(objectFactoryDirectoryCpp)) {
        VPGObjectFactoryFileGenerationService::generateHpp(
            logConfig,
            vcc::concatPaths({projWorkspace, objectFactoryDirectoryHpp, objectFactoryFileNameHpp}));
        VPGObjectFactoryFileGenerationService::generateCpp(
            logConfig, projPrefix, objectFileNames,
            vcc::concatPaths({projWorkspace, objectFactoryDirectoryCpp, objectFactoryFileNameCpp}),
            objectTypes);
    }
    // ------------------------------------------------------------------------------------------ //
    //                               Generate Property Accessor Factory File                      //
    // ------------------------------------------------------------------------------------------ //
    if (!vcc::isBlank(propertyAccessorFactoryDirectoryHpp) &&
        !vcc::isBlank(propertyAccessorFactoryDirectoryCpp)) {
        dllOption->setIsGeneratePropertyAccessor(true);
        VPGPropertyAccessorFactoryFileGenerationService::generateHpp(
            logConfig, vcc::concatPaths({projWorkspace, propertyAccessorFactoryDirectoryHpp,
                                         propertyAccessorFactoryFileNameHpp}));
        VPGPropertyAccessorFactoryFileGenerationService::generateCpp(
            logConfig, projPrefix, propertyAccessorFileNames,
            vcc::concatPaths({projWorkspace, propertyAccessorFactoryDirectoryCpp,
                              propertyAccessorFactoryFileNameCpp}),
            objectTypes);
    }
    ...
    // ------------------------------------------------------------------------------------------ //
    VPGDllFileGenerationService::generateHpp(
        logConfig, vcc::concatPaths({projWorkspace, L"DllFunctions.h"}), dllOption.get());
    VPGDllFileGenerationService::generateCpp(
        logConfig, vcc::concatPaths({projWorkspace, L"DllFunctions.cpp"}), dllOption.get());

    // ------------------------------------------------------------------------------------------ //
    //                               Generate JAVA bridge                                         //
    // ------------------------------------------------------------------------------------------ //
    VPGJavaGenerationService::generateJavaBridge(
        logConfig, _Workspace, vcc::concatPaths({projWorkspace, L"DllFunctions.h"}), option);
