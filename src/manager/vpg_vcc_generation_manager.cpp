...
std::wstring VPGVccGenerationManager::adjustAppliationCpp(const std::wstring& fileContent) const {
    ...
    TRY auto elements = std::make_shared<vcc::Xml>();
    ...
    for (std::shared_ptr<vcc::Xml> element : elements->getChildren()) {
        if (element->getName() == L"vcc:vccconfig") {
            ...
            if (_Option->getBehavior()->getActionHistoryType() ==
                VPGConfigActionHistoryType::Global)
...
std::vector<std::wstring> VPGVccGenerationManager::getUpdateList() const {
    ...
    // type
    result.push_back(vcc::concatPaths(
        {_Option->getOutput() != nullptr ? _Option->getOutput()->getExceptionTypeDirectory() : L"",
         L"exception_type.hpp"}));
    result.push_back(vcc::concatPaths(
        {_Option->getOutput() != nullptr ? _Option->getOutput()->getObjectTypeDirectory() : L"",
         L"object_type.hpp"}));

    // application
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getApplicationDirectoryHpp()))
        result.push_back(vcc::concatPaths(
            {_Option->getOutput()->getApplicationDirectoryHpp(), L"application.hpp"}));
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getApplicationDirectoryCpp()))
        result.push_back(vcc::concatPaths(
            {_Option->getOutput()->getApplicationDirectoryCpp(), L"application.cpp"}));

    // factory
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getObjectFactoryDirectoryHpp()))
        result.push_back(vcc::concatPaths(
            {_Option->getOutput()->getObjectFactoryDirectoryHpp(), L"object_factory.hpp"}));
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getObjectFactoryDirectoryCpp()))
        result.push_back(vcc::concatPaths(
            {_Option->getOutput()->getObjectFactoryDirectoryCpp(), L"object_factory.cpp"}));
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getPropertyAccessorFactoryDirectoryHpp()))
        result.push_back(
            vcc::concatPaths({_Option->getOutput()->getPropertyAccessorFactoryDirectoryHpp(),
                              L"property_accessor_factory.hpp"}));
    if (_Option->getOutput() != nullptr &&
        !vcc::isBlank(_Option->getOutput()->getPropertyAccessorFactoryDirectoryCpp()))
        result.push_back(
            vcc::concatPaths({_Option->getOutput()->getPropertyAccessorFactoryDirectoryCpp(),
                              L"property_accessor_factory.cpp"}));
...
std::vector<std::wstring> VPGVccGenerationManager::getUpdateUnitTestList() const {
    std::vector<std::wstring> result;
    bool isExcludeUnittest =
        _Option->getTemplate() != nullptr && _Option->getTemplate()->getIsExcludeUnittest();
    bool isExcludeVCCUnittest =
        _Option->getTemplate() != nullptr && _Option->getTemplate()->getIsExcludeVCCUnitTest();
    if (!isExcludeVCCUnittest) result.push_back(L"external/vcc/core/*");
...
void VPGVccGenerationManager::CreateVccJson(bool isNew) const {
    TRY auto jsonBuilder = std::make_unique<vcc::JsonBuilder>();
    ...
    vcc::appendFileOneLine(vcc::concatPaths({_Workspace, VPGGlobal::getVccJsonFileName()}),
                           _Option->serializeJson(jsonBuilder.get()), true);
    CATCH
}

void VPGVccGenerationManager::ReadVccJson() const {
    TRY std::wstring fileContent =
        vcc::readFile(vcc::concatPaths({_Workspace, VPGGlobal::getVccJsonFileName()}));
...
void VPGVccGenerationManager::add() const {
    TRY assert(_Option->getTemplate() != nullptr);
    ...
    for (auto const& str : getUpdateList()) copyDirectoryOption.insertIncludeFileFilters(str);
    ...
    if (_Option->getTemplate() == nullptr || !_Option->getTemplate()->getIsExcludeUnittest()) {
        ...
        if (!copyDirectoryOption.getIncludeFileFilters().empty())
            vcc::copyDirectory(vcc::concatPaths({src, unittestFolderName}),
                               vcc::concatPaths({dest, unittestFolderName}), &copyDirectoryOption);
    }
...
void VPGVccGenerationManager::update() const {
    ...
    if (_Option->getTemplate() == nullptr || !_Option->getTemplate()->getIsExcludeUnittest()) {
        ...
        if (!list.empty())
            syncWorkspace(this->_LogConfig.get(), vcc::concatPaths({src, unittestFolderName}),
                          vcc::concatPaths({dest, unittestFolderName}), list, {});
    }

    // Update Makefile and unittest
    vcc::LogService::logInfo(this->_LogConfig.get(), classId,
                             L"Update Project according to vcc.json");
    if (!vcc::isFilePresent(vcc::concatPaths({dest, MakeFileName})))
        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                            L"Cannot find " + vcc::concatPaths({dest, MakeFileName}));
    ...
    // Update application
    std::wstring applicationFilePath = vcc::concatPaths(
        {dest, _Option->getOutput()->getApplicationDirectoryCpp(), L"application.cpp"});
    if (vcc::isFilePresent(applicationFilePath))
        vcc::writeFile(applicationFilePath,
                       this->adjustAppliationCpp(vcc::readFile(applicationFilePath)), true);
...
void VPGVccGenerationManager::generate() const {
    ...
    if (!vcc::isFilePresent(vcc::concatPaths({_Workspace, MakeFileName})))
        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                            L"Cannot find " + vcc::concatPaths({_Workspace, MakeFileName}));
    ...
    // Update application
    std::wstring applicationFilePath = vcc::concatPaths(
        {_Workspace, _Option->getOutput()->getApplicationDirectoryCpp(), L"application.cpp"});
    if (vcc::isFilePresent(applicationFilePath))
        vcc::writeFile(applicationFilePath,
                       this->adjustAppliationCpp(vcc::readFile(applicationFilePath)), true);
