...
void VPGProcessManager::initLogConfig() {
    if (this->_LogConfig != nullptr) this->_LogConfig->setIsConsoleLog(true);
}
...
void VPGProcessManager::verifyLocalResponse() {
    TRY
        ...
        // std::wstring localResponseDirectory =
        // VPGGlobal::getConvertedPath(_Option->getWorkspace()); if
    ...
    std::wstring gitUrl =
        _Option->getTemplate() != nullptr ? _Option->getTemplate()->getUrl() : L"";

    vcc::LogService::logInfo(
        this->getLogConfig().get(), L"",
        L"Check VCC Local response existance: " + localResponseDirectoryProject);
    bool isNeedToCloneGitResponse = false;
    if (vcc::isDirectoryExists(localResponseDirectoryProject)) {
        if (vcc::GitService::IsGitResponse(this->getLogConfig().get(),
                                           localResponseDirectoryProject)) {
            ...
            if (_Option->getProjectType() == VPGProjectType::VccComplex ||
                ...
                // if not same, then check verison of genertor exists, if not exists, then master,
                // else switch to correct branch
                auto currentLog = vcc::GitService::getCurrentLog(this->getLogConfig().get(),
                                                                 localResponseDirectoryProject);
                if (!vcc::isContain(currentLog->getTags(), VPGGlobal::getVersion())) {
                    std::wstring currentBranchName = L"";
                    TRY auto currentTag = vcc::GitService::getCurrentTag(
                        this->getLogConfig().get(), localResponseDirectoryProject);
                    if (vcc::isBlank(currentTag->getTagName()))
                        currentBranchName = vcc::GitService::getCurrentBranchName(
                            this->getLogConfig().get(), localResponseDirectoryProject);
                    CATCH_SLIENT
                    // If version is main and current tag version not exists, then no switch
                    std::wstring mainBranch = L"main";
                    std::vector<std::wstring> allTags = vcc::GitService::getTags(
                        this->getLogConfig().get(), localResponseDirectoryProject);
                    if (currentBranchName == L"main" &&
                        !vcc::isContain(allTags, VPGGlobal::getVersion())) {
                        vcc::LogService::logInfo(this->getLogConfig().get(), L"",
                                                 L"Currently in main branch and " +
                                                     VPGGlobal::getVersion() +
                                                     L" is not found. Keep in main branch.");
                    } else {
            ...
            isNeedToCloneGitResponse = true;
            vcc::LogService::logInfo(this->getLogConfig().get(), L"",
                                     localResponseDirectoryProject + L" already exists.");
        }
    } else {
        isNeedToCloneGitResponse = true;
        vcc::LogService::logInfo(this->getLogConfig().get(), L"",
                                 localResponseDirectoryProject + L" not Exists.");
    }
    if (isNeedToCloneGitResponse) {
        // Try to drop and create
        // Note: Window version will hold folder for a while because of some threads of project have
        // not been terminated. Cannot use drop create
        try {
            if (vcc::isDirectoryExists(localResponseDirectoryProject)) {
                vcc::LogService::logInfo(this->getLogConfig().get(), L"",
                                         L"remove current response.");
        ...
        try {
            ...
            vcc::GitService::cloneGitResponse(this->getLogConfig().get(),
                                              localResponseDirectoryBase, gitUrl, &cloneOption);
            vcc::LogService::logInfo(this->getLogConfig().get(), L"", L"Done.");
        } catch (const std::exception& e) {
            std::string msg(e.what());
            vcc::LogService::logWarning(this->getLogConfig().get(), L"", vcc::str2wstr(msg));
        ...
        vcc::LogService::logInfo(this->getLogConfig().get(), L"",
                                 L"Switch to current version " + VPGGlobal::getVersion());
        try {
            vcc::GitService::Switch(this->getLogConfig().get(), localResponseDirectoryProject,
                                    VPGGlobal::getVersion());
            vcc::LogService::logInfo(this->getLogConfig().get(), L"", L"Done.");
        } catch (const std::exception& e) {
            try {
                vcc::LogService::logError(
                    this->getLogConfig().get(), L"",
                    L"VCC Project Generator version Not Exists. Switch to main");
                vcc::GitService::Switch(this->getLogConfig().get(), localResponseDirectoryProject,
                                        L"main");
                vcc::GitService::Pull(this->getLogConfig().get(), localResponseDirectoryProject);
                vcc::LogService::logInfo(this->getLogConfig().get(), L"", L"Done.");
            } catch (const std::exception& e) {
                vcc::LogService::logWarning(this->getLogConfig().get(), L"",
                                            vcc::str2wstr(e.what()));
            }
        }
    }
...
std::shared_ptr<IVPGGenerationManager> VPGProcessManager::getGenerationManager() {
    switch (_Option->getProjectType()) {
        ...
        case VPGProjectType::VccDll:
        case VPGProjectType::VccExe:
            return std::make_shared<VPGVccGenerationManager>(this->getLogConfig(), _Workspace,
                                                             _Option);
        case VPGProjectType::CppComplex:
        case VPGProjectType::CppDll:
        case VPGProjectType::CppExe:
            return std::make_shared<VPGCppGenerationManager>(this->getLogConfig(), _Workspace,
                                                             _Option);
        default:
            assert(false);
            break;
...
void VPGProcessManager::execute(const std::vector<std::wstring>& cmds) {
    ...
    try {
        ...
        if (mode == L"-Add") {
            ...
            std::wstring vccJsonFilePath =
                vcc::concatPaths({_Workspace, VPGGlobal::getVccJsonFileName()});
            if (!vcc::isFilePresent(vccJsonFilePath))
                THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                    vccJsonFilePath + L": File not found.");
        ...
        std::wstring localResponseDirectory =
            _Option->getTemplate() != nullptr &&
                ...
                ? _Option->getTemplate()->getWorkspace()
                : VPGGlobal::getVccProjectLocalResponseDirectory(_Option->getProjectType());
        std::wstring gitUrl =
            _Option->getTemplate() != nullptr && !vcc::isBlank(_Option->getTemplate()->getUrl())
                ? _Option->getTemplate()->getUrl()
                : VPGGlobal::getProjecURL(_Option->getProjectType());
        ...
        vcc::LogService::logInfo(this->_LogConfig.get(), L"VPGProcessManager",
                                 L"Process Complete Successfully!");
    } catch (const std::exception& e) {
        THROW_EXCEPTION(e);
    }
}
