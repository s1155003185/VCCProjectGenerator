...
void VPGBaseGenerationManager::validateOption() const {
    TRY VALIDATE(L"Template Workspace is emtpy.",
                 (_Option->getTemplate() != nullptr &&
                  !vcc::isBlank(_Option->getTemplate()->getWorkspace())));
...
void VPGBaseGenerationManager::getDLLTestFileContent(std::wstring& fileContent) const {
    vcc::replaceRegex(fileContent, L"#define DLL_NAME L\"([^\"]*)\"",
                      L"#define DLL_NAME L\"" + _Option->getProjectNameDll() + L"\"");
}
...
void VPGBaseGenerationManager::CreateBasicProject() const {
    ...
    if (_Option->getIsGit()) {
        vcc::copyFile(vcc::concatPaths({src, L".gitignore"}),
                      vcc::concatPaths({dest, L".gitignore"}), true);
    }
    if (!vcc::isBlank(_Option->getProjectNameExe())) {
        vcc::copyFile(vcc::concatPaths({src, L"main.cpp"}), vcc::concatPaths({dest, L"main.cpp"}),
                      true);
    }
    if (!vcc::isBlank(_Option->getProjectNameDll())) {
        vcc::copyFile(vcc::concatPaths({src, L"DllEntryPoint.cpp"}),
                      vcc::concatPaths({dest, L"DllEntryPoint.cpp"}), true);
        vcc::copyFile(vcc::concatPaths({src, L"DllFunctions.cpp"}),
                      vcc::concatPaths({dest, L"DllFunctions.cpp"}), true);
        vcc::copyFile(vcc::concatPaths({src, L"DllFunctions.h"}),
                      vcc::concatPaths({dest, L"DllFunctions.h"}), true);
    }
    if (_Option->getTemplate() == nullptr || !_Option->getTemplate()->getIsExcludeUnittest()) {
        vcc::copyFile(vcc::concatPaths({src, unittestFolderName, L"gtest_main.cpp"}),
                      vcc::concatPaths({dest, unittestFolderName, L"gtest_main.cpp"}), true);

        if (!vcc::isBlank(_Option->getProjectNameDll())) {
            std::wstring dllUnitTestContent =
                vcc::readFile(vcc::concatPaths({src, unittestFolderName, L"Dll/dll_test.cpp"}));
            getDLLTestFileContent(dllUnitTestContent);
            vcc::appendFileOneLine(
                vcc::concatPaths({dest, unittestFolderName, L"Dll/dll_test.cpp"}),
                dllUnitTestContent, true);
        }
    }
    // Cannot Copy
...
void VPGBaseGenerationManager::syncWorkspace(
    const vcc::LogConfig* logConfig, const std::wstring& sourceWorkspace,
    const std::wstring& targetWorkspace, const std::vector<std::wstring>& includeFileFilters,
    const std::vector<std::wstring>& excludeFileFilters) const {
    TRY std::vector<std::wstring> needToAdd, needToModify, needToDelete;
    TRY vcc::getFileDifferenceBetweenWorkspaces(sourceWorkspace, targetWorkspace, needToAdd,
                                                needToModify, needToDelete);
    CATCH
    ...
    for (auto path : needToAdd) {
        ...
        if (vcc::isFile(sourcePath)) {
            VPGFileSyncService::copyFile(logConfig, VPGFileContentSyncTagMode::Synchronization,
    ...
    for (auto path : needToModify) {
        ...
        std::wstring sourcePath = vcc::concatPaths({sourceWorkspace, path});
        std::wstring targetPath = vcc::concatPaths({targetWorkspace, path});
        if (!vcc::isFile(sourcePath) || !vcc::isFile(targetPath)) continue;
    ...
    for (std::shared_ptr<vcc::Xml> element : elements->getChildren()) {
        if (element->getName() == L"vcc:name") {
            std::wstring projName =
                !vcc::isBlank(_Option->getProjectName()) ? (L" " + _Option->getProjectName()) : L"";
            std::wstring dllName = !vcc::isBlank(_Option->getProjectNameDll())
                                       ? (L" " + _Option->getProjectNameDll())
                                       : L"";
            std::wstring exeName = !vcc::isBlank(_Option->getProjectNameExe())
                                       ? (L" " + _Option->getProjectNameExe())
                                       : L"";
            std::wstring IsExcludeUnittest =
            ...
            for (auto const& exportOption : _Option->getExports()) {
                if (vcc::isBlank(exportOption->getWorkspace())) continue;
                std::wstring workspace =
                    exportOption
                        ->getWorkspace();  // isAbsolutePath(exportOption->getWorkspace()) ?
                                           // exportOption->getWorkspace() : vcc::ConcatPaths({
                                           // _Workspace, exportOption->getWorkspace() });

                if (!vcc::isBlank(exportOption->getExportDirectoryDll())) {
                    exportDllDirWindow +=
                        L" " + vcc::getWindowPath(vcc::concatPaths(
                                   {workspace, exportOption->getExportDirectoryDll()}));
                    exportDllDirLinux +=
                        L" " + vcc::getLinuxPath(vcc::concatPaths(
                                   {workspace, exportOption->getExportDirectoryDll()}));
                    if (exportOption->getIsExportExternalLib()) {
                        exportExternalLibDirWindow +=
                            L" " + vcc::getWindowPath(vcc::concatPaths(
                                       {workspace, exportOption->getExportDirectoryDll()}));
                        exportExternalLibDirLinux +=
                            L" " + vcc::getLinuxPath(vcc::concatPaths(
                                       {workspace, exportOption->getExportDirectoryDll()}));
                    }
                }
                if (!vcc::isBlank(exportOption->getExportDirectoryExe())) {
                    exportExeDirWindow +=
                        L" " + vcc::getWindowPath(vcc::concatPaths(
                                   {workspace, exportOption->getExportDirectoryExe()}));
                    exportExeDirLinux +=
                        L" " + vcc::getLinuxPath(vcc::concatPaths(
                                   {workspace, exportOption->getExportDirectoryExe()}));
                    if (exportOption->getIsExportExternalLib()) {
            ...
            result +=
                L"# <vcc:export sync=\"ALERT\" gen=\"ALERT\">\r\n"
                "#----------------------------------#\r\n"
                "#---------- Export ----------------#\r\n"
                "#----------------------------------#\r\n"
                "ifeq ($(OS),Windows_NT)\r\n"
...
std::wstring VPGBaseGenerationManager::AdjustVSCodeLaunchJson(
    const std::wstring& fileContent) const {
    TRY std::wstring programPath = L"${workspaceFolder}/bin/Debug/unittest";
    if (_Option->getTemplate() != nullptr && _Option->getTemplate()->getIsExcludeUnittest() &&
        !_Option->getProjectNameExe().empty()) {
