...
std::shared_ptr<vcc::IObject> Application::getFormSharedPtr(vcc::IObject* form) {
    TRY for (auto tmpForm : application->_Forms) {
        if (tmpForm.get() == form) return tmpForm;
    }
    CATCH
    return nullptr;
}
...
vcc::IForm* Application::getIFormPtrFromIObject(vcc::IObject* obj) {
    TRY return static_cast<BaseForm*>(obj);
    CATCH
    return nullptr;
}

vcc::IObject* Application::getIObjectPtrFromIResult(vcc::IResult* obj) {
    TRY return static_cast<vcc::BaseResult*>(obj);
    CATCH
    return nullptr;
}
...
void Application::Run() {
    TRY if (application == nullptr) application = std::make_unique<Application>();
    CATCH
}

std::shared_ptr<vcc::IObject> Application::createForm(const ObjectType& objectType) {
    TRY auto form = ObjectFactory::create(objectType);
    assert(form != nullptr);
...
int64_t Application::getResultErrorCode(vcc::IObject* result) {
    TRY return static_cast<int64_t>(getIResultPtrFromIObject(result)->getExceptionType());
    CATCH
    return -1;
}

std::wstring Application::getResultMessage(vcc::IObject* result) {
    TRY return getIResultPtrFromIObject(result)->getMessage();
    CATCH
    return L"";
}

bool Application::isErrorResult(vcc::IObject* result) {
    TRY return getIResultPtrFromIObject(result)->isError();
    CATCH
    return false;
}

bool Application::isWarningResult(vcc::IObject* result) {
    TRY return getIResultPtrFromIObject(result)->isWarning();
    CATCH
    return false;
}

void Application::eraseResult(vcc::IObject* result) {
    TRY removeIObjectAll(application->_Results, result);
    CATCH
}

std::shared_ptr<vcc::IObject> Application::createActionArgument(const ObjectType& objectType) {
    TRY auto actionArgument = ObjectFactory::create(objectType);
...
std::shared_ptr<vcc::IObject> Application::doFormAction(vcc::IObject* form,
                                                        const int64_t& formProperty,
                                                        vcc::IObject* argument) {
    TRY if (form == nullptr) return nullptr;
    auto result = getIFormPtrFromIObject(form)->doAction(
        formProperty, argument != nullptr ? argument->sharedPtr() : nullptr);
    if (argument != nullptr) removeIObjectAll(application->_ActionArguments, argument);
...
void Application::redoFormActionToSeqNo(vcc::IObject* form, const int64_t& seqNo) {
    TRY if (form == nullptr) return;
    getIObjectPtrFromIResult(getIFormPtrFromIObject(form)->redoActionToSeqNo(seqNo).get())
        ->sharedPtr();
    CATCH
}
...
void Application::undoFormActionToSeqNo(vcc::IObject* form, const int64_t& seqNo) {
    TRY if (form == nullptr) return;
    getIObjectPtrFromIResult(getIFormPtrFromIObject(form)->undoActionToSeqNo(seqNo).get())
        ->sharedPtr();
    CATCH
}
...
bool Application::isFormClosable(vcc::IObject* form) {
    ...
    TRY return getIFormPtrFromIObject(form)->isClosable();
    CATCH
    return false;
}

bool Application::isFormClosed(vcc::IObject* form) {
    TRY if (form == nullptr) return true;
...
bool Application::closeForm(vcc::IObject* form, const bool& isForce) {
    ...
    TRY if (getIFormPtrFromIObject(form)->Close(isForce)) {
        if (getFormSharedPtr(form) != nullptr) application->_Forms.erase(getFormSharedPtr(form));
        return true;
    }
    CATCH
    return false;
