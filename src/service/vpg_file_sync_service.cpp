...
VPGFileContentSyncMode VPGFileSyncService::getSyncMode(const VPGFileContentSyncTagMode& mode,
                                                       const vcc::Xml* codeElemet) {
    TRY for (std::shared_ptr<vcc::Xml> child : codeElemet->getChildren()) {
...
const vcc::Xml* VPGFileSyncService::getTagFromCode(const vcc::Xml* code,
                                                   const std::wstring tagName) {
    TRY for (size_t i = 0; i < code->getChildren().size(); i++) {
        if (code->getChildren().at(i)->getName() == tagName) return code->getChildren().at(i).get();
    }
    CATCH
    return nullptr;
}

bool VPGFileSyncService::IsTagReplace(const VPGFileContentSyncTagMode& mode,
                                      const vcc::Xml* child) {
    TRY for (std::shared_ptr<vcc::XmlAttribute> attr : child->getAttributes()) {
        if (isSyncTag(mode, attr->getName())) {
            std::wstring value = attr->getValue();
            vcc::toUpper(value);
...
bool VPGFileSyncService::IsTagReserve(const VPGFileContentSyncTagMode& mode,
                                      const vcc::Xml* child) {
    TRY for (std::shared_ptr<vcc::XmlAttribute> attr : child->getAttributes()) {
        if (isSyncTag(mode, attr->getName())) {
            std::wstring value = attr->getValue();
...
void VPGFileSyncService::copyFile(const vcc::LogConfig* logConfig,
                                  const VPGFileContentSyncTagMode& mode,
                                  const std::wstring& sourcePath,
                                  const std::wstring& originalCodePath) {
    TRY if (!vcc::isFilePresent(sourcePath))
    ...
    if (vcc::isFilePresent(originalCodePath)) {
        ...
        std::wstring fileContent = VPGFileSyncService::SyncFileContent(
            mode, vcc::readFile(sourcePath), vcc::readFile(originalCodePath),
            VPGFileContentSyncMode::Demand, commandDelimiter);
        vcc::writeFile(originalCodePath, fileContent, true);
        vcc::LogService::logInfo(logConfig, L"", L"Updated File: " + originalCodePath);
    } else {
        std::filesystem::copy_file(PATH(sourcePath), PATH(originalCodePath),
...
std::wstring VPGFileSyncService::generateForceCode(const VPGFileContentSyncMode updatedCodeMode,
                                                   const VPGFileContentSyncMode originalCodeMode,
                                                   const vcc::Xml* updatedCode,
                                                   const vcc::Xml* originalCode) {
    std::wstring result = L"";
    TRY if (originalCodeMode == VPGFileContentSyncMode::NA) {
        ...
        for (std::shared_ptr<vcc::Xml> child : originalCode->getChildren()) {
            result += child->getFullText();
            if (child->getName() == VCC_NAME) break;
        }
        bool shouldSkip = updatedCodeMode != VPGFileContentSyncMode::NA;
        for (std::shared_ptr<vcc::Xml> child : updatedCode->getChildren()) {
            if (!shouldSkip) result += child->getFullText();
            if (child->getName() == VCC_NAME) shouldSkip = false;
        }
    }
    CATCH
    return result;
...
std::wstring VPGFileSyncService::GenerateFullCode(const VPGFileContentSyncTagMode& mode,
    ...
    for (std::shared_ptr<vcc::Xml> child : updatedCode->getChildren()) {
        if (!shouldSkip) {
            ...
            if (vcc::isStartWith(child->getName(), L"vcc:")) {
                const vcc::Xml* originalCodeTag =
                    VPGFileSyncService::getTagFromCode(originalCode, child->getName());
                if (originalCodeTag != nullptr &&
                    VPGFileSyncService::IsTagReserve(mode, originalCodeTag)) {
...
std::wstring VPGFileSyncService::generateDemandCode(const VPGFileContentSyncTagMode& mode,
                                                    const VPGFileContentSyncMode updatedCodeMode,
                                                    const VPGFileContentSyncMode originalCodeMode,
                                                    const vcc::Xml* updatedCode,
                                                    const vcc::Xml* originalCode) {
    ...
    bool shouldSkip = updatedCodeMode != VPGFileContentSyncMode::NA &&
                      originalCodeMode != VPGFileContentSyncMode::NA;
    for (std::shared_ptr<vcc::Xml> child : originalCode->getChildren()) {
        if (!shouldSkip) {
            // if find tag then search tag in source, if reserve, then use source
            if (vcc::isStartWith(child->getName(), L"vcc:")) {
                const vcc::Xml* updatedCodeTag =
                    VPGFileSyncService::getTagFromCode(updatedCode, child->getName());
                if (updatedCodeTag != nullptr &&
                    VPGFileSyncService::IsTagReplace(mode, child.get())) {
...
std::wstring VPGFileSyncService::SyncFileContent(const VPGFileContentSyncTagMode& mode,
    ...
    std::wstring result = L"";
    TRY std::unique_ptr<VPGCodeReader> reader = std::make_unique<VPGCodeReader>(commandDelimiter);
    ...
    // read first line to determine sync mode. originalCode first, then updatedCode. If not exists,
    // then default
    VPGFileContentSyncMode syncMode =
        originalCodeSyncMode != VPGFileContentSyncMode::NA
            ? originalCodeSyncMode
            : (updatedCodeSyncMode != VPGFileContentSyncMode::NA ? updatedCodeSyncMode
                                                                 : defaultMode);
    switch (syncMode) {
        case VPGFileContentSyncMode::Force:
            return VPGFileSyncService::generateForceCode(updatedCodeSyncMode, originalCodeSyncMode,
                                                         updatedCodeElement.get(),
                                                         originalCodeElement.get());
        case VPGFileContentSyncMode::Full:
            return VPGFileSyncService::GenerateFullCode(
                mode, updatedCodeSyncMode, originalCodeSyncMode, updatedCodeElement.get(),
                originalCodeElement.get());
        case VPGFileContentSyncMode::Demand:
            return VPGFileSyncService::generateDemandCode(
                mode, updatedCodeSyncMode, originalCodeSyncMode, updatedCodeElement.get(),
                originalCodeElement.get());
