...
size_t VPGFileGenerationService::getLeadingSpace(const std::wstring& line) {
    TRY for (size_t i = 0; i < line.size(); i++) {
        if (!std::iswspace(line.at(i))) return i;
    }
    CATCH
    return 0;
}
...
std::wstring VPGFileGenerationService::getIndent(const std::wstring& str) {
    ...
    TRY std::vector<std::wstring> lines = vcc::splitStringByLine(str);
    if (lines.empty()) return L"";

    for (size_t i = 0; i < lines.size(); i++) {
        std::wstring line = lines[lines.size() - i - 1];
        while (!line.empty() && line[line.length() - 1] == L'\r') line.pop_back();
        ...
        if (vcc::isBlank(line))
            ...
            if (pos > 0 && pos < line.length()) result = line.substr(0, pos);
        }
        break;
    }
    CATCH
...
const vcc::Xml* VPGFileGenerationService::getTagFromCode(const vcc::Xml* code,
...
VPGFileContentGenerationMode VPGFileGenerationService::getGenerationMode(
...
std::wstring VPGFileGenerationService::generateForceCode(const vcc::Xml* src,
                                                         const std::wstring& tagName,
                                                         const std::wstring& generatedContent,
                                                         const std::wstring& commandDelimiter) {
    std::wstring result = L"";
    TRY
        // split replacement to lines
        std::vector<std::wstring>
            lines = vcc::splitStringByLine(generatedContent);
    bool isFound = false;
...
std::wstring VPGFileGenerationService::generateDemandCode(const vcc::Xml* src,
                                                          const std::wstring& tagName,
                                                          const std::wstring& generatedContent,
                                                          const std::wstring& commandDelimiter) {
    std::wstring result = L"";
    ...
    for (const auto& child : src->getChildren()) {
        if (vcc::isStartWith(child->getName(), L"vcc:") && child->getName() == tagName) {
            const vcc::Xml* srcTag =
                VPGFileGenerationService::getTagFromCode(src, child->getName());
...
std::wstring VPGFileGenerationService::generateSkipCode(const std::wstring& src) { return src; }

std::wstring VPGFileGenerationService::generateFileContent(const std::wstring& code,
                                                           const std::wstring& tagName,
                                                           const std::wstring& generatedContent,
                                                           const std::wstring& commandDelimiter) {
    ...
    switch (getGenerationMode(codeElement.get())) {
        case VPGFileContentGenerationMode::Force:
            return VPGFileGenerationService::generateForceCode(codeElement.get(), tagName,
                                                               generatedContent, commandDelimiter);
        case VPGFileContentGenerationMode::Demand:
            return VPGFileGenerationService::generateDemandCode(codeElement.get(), tagName,
                                                                generatedContent, commandDelimiter);
        ...
        default:
            THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                L"Unknown File Content Generation Mode");
            break;
    }
    CATCH
    return code;
