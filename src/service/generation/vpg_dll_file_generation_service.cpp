...
std::wstring VPGDllFileGenerationService::generateApplicationHpp(
    const VPGDllFileGenerationServiceOption* option) {
    std::wstring result = L"";
    TRY if (!option->getIsGenerateApplication()) return result;

    std::map<std::wstring, std::wstring> functionMap;
    functionMap.insert(
        std::make_pair(L"applicationStart", L"DLLEXPORT void applicationStart();\r\n"));

    // Initialize Form
    functionMap.insert(
        std::make_pair(L"applicationCreateForm",
                       L"DLLEXPORT void *applicationCreateForm(int64_t objectType);\r\n"));

    // Result
    functionMap.insert(
        std::make_pair(L"applicationGetResultErrorCode",
                       L"DLLEXPORT int64_t applicationGetResultErrorCode(void *result);\r\n"));
    functionMap.insert(std::make_pair(
        L"applicationGetResultMessage",
        L"DLLEXPORT void applicationGetResultMessage(void *result, wchar_t **value);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationIsErrorResult",
                       L"DLLEXPORT bool applicationIsErrorResult(void *result);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationIsWarningResult",
                       L"DLLEXPORT bool applicationIsWarningResult(void *result);\r\n"));
    functionMap.insert(std::make_pair(L"applicationEraseResult",
                                      L"DLLEXPORT void applicationEraseResult(void *result);\r\n"));

    // Form Action
    functionMap.insert(std::make_pair(
        L"applicationCreateActionArgument",
        L"DLLEXPORT void *applicationCreateActionArgument(int64_t objectType);\r\n"));

    functionMap.insert(std::make_pair(L"applicationDoFormAction",
                                      L"DLLEXPORT void *applicationDoFormAction(void *form, "
                                      L"int64_t formProperty, void *argument);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationGetFormActionCurrentSeqNo",
                       L"DLLEXPORT int64_t applicationGetFormActionCurrentSeqNo(void *form);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationGetFormActionFirstSeqNo",
                       L"DLLEXPORT int64_t applicationGetFormActionFirstSeqNo(void *form);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationGetFormActionLastSeqNo",
                       L"DLLEXPORT int64_t applicationGetFormActionLastSeqNo(void *form);\r\n"));

    functionMap.insert(std::make_pair(
        L"applicationRedoFormAction",
        L"DLLEXPORT void applicationRedoFormAction(void *form, int64_t noOfStep);\r\n"));
    functionMap.insert(std::make_pair(
        L"applicationRedoFormActionToSeqNo",
        L"DLLEXPORT void applicationRedoFormActionToSeqNo(void *form, int64_t seqNo);\r\n"));

    functionMap.insert(std::make_pair(
        L"applicationUndoFormAction",
        L"DLLEXPORT void applicationUndoFormAction(void *form, int64_t noOfStep);\r\n"));
    functionMap.insert(std::make_pair(
        L"applicationUndoFormActionToSeqNo",
        L"DLLEXPORT void applicationUndoFormActionToSeqNo(void *form, int64_t seqNo);\r\n"));

    functionMap.insert(
        std::make_pair(L"applicationClearFormAction",
                       L"DLLEXPORT int64_t applicationClearFormAction(void *form);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationTruncateFormAction",
                       L"DLLEXPORT int64_t applicationTruncateFormAction(void *form);\r\n"));

    // Close Form
    functionMap.insert(
        std::make_pair(L"applicationIsFormClosable",
                       L"DLLEXPORT bool applicationIsFormClosable(void *form);\r\n"));
    functionMap.insert(std::make_pair(L"applicationIsFormClosed",
                                      L"DLLEXPORT bool applicationIsFormClosed(void *form);\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationCloseForm",
                       L"DLLEXPORT bool applicationCloseForm(void *form, bool isForce);\r\n"));
...
std::wstring VPGDllFileGenerationService::generateApplicationCpp(
    const VPGDllFileGenerationServiceOption* option, std::set<std::wstring>& customIncludeFiles) {
    std::wstring result = L"";
    TRY if (!option->getIsGenerateApplication()) return result;
    ...
    functionMap.insert(std::make_pair(
        ...
        "}\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationGetResultMessage",
                       L"void applicationGetResultMessage(void *result, wchar_t **value)\r\n"
                       "{\r\n"
                       "    TRY\r\n"
                       "        std::wstring message = "
                       "Application::getResultMessage(static_cast<vcc::IObject *>(result));\r\n"
    ...
    functionMap.insert(std::make_pair(
        ...
        "}\r\n"));
    functionMap.insert(
        std::make_pair(L"applicationEraseResult",
...
std::wstring VPGDllFileGenerationService::generatePropertyAccessorHpp(
    const VPGDllFileGenerationServiceOption* option, std::set<std::wstring>& customIncludeFiles) {
    std::wstring result = L"";
    TRY if (!option->getIsGeneratePropertyAccessor()) return result;
...
std::wstring VPGDllFileGenerationService::generatePropertyAccessorCpp(
    const VPGDllFileGenerationServiceOption* option, std::set<std::wstring>& customIncludeFiles) {
    std::wstring result = L"";
    TRY if (!option->getIsGeneratePropertyAccessor()) return result;
...
void VPGDllFileGenerationService::generateHpp(const vcc::LogConfig* logConfig,
                                              const std::wstring& filePathHpp,
                                              const VPGDllFileGenerationServiceOption* option) {
    TRY assert(option != nullptr);
    if (!vcc::isFilePresent(filePathHpp)) return;
    ...
    if (!customIncludeFiles.empty()) {
        for (auto const& file : customIncludeFiles)
            content += L"#include " +
                       vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, file) +
                       L"\r\n";
    }
    vcc::writeFile(filePathHpp,
                   VPGFileGenerationService::generateFileContent(
                       vcc::readFile(filePathHpp), L"vcc:dllInterfaceHeader", content, L"//"),
                   true);
    ...
    vcc::writeFile(filePathHpp,
                   VPGFileGenerationService::generateFileContent(
                       vcc::readFile(filePathHpp), L"vcc:dllInterface", content, L"//"),
                   true);
    vcc::LogService::logInfo(logConfig, logId, L"Modify DllFunctions.hpp file completed.");
    CATCH
}

void VPGDllFileGenerationService::generateCpp(const vcc::LogConfig* logConfig,
                                              const std::wstring& filePathCpp,
                                              const VPGDllFileGenerationServiceOption* option) {
    TRY assert(option != nullptr);
    if (!vcc::isFilePresent(filePathCpp)) return;
    ...
    vcc::writeFile(filePathCpp,
                   VPGFileGenerationService::generateFileContent(
                       vcc::readFile(filePathCpp), L"vcc:dllInterfaceHeader", content, L"//"),
                   true);
    ...
    vcc::writeFile(filePathCpp,
                   VPGFileGenerationService::generateFileContent(
                       vcc::readFile(filePathCpp), L"vcc:dllInterface", content, L"//"),
                   true);
