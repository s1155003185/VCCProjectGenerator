    ...
    TRY
        ...
        for (auto const &property : enumClassPropertiesReadOnly) {
            ...
            if (property->GetIsVector() || property->GetIsOrderedMap()) {
                ...
                if (type == objectToken)
                    ...
                    if (!type.empty() && IsCapital(type))
                        returnStr = L"static_cast<long>(" + returnStr + (isOrderedMap ? L"AtIndex" : L"") + L"(index)"+ (isOrderedMap ? L".second" : L"") + L");\r\n";
            ...
            if (property->GetIsMap() || property->GetIsOrderedMap()) {
                ...
                std::wstring returnStr = L"obj->Get" + property->GetPropertyName();
                if (type == objectToken)
                    returnStr = L"std::static_pointer_cast<IObject>(" + returnStr + (isOrderedMap ? L"AtKey" : L"") + L"(" + mapGetKey + L")" + (isOrderedMap ? L".second" : L"") + L");\r\n";
                else if (!property->GetType2().empty() && IsCapital(property->GetType2()))
                    returnStr = L"static_cast<long>(" + returnStr + (isOrderedMap ? L"AtKey" : L"") + L"(" + mapGetKey + L")" + (isOrderedMap ? L".second" : L"") + L");\r\n";
...
void VPGPropertyAccessorGenerationService::GenerateWrite(const std::wstring &propertyName, const std::wstring &type, const std::vector<std::shared_ptr<VPGEnumClassProperty>> &enumClassPropertiesWriteOnly, std::wstring &result)
{
    ...
    TRY
        ...
        if (isHavingGeneralType) {
            ...
            for (auto const &property : enumClassPropertiesWriteOnly) {
                ...
                if (IsStartWith(property->GetMacro(), L"VECTOR") || IsStartWith(property->GetMacro(), L"ORDERED_MAP")) {
                    ...
                    result += INDENT + INDENT + INDENT + L"obj->Set" + property->GetPropertyName() + L"(wstr2str(value));\r\n";
                else
        ...
        if (isHavingMapType) {
            if (type == objectToken)
                result += L"(const int64_t &objectProperty, " + convertedType + L" value, const void *key) const\r\n";
            else
                result += L"(const int64_t &objectProperty, const " + convertedType + L" &value, const void *key) const\r\n";
        } else {
            if (type == objectToken)
                result += L"(const int64_t &objectProperty, " + convertedType + L" /*value*/, const void * /*key*/) const\r\n";
            else
...
void VPGPropertyAccessorGenerationService::GenerateContainerRemove(const std::wstring &propertyName, const std::wstring &type, const std::vector<std::shared_ptr<VPGEnumClassProperty>> &enumClassProperties, std::wstring &result)
{
    ...
    TRY
        ...
        for (auto const &item : enumClassProperties) {
            if (IsStartWith(item->GetMacro(), L"VECTOR")) {
        ...
        if (isHavingVectorObject)
            result += L"(const int64_t &objectProperty, const void *value) const\r\n";
        else
            result += L"(const int64_t &objectProperty, const void * /*value*/) const\r\n";
        result += L"{\r\n"
            + INDENT + L"TRY\r\n";
