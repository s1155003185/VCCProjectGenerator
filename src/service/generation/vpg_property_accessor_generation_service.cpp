...
const std::wstring logId = L"Property Accessor Generation";
...
bool VPGPropertyAccessorGenerationService::isIncludeString(
    ...
    bool result = false;
    TRY for (auto const& enumClass : enumClassList) {
...
void VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(
    const std::wstring& originalType, std::wstring& convertedType, std::wstring& convertedName,
    std::wstring& returnResult) {
    TRY if (originalType == objectToken) {
    ...
    else if (originalType == L"char" || originalType == L"byte") {
    ...
    else if (originalType == L"short" || originalType == L"int16_t") {
        convertedType = L"short";
        convertedName = L"Short";
        returnResult = L"0";
    }
    else if (originalType == L"uint16_t" || originalType == L"unsigned short" ||
             originalType == L"unsigned int16_t") {
        convertedType = L"uint16_t";
        convertedName = L"UnsignedShort";
        returnResult = L"0";
    }
    else if (originalType == L"int" || originalType == L"unsigned int32_t") {
        convertedType = L"int";
        convertedName = L"Int";
        returnResult = L"0";
    }
    else if (originalType == L"uint32_t" || originalType == L"unsigned int" ||
             originalType == L"unsigned int32_t") {
        convertedType = L"uint32_t";
        convertedName = L"UnsignedInt";
        returnResult = L"0";
    }
    else if (originalType == L"long" || originalType == L"int64_t" || originalType == L"time_t" ||
             originalType == enumToken ||
             (!originalType.empty() &&
              vcc::isCapital(getTypeOrClassWithoutNamespace(originalType)) /* enum */)) {
        convertedType = L"long";
        convertedName = L"Long";
        returnResult = L"0L";
    }
    else if (originalType == L"size_t" || originalType == L"unsigned long" ||
    ...
    else if (originalType == L"string" || originalType == L"std::string" ||
             originalType == L"wstring" || originalType == L"std::wstring") {
        convertedType = L"std::wstring";
        convertedName = L"String";
        returnResult = L"L\"\"";
    }
    else THROW_EXCEPTION_MSG(ExceptionType::ParserError, L"Unknown type: " + originalType);
    CATCH
}
...
std::wstring VPGPropertyAccessorGenerationService::getIncludeFile(
...
void VPGPropertyAccessorGenerationService::generateHpp(
    const vcc::LogConfig* logConfig, const std::wstring& projectPrefix,
    const std::wstring& filePathHpp,
    const std::vector<std::shared_ptr<VPGEnumClass>>& enumClassList) {
    TRY vcc::LogService::logInfo(logConfig, logId,
                                 L"Generate property accessor hpp file: " + filePathHpp);
    ...
    result +=
        L"#include \"base_property_accessor.hpp\"\r\n"
    ...
    for (auto const& enumClass : enumClassList) {
        ...
        if (!isPropertyClass(originalClassName, projectPrefix)) continue;
        ...
        for (auto const& property : readWrite) {
            ...
            if (property->getIsSet()) {
                vcc::LogService::logWarning(logConfig, L"Property Accessor Generation Service",
                                            L"Property Accessor not support SET: class " +
                                                enumClass->getName() + L": " +
                                                property->getMacro());
                continue;
            }
            ...
            if (!property->getIsObject()) {
                std::wstring type = property->getIsMap() || property->getIsOrderedMap()
                                        ? property->getType2()
                                        : property->getType1();
                if (type.empty())
                    THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                                        L"Unknow Macro found in " + enumClass->getName() + L": " +
                                            property->getMacro());

                std::wstring convertedType = L"";
                std::wstring converttedName = L"";
                std::wstring returnResult = L"";
                VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(
                    type, convertedType, converttedName, returnResult);
        ...
        for (auto const& type : getPropertyAccessorTypeOrder()) {
            if (types.find(type) != types.end())
                code +=
                    INDENT + L"PROPERTY_ACCESSOR_HEADER(" + type + L", " + types[type] + L")\r\n";
        }

        if (isObject)
            code += INDENT +
                    L"PROPERTY_ACCESSOR_OBJECT_HEADER(std::shared_ptr<vcc::IObject>, Object)\r\n";
        if (isCollection) code += L"\r\n" + INDENT + L"PROPERTY_ACCESSOR_CONTAINER_HEADER\r\n";

        code +=
            L"\r\n" + INDENT + L"public:\r\n" + INDENT + INDENT + className +
            L"(std::shared_ptr<vcc::IObject> object) : vcc::BasePropertyAccessor(object) {}\r\n" +
            INDENT + INDENT + L"virtual ~" + className +
            L"() {}\r\n"
            "};\r\n";
        ...
        if (namespaceClassMapping.find(namespaceName) != namespaceClassMapping.end()) {
            ...
            namespaceClassMapping.insert(
                std::make_pair(namespaceName, std::vector<std::wstring>{code}));
        }
    }
    result += generateCodeWithNamespace(namespaceClassMapping);

    vcc::writeFile(filePathHpp, result, true);
    vcc::LogService::logInfo(logConfig, logId, L"Generate property accessor hpp completed.");
    CATCH
}

void VPGPropertyAccessorGenerationService::generateRead(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassPropertiesReadOnly,
    std::wstring& result) {
    if (type == containerToken || enumClassPropertiesReadOnly.empty()) return;
    TRY std::wstring convertedType = L"";
    std::wstring convertedName = L"";
    std::wstring returnResult = L"";
    VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(type, convertedType,
    ...
    for (auto const& property : enumClassPropertiesReadOnly) {
        ...
        if (property->getIsVector() || property->getIsOrderedMap()) {
            ...
            VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(
                property->getType1(), tmpConvertedType, tmpConvertedName, tmpReturnResult);

            if (type == objectToken)
                returnStr = L"std::static_pointer_cast<vcc::IObject>(" + returnStr +
                            L"AtIndex(index)" + (isOrderedMap ? L".second" : L"") + L")";
            else {
                std::wstring type = isOrderedMap ? property->getType2() : property->getType1();
                if (!type.empty() && vcc::isCapital(type))
                    returnStr = L"static_cast<long>(" + returnStr + L"AtIndex(index)" +
                                (isOrderedMap ? L".second" : L"") + L")";
                else if (type == L"string")
                    returnStr = L"vcc::str2wstr(" + returnStr + L"AtIndex(index)" +
                                (isOrderedMap ? L".second" : L"") + L")";
                else
        ...
        if (property->getIsMap() || property->getIsOrderedMap()) {
            // key
            std::wstring mapGetKey =
                (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                    ? L"keyPtr"
                    : L"*keyPtr";
            std::wstring castType =
                (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                    ? L"wchar_t"
                    : property->getType1();
            ...
            std::wstring returnStr = L"obj->get" + property->getPropertyName();
            if (type == objectToken)
                returnStr = L"std::static_pointer_cast<vcc::IObject>(" + returnStr + L"AtKey(" +
                            mapGetKey + L")" + L")";
    ...
    if (!generalCases.empty()) {
        ...
        result += INDENT + INDENT +
                  L"THROW_EXCEPTION_MSG_FOR_BASE_PROPERTY_ACCESSOR_DETAIL_PROPERTY_NOT_FOUND\r\n";
    result += INDENT + L"CATCH\r\n" + INDENT + L"return " + returnResult +
              L";\r\n"
              "}\r\n";
    ...
    if (!mapCases.empty()) {
        ...
        for (auto const& pair : mapCases)
            result += INDENT + INDENT + L"case " + propertyName + L"::" + pair.first + L": {\r\n" +
                      INDENT + INDENT + INDENT + L"auto keyPtr = static_cast<const " +
                      mapCasekeyTypes.at(pair.first) + L" *>(key);\r\n" + INDENT + INDENT + INDENT +
                      L"assert(keyPtr != nullptr);\r\n" + INDENT + INDENT + INDENT +
                      L"if (keyPtr == nullptr)\r\n" + INDENT + INDENT + INDENT + INDENT +
                      L"THROW_EXCEPTION_MSG(ExceptionType::KeyInvalid, L\"Invalid Property "
                      L"Accessor Map Key\");\r\n" +
                      INDENT + INDENT + INDENT + L"return " + pair.second + L";\r\n" + INDENT +
                      INDENT + L"}\r\n";
        result += generalTypeContentFooter;
...
void VPGPropertyAccessorGenerationService::generateWrite(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassPropertiesWriteOnly,
    std::wstring& result) {
    if (type == containerToken || enumClassPropertiesWriteOnly.empty()) return;
    TRY std::wstring convertedType = L"";
    std::wstring convertedName = L"";
    std::wstring returnResult = L"";
    VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(type, convertedType,
    ...
    for (auto const& property : enumClassPropertiesWriteOnly) {
        ...
        if (property->getIsGeneralType()) {
            ...
            if (type == objectToken)
                returnStr +=
                    L"(std::static_pointer_cast<" + property->getType1() + L">(value));\r\n";
            else if (!property->getType1().empty() && vcc::isCapital(property->getType1()))
        ...
        if (property->getIsVector()) {
            ...
            if (property->getIsObject()) {
                returnStr = INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                            INDENT + INDENT + L"obj->set" + property->getPropertyName() +
                            L"AtIndex(index, std::static_pointer_cast<" + type + L">(value));\r\n" +
                            INDENT + INDENT + INDENT + L"else\r\n" + INDENT + INDENT + INDENT +
                            INDENT + L"obj->insert" + property->getPropertyName() +
                            L"(std::static_pointer_cast<" + type + L">(value));\r\n";
            } else if (!type.empty() && vcc::isCapital(type)) {
                returnStr = INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                            INDENT + INDENT + L"obj->set" + property->getPropertyName() +
                            L"AtIndex(index, static_cast<" + type + L">(value));\r\n" + INDENT +
                            INDENT + INDENT + L"else\r\n" + INDENT + INDENT + INDENT + INDENT +
                            L"obj->insert" + property->getPropertyName() + L"(static_cast<" + type +
                            L">(value));\r\n";
            } else if (type == L"string") {
                returnStr = INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                            INDENT + INDENT + L"obj->set" + property->getPropertyName() +
                            L"AtIndex(index, wstr2str(value));\r\n" + INDENT + INDENT + INDENT +
                            L"else\r\n" + INDENT + INDENT + INDENT + INDENT + L"obj->insert" +
                            property->getPropertyName() + L"(wstr2str(value));\r\n";
            } else {
                returnStr = INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                            INDENT + INDENT + L"obj->set" + property->getPropertyName() +
                            L"AtIndex(index, value);\r\n" + INDENT + INDENT + INDENT + L"else\r\n" +
                            INDENT + INDENT + INDENT + INDENT + L"obj->insert" +
                            property->getPropertyName() + L"(value);\r\n";
            };
        ...
        if (property->getIsMap() || property->getIsOrderedMap()) {
            ...
            if (type == objectToken) {
                returnStr +=
                    INDENT + INDENT + INDENT + L"if (obj->is" + property->getPropertyName() +
                    L"ContainKey(" + mapGetKey + L"))\r\n" + INDENT + INDENT + INDENT + INDENT +
                    L"obj->set" + property->getPropertyName() + L"AtKey(" + mapGetKey +
                    L", std::static_pointer_cast<" + property->getType2() + L">(value));\r\n" +
                    INDENT + INDENT + INDENT + L"else\r\n" + INDENT + INDENT + INDENT + INDENT +
                    L"obj->insert" + property->getPropertyName() + L"AtKey(" + mapGetKey +
                    L", std::static_pointer_cast<" + property->getType2() + L">(value));\r\n";
            } else {
                std::wstring type = property->getType2();
                if (!type.empty() && vcc::isCapital(type)) {
                    returnStr += INDENT + INDENT + INDENT + L"if (obj->is" +
                                 property->getPropertyName() + L"ContainKey(" + mapGetKey +
                                 L"))\r\n" + INDENT + INDENT + INDENT + INDENT + L"obj->set" +
                                 property->getPropertyName() + L"AtKey(" + mapGetKey +
                                 L", static_cast<" + type + L">(value));\r\n" + INDENT + INDENT +
                                 INDENT + L"else\r\n" + INDENT + INDENT + INDENT + INDENT +
                                 L"obj->insert" + property->getPropertyName() + L"AtKey(" +
                                 mapGetKey + L", static_cast<" + type + L">(value));\r\n";
                } else if (type == L"string") {
                    returnStr += INDENT + INDENT + INDENT + L"if (obj->is" +
                                 property->getPropertyName() + L"ContainKey(" + mapGetKey +
                                 L"))\r\n" + INDENT + INDENT + INDENT + INDENT + L"obj->set" +
                                 property->getPropertyName() + L"AtKey(" + mapGetKey +
                                 L", vcc::str2wstr(value));\r\n" + INDENT + INDENT + INDENT +
                                 L"else\r\n" + INDENT + INDENT + INDENT + INDENT + L"obj->insert" +
                                 property->getPropertyName() + L"AtKey(" + mapGetKey +
                                 L", wstr2str(value));\r\n";
                } else {
                    returnStr +=
                        ...
                        L"obj->set" + property->getPropertyName() + L"AtKey(" + mapGetKey +
                        L", value);\r\n" + INDENT + INDENT + INDENT + L"else\r\n" + INDENT +
                        INDENT + INDENT + INDENT + L"obj->insert" + property->getPropertyName() +
                        L"AtKey(" + mapGetKey + L", value);\r\n";
                }
            }
            mapCases.insert(std::make_pair(property->getEnum(), returnStr));
        }
    ...
    if (!generalCases.empty()) {
        ...
        for (auto const& pair : generalCases)
            result += INDENT + INDENT + L"case " + propertyName + L"::" + pair.first + L":\r\n" +
                      INDENT + INDENT + INDENT + pair.second + INDENT + INDENT + INDENT +
                      L"break;\r\n";
        result += generalTypeContentFooter;
    } else
        result += INDENT + INDENT +
    ...
    result +=
        L"\r\n"
        "void " +
        propertyName + L"Accessor::_write" + convertedName +
        L"AtIndex(const int64_t &objectProperty, " +
        (vcc::isEqual(type, objectToken) ? L"" : L"const ") + convertedType + L" " +
        (vcc::isEqual(type, objectToken) ? L"" : L"&") +
    ...
    result +=
        ...
        "void " +
        propertyName + L"Accessor::_write" + convertedName +
        L"AtKey(const int64_t &objectProperty, " +
        (vcc::isEqual(type, objectToken) ? L"" : L"const ") + convertedType + L" " +
        (vcc::isEqual(type, objectToken) ? L"" : L"&") +
...
void VPGPropertyAccessorGenerationService::generateInsert(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassPropertiesWriteOnly,
    std::wstring& result) {
    if (type == containerToken || enumClassPropertiesWriteOnly.empty()) return;
    ...
    for (auto const& property : enumClassPropertiesWriteOnly) {
        if (property->getIsVector()) {
            ...
            if (property->getIsObject()) {
                returnStr += INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                             INDENT + INDENT + L"obj->insert" + property->getPropertyName() +
                             L"AtIndex(index, std::static_pointer_cast<" + type +
                             L">(value));\r\n" + INDENT + INDENT + INDENT + L"else\r\n" + INDENT +
                             INDENT + INDENT + INDENT + L"obj->insert" +
                             property->getPropertyName() + L"(std::static_pointer_cast<" + type +
                             L">(value));\r\n";
            } else if (!type.empty() && vcc::isCapital(type)) {
                returnStr += INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                             INDENT + INDENT + L"obj->insert" + property->getPropertyName() +
                             L"AtIndex(index, static_cast<" + type + L">(value));\r\n" + INDENT +
                             INDENT + INDENT + L"else\r\n" + INDENT + INDENT + INDENT + INDENT +
                             L"obj->insert" + property->getPropertyName() + L"(static_cast<" +
                             type + L">(value));\r\n";
            } else if (type == L"string") {
                returnStr += INDENT + INDENT + INDENT + L"if (index > -1)\r\n" + INDENT + INDENT +
                             INDENT + INDENT + L"obj->insert" + property->getPropertyName() +
    ...
    result +=
        L"\r\n"
        "void " +
        propertyName + L"Accessor::_insert" + convertedName +
        L"AtIndex(const int64_t &objectProperty, " +
        (vcc::isEqual(type, objectToken) ? L"" : L"const ") + convertedType + L" " +
        (vcc::isEqual(type, objectToken) ? L"" : L"&") +
        (!vectorCases.empty() ? L"value" : L"/*value*/") + L", const int64_t &" +
...
void VPGPropertyAccessorGenerationService::generateClone(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassPropertiesReadOnly,
    std::wstring& result) {
    ...
    for (auto const& property : enumClassPropertiesReadOnly) {
        if (!property->getIsObject()) continue;
        // General
        if (property->getIsGeneralType()) {
            std::wstring returnStr = L"std::static_pointer_cast<vcc::IObject>(obj->get" +
                                     property->getPropertyName() + L"()->clone())";
            generalCases.insert(std::make_pair(property->getEnum(), returnStr));
        }
        // Vector
        if (property->getIsVector() || property->getIsOrderedMap()) {
            std::wstring returnStr = L"std::static_pointer_cast<vcc::IObject>(obj->clone" +
                                     property->getPropertyName() + L"AtIndex(index))";
            vectorCases.insert(std::make_pair(property->getEnum(), returnStr));
        }
        // Map
        if (property->getIsMap() || property->getIsOrderedMap()) {
            ...
            std::wstring returnStr =
                INDENT + INDENT + INDENT + L"auto keyPtr = static_cast<const " + castType +
                L" *>(key);\r\n" + INDENT + INDENT + INDENT + L"assert(keyPtr != nullptr);\r\n" +
                INDENT + INDENT + INDENT + L"if (keyPtr == nullptr)\r\n" + INDENT + INDENT +
                INDENT + INDENT +
                L"THROW_EXCEPTION_MSG(ExceptionType::KeyInvalid, L\"Invalid Property Accessor Map "
                L"Key\");\r\n" +
                INDENT + INDENT + INDENT +
                L"return std::static_pointer_cast<vcc::IObject>(obj->clone" +
                property->getPropertyName() + L"AtKey(" + mapGetKey + L"));\r\n";
            mapCases.insert(std::make_pair(property->getEnum(), returnStr));
        }
    }
...
void VPGPropertyAccessorGenerationService::generateContainerCount(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassProperties,
    std::wstring& result) {
    if (type != containerToken || enumClassProperties.empty()) return;
    TRY result +=
        L"\r\n"
        "size_t " +
        propertyName +
    ...
    for (auto const& property : enumClassProperties) {
        ...
        result += INDENT + INDENT + L"case " + propertyName + L"::" + property->getEnum() +
                  L":\r\n" + INDENT + INDENT + INDENT + L"return obj->get" +
                  property->getPropertyName() + L"().size();\r\n";
    }
    result += generalTypeContentFooter + INDENT + L"CATCH\r\n" + INDENT +
              L"return 0;\r\n"
              "}\r\n";
    CATCH
}

void VPGPropertyAccessorGenerationService::generateContainerMapKey(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassProperties,
    std::wstring& result) {
    if (type != containerToken || enumClassProperties.empty()) return;
    TRY std::map<std::wstring, std::wstring> mapCases;
    for (auto const& property : enumClassProperties)
        if (property->getIsMap() || property->getIsOrderedMap())
            mapCases.insert(std::make_pair(
                property->getEnum(), L"obj->get" + property->getPropertyName() + L"VoidKeys()"));
...
void VPGPropertyAccessorGenerationService::generateContainerIsContainKey(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassProperties,
    std::wstring& result) {
    if (type != containerToken || enumClassProperties.empty()) return;
        ...
        if (property->getIsMap() || property->getIsOrderedMap()) {
            ...
            mapCases.insert(std::make_pair(
                ...
                INDENT + INDENT + INDENT + L"auto keyPtr = static_cast<const " + castType +
                    ...
                    L"assert(keyPtr != nullptr);\r\n" + INDENT + INDENT + INDENT +
                    L"if (keyPtr == nullptr)\r\n" + INDENT + INDENT + INDENT + INDENT +
                    L"THROW_EXCEPTION_MSG(ExceptionType::KeyInvalid, L\"Invalid Property Accessor "
                    L"Map Key\");\r\n" +
                    INDENT + INDENT + INDENT + L"return obj->is" + property->getPropertyName() +
                    L"ContainKey(" + mapGetKey + L");\r\n"));
        }
...
void VPGPropertyAccessorGenerationService::generateContainerRemove(
    const std::wstring& propertyName, const std::wstring& type,
    const std::vector<std::shared_ptr<VPGEnumClassAttribute>>& enumClassProperties,
    std::wstring& result) {
    if (type != containerToken || enumClassProperties.empty()) return;
    TRY std::map<std::wstring, std::wstring> vectorCasesGeneral;
    ...
    for (auto const& property : enumClassProperties) {
        if (property->getIsVector()) {
            if (property->getIsObject()) {
                std::wstring result = INDENT + INDENT + INDENT + L"obj->remove" +
                                      property->getPropertyName() + L"(value);\r\n";
                ...
                std::wstring castType =
                    (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                        ? L"wchar_t"
                        : property->getType1();
                std::wstring valueAssign =
                    (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                        ? L"valuePtr"
                        : L"*valuePtr";
                std::wstring result =
                    INDENT + INDENT + INDENT + L"auto valuePtr = static_cast<const " + castType +
                    L" *>(value);\r\n" + INDENT + INDENT + INDENT +
                    L"assert(valuePtr != nullptr);\r\n" + INDENT + INDENT + INDENT +
                    L"obj->remove" + property->getPropertyName() + L"(" + valueAssign + L");\r\n";
                vectorCasesGeneral.insert(std::make_pair(property->getEnum(), result));
            }
        }
        if (property->getIsVector() || property->getIsOrderedMap())
            vectorCasesIndex.insert(
                std::make_pair(property->getEnum(),
                               L"obj->remove" + property->getPropertyName() + L"AtIndex(index)"));
        if (property->getIsMap() || property->getIsOrderedMap()) {
            std::wstring castType =
                (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                    ? L"wchar_t"
            ...
            std::wstring mapGetKey =
                (property->getType1() == L"wstring" || property->getType1() == L"std::wstring")
                    ? L"keyPtr"
                    : L"*keyPtr";
            std::wstring result = INDENT + INDENT + INDENT + L"auto keyPtr = static_cast<const " +
                                  castType + L" *>(key);\r\n" + INDENT + INDENT + INDENT +
                                  L"assert(keyPtr != nullptr);\r\n" + INDENT + INDENT + INDENT +
                                  L"if (keyPtr == nullptr)\r\n" + INDENT + INDENT + INDENT +
                                  INDENT +
                                  L"THROW_EXCEPTION_MSG(ExceptionType::KeyInvalid, L\"Invalid "
                                  L"Property Accessor Map Key\");\r\n" +
                                  INDENT + INDENT + INDENT + L"obj->remove" +
                                  property->getPropertyName() + L"AtKey(" + mapGetKey + L");\r\n";
    ...
    result +=
        L"\r\n"
        "void " +
        propertyName + L"Accessor::_removeAtKey(const int64_t &objectProperty, const void *" +
    ...
    for (auto const& property : enumClassProperties) {
        if (!property->getIsCollection()) continue;
        result += INDENT + INDENT + L"case " + propertyName + L"::" + property->getEnum() +
                  L":\r\n" + INDENT + INDENT + INDENT + L"obj->clear" +
...
void VPGPropertyAccessorGenerationService::generateCpp(
    const vcc::LogConfig* logConfig, const std::wstring& projectPrefix,
    const std::map<std::wstring, std::wstring>& projectClassIncludeFiles,
    const std::wstring& filePathCpp,
    const std::vector<std::shared_ptr<VPGEnumClass>>& enumClassList) {
    TRY vcc::LogService::logInfo(logConfig, logId,
                                 L"Generate property accessor cpp file: " + filePathCpp);
    ...
    std::map<std::wstring,
             std::map<std::wstring, std::vector<std::shared_ptr<VPGEnumClassAttribute>>>>
        typeMacroMap;
    // type macro
    for (auto const& enumClass : enumClassList) {
        if (!isPropertyClass(enumClass->getName(), projectPrefix)) continue;
        // include orginal class and enum class
        std::wstring className = getClassNameFromPropertyClassName(enumClass->getName());

        // include object class
        std::wstring includePath = VPGPropertyAccessorGenerationService::getIncludeFile(
            projectClassIncludeFiles, className);
        if (includePath.empty())
            THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                L"Header file of class " + className + L" not found");
        projectIncludeFiles.insert(includePath);

        // include enum class
        includePath = VPGPropertyAccessorGenerationService::getIncludeFile(projectClassIncludeFiles,
                                                                           enumClass->getName());
        if (includePath.empty())
            THROW_EXCEPTION_MSG(
                ExceptionType::CustomError,
                L"Header file of enum class " + enumClass->getName() + L" not found");
        projectIncludeFiles.insert(includePath);
        ...
        for (auto const& property : enumClass->getProperties()) {
            ...
            // not support set
            if (property->getIsSet()) continue;

            bool isContainer = property->getIsCollection();
            if (property->getIsVector()) systemIncludeFiles.insert(L"vector");
            if (property->getIsMap()) systemIncludeFiles.insert(L"map");
            if (property->getIsOrderedMap()) {
                systemIncludeFiles.insert(L"vector");
                systemIncludeFiles.insert(L"map");
            }
            if (property->getIsSet()) systemIncludeFiles.insert(L"set");

            // container
            if (isContainer) {
                if (!vcc::isContain(typeMacroMap[enumClass->getName()], containerToken)) {
                    std::vector<std::shared_ptr<VPGEnumClassAttribute>> initVector;
                    typeMacroMap[enumClass->getName()].insert(
                        std::make_pair(containerToken, initVector));
                }
            ...
            if (!type1.empty()) {
                if (vcc::isCapital(getTypeOrClassWithoutNamespace(type1))) {
                    includePath = VPGPropertyAccessorGenerationService::getIncludeFile(
                        projectClassIncludeFiles, type1);
                    if (includePath.empty())
                        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                            L"Header file of class " + type1 + L" not found");
                    projectIncludeFiles.insert(includePath);
            ...
            if (!type2.empty()) {
                if (vcc::isCapital(getTypeOrClassWithoutNamespace(type2))) {
                    includePath = VPGPropertyAccessorGenerationService::getIncludeFile(
                        projectClassIncludeFiles, type2);
                    if (includePath.empty())
                        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                            L"Header file of class " + type2 + L" not found");
                    projectIncludeFiles.insert(includePath);
            ...
            if (property->getIsObject()) {
                ...
                if (!vcc::isContain(typeMacroMap[enumClass->getName()], (objectToken))) {
                    std::vector<std::shared_ptr<VPGEnumClassAttribute>> initVector;
                    typeMacroMap[enumClass->getName()].insert(
                        std::make_pair(objectToken, initVector));
                }
                typeMacroMap[enumClass->getName()][objectToken].push_back(property);
            } else {
                std::wstring type =
                    property->getIsMap() || property->getIsOrderedMap() ? type2 : type1;
                if (!type.empty() && vcc::isCapital(type)) type = enumToken;
                std::wstring convertedType = L"";
                std::wstring converttedName = L"";
                std::wstring returnResult = L"";
                VPGPropertyAccessorGenerationService::getPropertyAccessorTypeName(
    ...
    std::wstring result =
        L"#include " +
        vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, headerFileName) + L"\r\n";
    // system include files
    if (!systemIncludeFiles.empty()) result += L"\r\n";
    ...
    for (auto const& includeFiles : projectIncludeFiles)
        result += L"#include " +
                  vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, includeFiles) +
                  L"\r\n";
    ...
    for (auto const& enumClass : typeMacroMap) {
        for (auto const& type : getPropertyAccessorTypeOrder()) {
            if (enumClass.second.find(type) != enumClass.second.end()) {
                ...
                VPGPropertyAccessorGenerationService::generateWrite(className, type, writeOnly,
                                                                    code);
                VPGPropertyAccessorGenerationService::generateInsert(className, type, writeOnly,
                                                                     code);
                VPGPropertyAccessorGenerationService::generateClone(className, type, readOnly,
                                                                    code);
                VPGPropertyAccessorGenerationService::generateContainerCount(className, type,
                                                                             readWrite, code);
                VPGPropertyAccessorGenerationService::generateContainerMapKey(className, type,
                                                                              readWrite, code);
                VPGPropertyAccessorGenerationService::generateContainerIsContainKey(className, type,
                                                                                    readOnly, code);
                VPGPropertyAccessorGenerationService::generateContainerRemove(className, type,
                                                                              writeOnly, code);

                std::wstring namespaceName = getNamespaceFromClassName(enumClass.first);
                if (namespaceClassMapping.find(namespaceName) == namespaceClassMapping.end()) {
                    namespaceClassMapping.insert(
                        std::make_pair(namespaceName, std::vector<std::wstring>()));
                }
                namespaceClassMapping[namespaceName].push_back(code);
            }
        }
    ...
    if (count > 0) {
        vcc::writeFile(filePathCpp, result + generateCodeWithNamespace(namespaceClassMapping),
                       true);
        vcc::LogService::logInfo(logConfig, logId, L"Generate property accessor cpp completed.");
    } else {
        if (vcc::isFilePresent(filePathCpp)) {
            vcc::removeFile(filePathCpp);
            vcc::LogService::logInfo(
                logConfig, logId,
                L"Removed property accessor cpp as no properties are acceesable.");
        } else
            vcc::LogService::logInfo(logConfig, logId,
                                     L"No property accessor cpp need to be generated.");
    }
    CATCH
}
