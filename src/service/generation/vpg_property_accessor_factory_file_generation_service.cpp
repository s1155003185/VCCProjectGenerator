...
const std::wstring logId = L"Prorperty Accessor Factory File Generation";

void VPGPropertyAccessorFactoryFileGenerationService::generateHpp(const vcc::LogConfig* logConfig,
                                                                  const std::wstring& filePathHpp) {
    TRY vcc::LogService::logInfo(logConfig, logId,
                                 L"Generate property accessor factory file: " + filePathHpp);
    std::wstring content =
        L""
        "#pragma once\r\n"
        "\r\n"
    ...
    vcc::LogService::logInfo(logConfig, logId,
                             L"Generate property accessor factory file completed.");
    CATCH
}

void VPGPropertyAccessorFactoryFileGenerationService::generateCpp(
    const vcc::LogConfig* logConfig, const std::wstring& projectPrefix,
    const std::set<std::wstring>& includeFiles, const std::wstring& filePathCpp,
    const std::set<std::wstring>& propertyTypes) {
    TRY vcc::LogService::logInfo(logConfig, logId,
                                 L"Generate property accessor factory file: " + filePathCpp);
    ...
    for (auto const& str : tmpIncludePaths)
        content += L"#include " +
                   vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, str) + L"\r\n";
    ...
    vcc::LogService::logInfo(logConfig, logId, L"Generate property accessor factory completed.");
    CATCH
}
