...
std::wstring VPGUnittestFileGenerationService::generateUnittestClass(
    ...
    TRY return L"class " + className + L" : public testing::Test {\r\n" + INDENT +
        ...
        L"\r\n" + INDENT + INDENT + L"}\r\n" + L"\r\n" + INDENT + INDENT +
        getVccTagHeaderCustomClassFunctions(VPGCodeType::Cpp, className) + L"\r\n" + INDENT +
        INDENT + getVccTagTailerCustomClassFunctions(VPGCodeType::Cpp, className) + L"\r\n" +
        L"};\r\n";
    CATCH
    return L"";
}

std::wstring VPGUnittestFileGenerationService::generateUnittestTestCase(
    const std::wstring& testClassName, const std::wstring& testName) {
    TRY return L"\r\n"
               "TEST_F(" +
        testClassName + L", " + testName + L") {\r\n" + INDENT +
        ...
        getVccTagTailerCustomClassCustomFunctions(VPGCodeType::Cpp, L"custom",
                                                  testClassName + testName, L"") +
        L"\r\n" + L"};\r\n";
    CATCH
    return L"";
...
void VPGUnittestFileGenerationService::generateCppAction(
    ...
    const std::vector<std::wstring>& unittestNames, const bool& isSeperateFile) {
    TRY if (unittestNames.empty()) return;
    ...
    if (!isSeperateFile) {
        if (enumClassList.size() == 1) {
            std::wstring className = getClassNameFromPropertyClassName(
                getTypeOrClassWithoutNamespace(enumClassList.front()->getName()));
            ...
            testClassName =
                getClassNameFromFileName(originalPropertyFilePath, projectPrefix) + L"ActionTest";
        }
        content += header + L"\r\n" +
    ...
    for (auto const& enumClass : enumClassList) {
        ...
        for (auto const& property : enumClass->getProperties()) {
            ...
            for (auto const& unittestName : unittestNames) {
                if (isSeperateFile)
                    content += VPGUnittestFileGenerationService::generateUnittestTestCase(
                        unittestClassName, unittestName);
                else
                    content += VPGUnittestFileGenerationService::generateUnittestTestCase(
                        unittestClassName, actionClassName + L"_" + unittestName);
            }

            if (isSeperateFile) {
                // Generate to seperate files
                std::wstring fileNameCpp =
                    getActionFileNameWithoutExtension(
                        getTypeOrClassWithoutNamespace(
                ...
                vcc::LogService::logInfo(logConfig, logId,
                                         L"Generate unittest action unittest file: " + filePathCpp);
                if (vcc::isFilePresent(filePathCpp))
                    content = VPGFileSyncService::SyncFileContent(
                        VPGFileContentSyncTagMode::Generation, content, vcc::readFile(filePathCpp),
                        VPGFileContentSyncMode::Full, L"//");
                vcc::lTrim(content);
                vcc::writeFile(filePathCpp, content, true);
                vcc::LogService::logInfo(logConfig, logId,
                                         L"Generate unittest action unittest file completed.");
    ...
    if (!isSeperateFile && isGenerated) {
        ...
        vcc::LogService::logInfo(logConfig, logId,
                                 L"Generate action unittest file: " + filePathCpp);
        if (vcc::isFilePresent(filePathCpp))
