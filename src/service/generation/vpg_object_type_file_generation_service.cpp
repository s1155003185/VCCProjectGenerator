...
void VPGObjectTypeFileGenerationService::generate(const vcc::LogConfig* logConfig,
                                                  const std::wstring& filePathHpp,
                                                  const std::set<std::wstring>& propertyTypes) {
    TRY vcc::LogService::logInfo(logConfig, logId, L"Generate object type file: " + filePathHpp);
    ...
    if (!overallPropertyTypes.empty())
        content +=
            INDENT + L", " + vcc::concat(overallPropertyTypes, L"\r\n" + INDENT + L", ") + L"\r\n";
