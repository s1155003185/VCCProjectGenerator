...
void VPGActionFileGenerationService::generateHpp(
    const vcc::LogConfig* logConfig, const std::map<std::wstring, std::wstring>& classPathMapping,
    const VPGEnumClass* enumClass, const std::wstring& projectPrefix,
    const std::wstring& folderPathHpp, std::map<std::wstring, std::wstring>& actionClassList,
    std::set<std::wstring>& globalSystemIncludeFiles,
    std::set<std::wstring>& globalCustomIncludeFiles) {
    TRY assert(enumClass != nullptr);
    if (!isPropertyClassNameValidToGenerateClass(enumClass->getName(), projectPrefix)) return;
    ...
    for (auto const& property : enumClass->getProperties()) {
        ...
        std::wstring actionClassName =
            getTypeOrClassWithoutNamespace(className) + property->getPropertyName();

        std::wstring propertyStr = L"";
        std::wstring assignmentStrSimple =
        ...
        if (!type1.empty() && vcc::isCapital(type1)) {
            customIncludeFiles.insert(VPGObjectFileGenerationService::getProjectClassIncludeFile(
                classPathMapping, type1));

            propertyStr = INDENT + L"GETSET_SPTR_NULL(" + type1 + L", Argument)\r\n";

            if (!assignmentStr.empty()) assignmentStr += L", ";
        ...
        std::wstring action =
            ...
            L"\r\n" +
            INDENT + L"private:\r\n" + INDENT + INDENT +
            getVccTagHeaderCustomClassPrivateFunctions(VPGCodeType::Cpp, actionClassName) +
            L"\r\n" + INDENT + INDENT +
            getVccTagTailerCustomClassPrivateFunctions(VPGCodeType::Cpp, actionClassName) +
            L"\r\n"
            "\r\n" +
            INDENT + L"protected:\r\n" + INDENT + INDENT +
            L"virtual std::wstring getRedoMessageStart() const override;\r\n" + INDENT + INDENT +
            L"virtual std::wstring getRedoMessageComplete() const override;\r\n";

        if (!property->getIsNoHistory())
            action += INDENT + INDENT +
                      L"virtual std::wstring getUndoMessageStart() const override;\r\n" + INDENT +
                      INDENT + L"virtual std::wstring getUndoMessageComplete() const override;\r\n";

        action += L"\r\n" + INDENT + INDENT +
                  L"virtual std::shared_ptr<vcc::IResult> onRedo() override;\r\n";

        if (!property->getIsNoHistory())
            action +=
                INDENT + INDENT + L"virtual std::shared_ptr<vcc::IResult> onUndo() override;\r\n";

        action += L"\r\n" + INDENT + INDENT +
                  getVccTagHeaderCustomClassProtectedFunctions(VPGCodeType::Cpp, actionClassName) +
                  L"\r\n" + INDENT + INDENT +
                  getVccTagTailerCustomClassProtectedFunctions(VPGCodeType::Cpp, actionClassName) +
                  L"\r\n"
                  "\r\n" +
                  INDENT + L"public:\r\n" + INDENT + INDENT + actionClassName +
                  L"() : vcc::BaseAction() {}\r\n" + INDENT + INDENT + actionClassName + L"(" +
                  assignmentStrSimple + L");\r\n";
        if (assignmentStr != assignmentStrSimple)
            action += INDENT + INDENT + actionClassName + L"(" + assignmentStr + L");\r\n";
        action += INDENT + INDENT + L"~" + actionClassName +
                  L"() {}\r\n"
                  "\r\n" +
                  INDENT + INDENT +
                  getVccTagHeaderCustomClassPublicFunctions(VPGCodeType::Cpp, actionClassName) +
                  L"\r\n" + INDENT + INDENT +
                  getVccTagTailerCustomClassPublicFunctions(VPGCodeType::Cpp, actionClassName) +
                  L"\r\n" + L"};\r\n";
        ...
        if (isSeperateFile) {
            ...
            for (auto const& str : customIncludeFiles)
                content += L"#include " +
                           vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, str) +
                           L"\r\n";
            ...
            // Generate File
            std::wstring filePathHpp = vcc::concatPaths(
                {folderPathHpp,
                 getActionFileNameWithoutExtension(actionClassName, projectPrefix) + L".hpp"});
            vcc::LogService::logInfo(logConfig, logId,
                                     L"Generate action class file: " + filePathHpp);
            ...
            actionClassList.insert(
                {(!currentNamespace.empty() ? (currentNamespace + L"::") : L"") + actionClassName,
                 action});
        }
    }
    CATCH
...
void VPGActionFileGenerationService::generateCpp(
    const vcc::LogConfig* logConfig, const std::map<std::wstring, std::wstring>& classPathMapping,
    const VPGEnumClass* enumClass, const std::wstring& projectPrefix,
    const std::wstring& folderPathCpp, std::map<std::wstring, std::wstring>& actionClassList,
    std::set<std::wstring>& globalSystemIncludeFiles,
    std::set<std::wstring>& globalCustomIncludeFiles) {
    TRY assert(enumClass != nullptr);
    if (!isPropertyClassNameValidToGenerateClass(enumClass->getName(), projectPrefix)) return;
    ...
    for (auto const& property : enumClass->getProperties()) {
        ...
        propertyAssignments.insert(propertyAssignments.end(), propertyAssignmentsSimple.begin(),
                                   propertyAssignmentsSimple.end());
        ...
        if (!vcc::isBlank(property->getActionResultRedoClass()))
            customIncludeFiles.insert(VPGObjectFileGenerationService::getProjectClassIncludeFile(
                classPathMapping, property->getActionResultRedoClass()));
        if (!vcc::isBlank(property->getActionResultUndoClass()))
            customIncludeFiles.insert(VPGObjectFileGenerationService::getProjectClassIncludeFile(
                classPathMapping, property->getActionResultUndoClass()));
        ...
        action +=
            ...
            INDENT + INDENT +
            getVccTagTailerCustomClassCustomFunctions(VPGCodeType::Cpp, L"", actionClassName,
                                                      L"GetRedoMessageStart") +
            L"\r\n" + INDENT + L"CATCH\r\n" + INDENT +
            L"return L\"\";\r\n"
            "}\r\n"
            ...
            L"\r\n" + INDENT + INDENT + L"return L\"execute " + actionClassName +
            L" complete\";\r\n" + INDENT + INDENT +
            getVccTagTailerCustomClassCustomFunctions(VPGCodeType::Cpp, L"", actionClassName,
                                                      L"GetRedoMessageComplete") +
            L"\r\n" + INDENT + L"CATCH\r\n" + INDENT +
        ...
        if (!property->getIsNoHistory())
            action +=
                ...
                L"\r\n" + INDENT + INDENT + L"return L\"Undo " + actionClassName +
                L" start\";\r\n" + INDENT + INDENT +
                getVccTagTailerCustomClassCustomFunctions(VPGCodeType::Cpp, L"", actionClassName,
                                                          L"GetUndoMessageStart") +
                L"\r\n" + INDENT + L"CATCH\r\n" + INDENT +
        ...
        std::wstring redoReturnClass = !vcc::isBlank(property->getActionResultRedoClass())
                                           ? property->getActionResultRedoClass()
                                           : L"vcc::OperationResult";
        action +=
            L"\r\n"
            ...
            L"return std::make_shared<" + redoReturnClass +
            L">();\r\n"
            "}\r\n";

        if (!property->getIsNoHistory()) {
            std::wstring undoReturnClass = !vcc::isBlank(property->getActionResultUndoClass())
                                               ? property->getActionResultUndoClass()
                                               : L"vcc::OperationResult";
            action +=
                L"\r\n"
                "std::shared_ptr<vcc::IResult> " +
                ...
                L"return std::make_shared<" + undoReturnClass +
                L">();\r\n"
                "}\r\n";
        }
        ...
        if (isSeperateFile) {
            // Generate to seperate files
            std::wstring fileNameHpp =
                getActionFileNameWithoutExtension(actionClassName, projectPrefix) + L".hpp";
            std::wstring content =
                L"// <vcc:vccproj sync=\"FULL\" gen=\"FULL\"/>\r\n"
                "#include " +
                vcc::getEscapeStringWithQuote(vcc::EscapeStringType::DoubleQuote, fileNameHpp) +
                L"\r\n"
                "\r\n";
            // system include files
            for (auto const& str : systemIncludeFiles) content += L"#include <" + str + L">\r\n";
            ...
            content += L"\r\n" + getVccTagHeaderCustomHeader(VPGCodeType::Cpp) + L"\r\n" +
                       getVccTagTailerCustomHeader(VPGCodeType::Cpp) + L"\r\n" +
                       generateCodeWithNamespace(namespaceClassMapping) + L"\r\n" +
                       getVccTagHeaderCustomClassFunctions(VPGCodeType::Cpp, L"") + L"\r\n" +
                       getVccTagTailerCustomClassFunctions(VPGCodeType::Cpp, L"") + L"\r\n";

            // Generate File
            std::wstring filePathCpp = vcc::concatPaths(
                {folderPathCpp,
                 getActionFileNameWithoutExtension(actionClassName, projectPrefix) + L".cpp"});
            vcc::LogService::logInfo(logConfig, logId,
                                     L"Generate action class file: " + filePathCpp);
