...
bool isFileStartWithProjectPrefix(const std::wstring& str, const std::wstring& projectPrefix) {
    TRY if (projectPrefix.empty()) return true;
...
bool isPropertyClassNameValidToGenerateClass(const std::wstring& str,
    ...
    TRY return isClassStartWithProjectPrefix(str, projectPrefix) &&
        vcc::isEndWith(str, propertyClassSuffix);
    CATCH
    return false;
}

bool isPropertyFile(const std::wstring& filePath, const std::wstring& projectPrefix) {
    TRY return isFileStartWithProjectPrefix(vcc::getFileName(filePath), projectPrefix) &&
        vcc::isEndWith(filePath, propertyClassFileSuffix);
    CATCH
    return false;
}

bool isPropertyClass(const std::wstring& className, const std::wstring& projectPrefix) {
    TRY return isClassStartWithProjectPrefix(className, projectPrefix) &&
        vcc::isEndWith(className, propertyClassSuffix);
    CATCH
    return false;
}

bool isCustomType(const std::wstring& value) {
    TRY if (value.empty()) return false;
...
std::wstring getNamespaceFromClassName(const std::wstring& className) {
    TRY auto namespaceList = vcc::splitString(className, {L"::"});
    if (namespaceList.size() < 2) return L"";
...
bool isClassInNamespace(const std::wstring& currentNamespace, const std::wstring& fullClassName) {
    TRY return currentNamespace == getNamespaceFromClassName(fullClassName);
    CATCH
    return false;
}
...
std::wstring getActionClassName(const VPGEnumClass* enumClass,
    ...
    TRY return getClassNameFromPropertyClassName(enumClass->getName()) +
...
std::wstring getActionFileNameWithoutExtension(const std::wstring& actionClassName,
    ...
    TRY std::wstring currentActionClassName = getTypeOrClassWithoutNamespace(actionClassName);
    std::wstring projectPrefixLower = projectPrefix;
    vcc::toLower(projectPrefixLower);
    if (!vcc::isBlank(projectPrefix) &&
        vcc::isStartWith(getTypeOrClassWithoutNamespace(actionClassName), projectPrefix)) {
...
std::wstring getClassNameFromPropertyClassName(const std::wstring& className) {
    TRY if (vcc::isEndWith(className, propertyClassSuffix) &&
            className.length() > propertyClassSuffix.length()) return className
        .substr(0, className.length() - propertyClassSuffix.length());
    CATCH
    return className;
}
...
std::wstring getClassNameFromFileName(const std::wstring& fileName,
                                      const std::wstring& projectPrefix) {
    TRY std::wstring baseFileName = fileName;
    if (vcc::isEndWith(baseFileName, propertyClassFileSuffix) &&
        baseFileName.length() > propertyClassFileSuffix.length())
        baseFileName =
            baseFileName.substr(0, baseFileName.length() - propertyClassFileSuffix.length());

    baseFileName = baseFileName.find('.') != std::wstring::npos
                       ? baseFileName.substr(0, baseFileName.find('.'))
                       : baseFileName;
    ...
    baseFileName = vcc::convertNamingStyle(baseFileName, vcc::NamingStyle::ConstantCase,
