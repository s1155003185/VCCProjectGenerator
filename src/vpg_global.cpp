...
std::wstring VPGGlobal::getVersion() { return L"v0.4.3"; }

std::wstring VPGGlobal::getVccLocalResponseFolder() {
    TRY return vcc::concatPaths({USER_HOME_VARIABLE, L"Documents", L"vcc"});
    CATCH
    return L"";
}
...
std::wstring VPGGlobal::getVccProjectLocalResponseDirectory(VPGProjectType projectType) {
    TRY return vcc::concatPaths(
        {VPGGlobal::getVccLocalResponseFolder(), VPGGlobal::getProjectName(projectType)});
    CATCH
    return L"";
}
...
std::wstring VPGGlobal::getVccJsonFileName() { return L".vcc/vcc.json"; }

std::wstring VPGGlobal::getCppDefaultIncludePathWindow() {
    return L"C:\\msys64\\mingw64\\include\\c++\\12.2.0";
}

std::wstring VPGGlobal::getCppDefaultIncludePathLinux() { return L"/usr/include"; }

std::wstring VPGGlobal::getCppDefaultIncludePathMacOs() {
    return L"/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/"
           L"MacOSX.sdk/usr/include/c++/V1";
}

std::wstring VPGGlobal::getVCCProjectManagerConfigFileName() { return L"vpm.config.json"; }

std::wstring VPGGlobal::getVCCProjectManagerConfigFileFullPath() {
    return VPGGlobal::getConvertedPath(
        vcc::concatPaths({VPGGlobal::getVccLocalResponseFolder(), L"config",
                          VPGGlobal::getVCCProjectManagerConfigFileName()}));
}
...
std::wstring VPGGlobal::getConvertedPath(const std::wstring& path) {
    ...
    TRY std::wstring userHomePath = vcc::getSystemFolderPath(vcc::SystemFolderType::UserHome);
...
void VPGGlobal::initialize(const std::wstring& projectWorkspace) {
    VPGGlobal::_FileGenerationManager = std::make_unique<VPGFileGenerationManager>(
        std::make_shared<vcc::LogConfig>(vcc::LogConfigInitialType::None), L"");
    VPGGlobal::_FileGenerationManager->getClassMacroList(projectWorkspace);
    VPGGlobal::_EnumClassReader =
        std::make_shared<VPGEnumClassReader>(VPGGlobal::_FileGenerationManager->getClassMacros());
}
