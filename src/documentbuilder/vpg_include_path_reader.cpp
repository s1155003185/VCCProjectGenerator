...
size_t VPGIncludePathReader::SkipCommand(const std::wstring& cppCode,
                                         const size_t& openCommandIndex, size_t& pos) const {
    TRY std::wstring closeCommand = _CloseCommands[openCommandIndex];
    size_t tmpPos = vcc::find(cppCode, closeCommand, pos);
    if (tmpPos != std::wstring::npos) pos = tmpPos + closeCommand.length() - 1;
    CATCH
    return false;
}

void VPGIncludePathReader::parseCustom(const std::wstring& cppCode,
                                       const std::wstring& currentNamespace,
                                       std::set<std::wstring>& classList) const {
    if (cppCode.empty()) return;
    TRY size_t pos = 0;
                ...
                if (isNamespaceTriggered) {
                    ...
                    if (nextToken == L"namespace") {
                        ...
                        if (nextToken == L"class") {
                            ...
                            nextToken =
                                vcc::getNextString(cppCode, pos, _Delimiter, _OpenCommandAndQuotes,
                                                   _CloseCommandAndQuotes);
                            vcc::trim(nextToken);
                            vcc::getNextCharPos(cppCode, pos, false);
                            // check if it is :
                            if (cppCode[pos] == L';') continue;
                            if (cppCode[pos] == L':') {
                            ...
                            if (!vcc::getNextQuotedString(cppCode, pos, {L";"}).empty())
                                classList.insert(
                                    (!currentNamespace.empty() ? (currentNamespace + L"::") : L"") +
                                    nextToken);
...
// void VPGIncludePathReader::ParseLinux(const std::wstring &cppCode, std::set<std::wstring>
// &classList) const
// {
//     if (cppCode.empty())
//         return;
...
// void VPGIncludePathReader::ParseMacOs(const std::wstring &cppCode, std::set<std::wstring>
// &classList) const
// {
//     if (cppCode.empty())
//         return;
...
//         parseCustom(cppCode.substr(startCommandPos, endCommandPos - startCommandPos), L"",
//         classList);
//     }
// }

// void VPGIncludePathReader::ParseWindow(const std::wstring &cppCode, std::set<std::wstring>
// &classList) const
// {
//     if (cppCode.empty())
//         return;
...
void VPGIncludePathReader::parse(const vcc::PlatformType& platformType, const std::wstring& cppCode,
                                 std::set<std::wstring>& classList) const {
    TRY switch (platformType) {
