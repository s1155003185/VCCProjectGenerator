...
std::wstring VPGEnumClassReader::_getErrorMessage(const std::wstring& str, const size_t& pos,
    ...
    return L"Error at position " + std::to_wstring(pos + 1) + L", row " + std::to_wstring(row) +
           L", column " + std::to_wstring(column) + L": " + preString + L"<<" +
           std::wstring(1, str[pos]) + L">>" + subString + L"\r\n " + L" Error message: " + msg;
}

size_t VPGEnumClassReader::isCommand(const std::wstring& cppCode, const size_t& pos) const {
    TRY size_t index = 0;
    for (auto const& quote : _OpenCommands) {
        if (vcc::isStartWith(cppCode, quote, pos)) return index;
        index++;
    }
    CATCH
...
std::wstring VPGEnumClassReader::_getEnum(const std::wstring& propertyStr, size_t& pos) const {
    ...
    TRY vcc::getNextCharPos(propertyStr, pos, true);
...
std::wstring VPGEnumClassReader::_getMacro(const std::wstring& propertyCommand, size_t& pos) const {
    std::wstring result = L"";
    TRY if (this->_ClassMacroList.empty()) return result;
    ...
    pos = posOfQuote;
    result +=
        vcc::getNextQuotedString(propertyCommand, pos, {L")", L" ", L"\t", L"\r", L"\n"},
                                 {L"\"", L"'", L"{", L"[", L"(", L"/*", L"//"},
                                 {L"\"", L"'", L"}", L"]", L")", L"*/", L"\n"},
                                 {L"\\", L"\\", L"", L"", L"", L"", L""}, {L"\"", L"//", L"/*"});
    vcc::trim(result);
...
std::wstring VPGEnumClassReader::_getPropertyName(const std::wstring& macroStr, size_t& pos) const {
    std::wstring result = L"";
    TRY size_t endPos = vcc::find(macroStr, L",", pos);
    if (endPos == std::wstring::npos) {
...
std::wstring VPGEnumClassReader::_getDefaultValue(const std::wstring& macroStr, size_t& pos) const {
    std::wstring result = L"";
    TRY size_t endPos = macroStr.find_last_of(L")");
    if (endPos == std::wstring::npos)
        THROW_EXCEPTION_MSG(ExceptionType::ParserError, macroStr + L": Macro ) missing");
...
std::shared_ptr<vcc::Json> VPGEnumClassReader::getJsonAttributes(
    const std::wstring& command, const std::wstring& attributeName) const {
    TRY size_t pos = vcc::find(command, attributeName, 0, true);
    if (pos == std::wstring::npos) return nullptr;
    pos += attributeName.length();
    vcc::getNextCharPos(command, pos, true);
    if (command[pos] == L'{') {
        std::wstring jsonStr =
            vcc::getNextQuotedString(command, pos, {L" ", L"\t", L"\n"}, {L"{", L"[", L"\"", L"'"},
                                     {L"}", L"]", L"\"", L"'"}, {L"", L"", L"\\", L"\\"}, {L"\""});
        TRY auto json = std::make_shared<vcc::Json>();
...
std::vector<std::wstring> VPGEnumClassReader::getAttribute(const std::wstring& str) const {
    ...
    if (!vcc::isStartWith(str, attributePrefix, pos)) return result;
...
void VPGEnumClassReader::_assignEnumClassProperty(
    const VPGEnumClass* enumClass, const std::wstring& propertyCommand,
    std::vector<std::shared_ptr<VPGEnumClassAttribute>>& properties) const {
    TRY auto property = std::make_shared<VPGEnumClassAttribute>();
    ...
    for (auto const& attribute : attributes) {
        ...
        // Action
            ...
            if (jsonAttributes == nullptr ||
                !((jsonAttributes->isContainKey(L"Redo.Class") &&
                   !vcc::isBlank(jsonAttributes->getString(L"Redo.Class"))) ||
                  (jsonAttributes->isContainKey(L"Undo.Class") &&
                   !vcc::isBlank(jsonAttributes->getString(L"Undo.Class")))))
                THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                                    L"Enum Class " + enumClass->getName() +
                                        L" has attribute @@ActionResult but missing Attribute "
                                        L"\"Redo.Class\" or \"Undo.Class\"");
            if (jsonAttributes->isContainKey(L"Redo.Class") &&
                !vcc::isBlank(jsonAttributes->getString(L"Redo.Class")))
                property->setActionResultRedoClass(jsonAttributes->getString(L"Redo.Class"));
            if (jsonAttributes->isContainKey(L"Undo.Class") &&
                !vcc::isBlank(jsonAttributes->getString(L"Undo.Class")))
    ...
    for (auto macro : macroList) {
        ...
        if (currentProperty->getIsManager()) {
            ...
            if (currentProperty->getMacro()[pos] == L',') {
                pos++;
                currentProperty->setPropertyName(
                    _getPropertyName(currentProperty->getMacro(), pos));
            }
            if (currentProperty->getMacro()[pos] == L',') {
                pos++;
                currentProperty->setDefaultValue(
                    _getDefaultValue(currentProperty->getMacro(), pos));
            }
            ...
            if (currentProperty->getIsMap() || currentProperty->getIsOrderedMap()) {
                ...
                if (currentProperty->getMacro()[pos] == L',') {
                    pos++;
                    currentProperty->setValidate(
                        _getDefaultValue(currentProperty->getMacro(), pos));
                }
            } else if (currentProperty->getMacroType() == VPGEnumClassMacroType::Setcustom ||
                       currentProperty->getMacroType() == VPGEnumClassMacroType::SetcustomSptr) {
                currentProperty->setPropertyName(_getType(currentProperty->getMacro(), pos));
...
std::wstring VPGEnumClassReader::_getCommand(const std::wstring& cppCode,
                                             const bool& isClassCommand, size_t& pos) const {
    std::wstring result = L"";
    TRY vcc::getNextCharPos(cppCode, pos, true);
    while (vcc::isStartWith(cppCode, L"//", pos) || vcc::isStartWith(cppCode, L"/*", pos)) {
        std::wstring tmpCmd =
            vcc::getNextQuotedString(cppCode, pos, {L";", L"{", L"\n", L" ", L"/*", L"//"},
                                     {L"/*", L"//"}, {L"*/", L"\n"}, {L"", L""}, {L"/*", L"//"});
        vcc::trim(tmpCmd);
        if (!result.empty()) result += L"\r\n";
        if (vcc::isStartWith(tmpCmd, L"//")) {
            tmpCmd =
                tmpCmd.substr(2, tmpCmd.length() - 2);  // only minus prefix as already vcc::trim
        ...
        if (isClassCommand) {
            ...
            if (nextNewLinePos != std::wstring::npos && pos != std::wstring::npos &&
                nextNewLinePos < pos &&
                (vcc::isStartWith(cppCode, L"//", pos) || vcc::isStartWith(cppCode, L"/*", pos))) {
                result = L"";
            }
            pos = currentPos;
        }
...
void VPGEnumClassReader::_parseProperties(const std::wstring& cppCode, size_t& pos,
                                          std::shared_ptr<VPGEnumClass> enumClass) const {
    TRY _EnumValue = -1;
    while (pos < cppCode.size()) {
        ...
        if (cppCode[pos] == L',') vcc::getNextCharPos(cppCode, pos, false);
        ...
        if (properties.empty()) properties.push_back(std::make_shared<VPGEnumClassAttribute>());

        for (auto property : properties) {
            property->setEnum(enumName);
            property->setEnumValue(_EnumValue);
        }
        enumClass->getProperties().insert(enumClass->getProperties().end(), properties.begin(),
                                          properties.end());
...
bool VPGEnumClassReader::_parseClass(const std::wstring& cppCode, size_t& pos,
                                     std::shared_ptr<VPGEnumClass> enumClass) const {
    TRY if (!vcc::isStartWith(cppCode, L"enum", pos)) THROW_EXCEPTION_MSG(
        ExceptionType::ParserError, _getErrorMessage(cppCode, pos, L"enum missing."));
    ...
    if (!std::iswalpha(cppCode[pos]))
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            _getErrorMessage(cppCode, pos, L"Class Name missing."));
            ...
            if (vcc::isStartWith(attribute, attributePrefix + L"Form", 0, true)) {
                ...
                if (vcc::isBlank(className))
                    THROW_EXCEPTION_MSG(
                        ...
                        L"Enum Class " + enumClass->getName() +
                            L" has attribute @@Inherit but missing Attribute \"Class\"");
                ...
                if (jsonAttributes != nullptr)
                    enumClass->setIsActionManagerIndependent(
                        jsonAttributes->getBool(L"IsIndependent"));
                command = L"";
                ...
                if (jsonAttributes != nullptr)
                    enumClass->setIsThreadManagerIndependent(
                        jsonAttributes->getBool(L"IsIndependent"));
                command = L"";
    ...
    if (cppCode[pos] == L';') {
        ...
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            _getErrorMessage(cppCode, pos, L"{ missing."));
    vcc::getNextCharPos(cppCode, pos, false);
...
std::wstring VPGEnumClassReader::getCppCodeLine(const std::wstring& str, size_t& pos,
                                                bool fromCurrentPos) const {
    if (!fromCurrentPos) pos++;
    size_t startPos = pos;
    TRY pos = vcc::find(str, L"\n", pos);
    std::wstring tailingSubstr =
        vcc::getTailingSubstring(str.substr(startPos, pos - startPos + 1), 3);
...
void VPGEnumClassReader::parseCustom(const std::wstring& cppCode,
                                     const std::wstring& currentNamespace,
                                     std::vector<std::shared_ptr<VPGEnumClass>>& results) const {
    TRY size_t pos = 0;
    ...
    while (pos < cppCode.size()) {
        ...
        if (commandIndex != std::wstring::npos) {
            ...
            vcc::trim(tmpCmd);
            if (!currentCommand.empty()) currentCommand += L"\r\n";
            currentCommand += tmpCmd;
        } else {
            size_t previousPos = pos;
            std::wstring nextToken = vcc::getNextString(
                cppCode, pos, _Delimiter, _OpenCommandAndQuotes, _CloseCommandAndQuotes);
            if (!vcc::isBlank(nextToken)) {
                if (isNamespaceTriggered) {
                    ...
                    if (!quoteStr.empty()) {
                        ...
                        parseCustom(quoteStr.substr(1, quoteStr.length() - 2), nextToken,
                                    tmpClassList);
                    ...
                    if (nextToken == L"namespace") {
                        ...
                        if (isFullEnumClass) results.push_back(enumClass);
                    }
                }
                currentCommand = L"";
            }
...
void VPGEnumClassReader::parse(const std::wstring& cppCode,
                               std::vector<std::shared_ptr<VPGEnumClass>>& results) const {
    TRY std::unique_lock lock(_mutex);
