...
std::wstring XmlBuilder::getString(const std::wstring& xmlData, size_t& pos) const {
    size_t endPos = pos;
    TRY if (xmlData.empty() || xmlData[pos] != L'"') THROW_EXCEPTION_MSG(
        ExceptionType::ParserError, getErrorMessage(xmlData, pos, L"char is not \""));
    ...
    while (endPos < xmlData.length()) {
        ...
        if (endPos >= xmlData.length())
            THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                                getErrorMessage(xmlData, pos, L"ending \" missing"));
    }
    CATCH
    // remove head and tail "
    std::wstring result = getUnescapeString(EscapeStringType::XML,
                                            xmlData.substr(pos + 1, (endPos - 1) - (pos + 1) + 1));
    pos = endPos;
    return result;
}
...
std::wstring XmlBuilder::getTag(const std::wstring& xmlData, size_t& pos) const {
    size_t endPos = pos;
    TRY if (xmlData.empty()) return L"";

    while (endPos < xmlData.length()) {
        if (!std::iswdigit(xmlData[endPos]) && !std::iswalpha(xmlData[endPos]) &&
            xmlData[endPos] != L'_' && xmlData[endPos] != L':' && xmlData[endPos] != L'-') {
            endPos--;
            break;
        }
        endPos++;
    ...
    CATCH
    std::wstring result =
        getUnescapeString(EscapeStringType::XML, xmlData.substr(pos, endPos - pos + 1));
    pos = endPos;
    return result;
}

bool XmlBuilder::isNextCharTagEnd(const std::wstring& xmlData, size_t& pos) const {
    TRY if (xmlData[pos] == L'/') {
        if (pos + 1 >= xmlData.length())
            THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                                getErrorMessage(xmlData, pos, L"> missing"));
        if (xmlData[pos + 1] == L'>') {
            pos++;
            return true;
        } else
...
bool XmlBuilder::parseXMLTagHeader(const std::wstring& xmlData, size_t& pos,
                                   std::shared_ptr<Xml> element) const {
    TRY if (xmlData[pos] != L'<') THROW_EXCEPTION_MSG(
        ExceptionType::ParserError,
        getErrorMessage(xmlData, pos, L"char is not < but " + std::wstring(1, xmlData[pos])));
    ...
    if (tagName.empty()) {
        ...
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            getErrorMessage(xmlData, pos, L"tag name missing"));
    }
    pos++;
    if (pos >= dataLength)
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
...
void XmlBuilder::parseXMLTagContent(const std::wstring& xmlData, size_t& pos,
                                    std::shared_ptr<Xml> element) const {
    TRY while (pos < xmlData.length()) {
        ...
        if (!child->_Name.empty()) {
            ...
            if (!child->_FullText.ends_with(endTag))
                THROW_EXCEPTION_MSG(
                    ExceptionType::ParserError,
                    getErrorMessage(xmlData, pos, L"end tab " + endTag + L" missing"));
        } else {
...
void XmlBuilder::parseXMLTag(const std::wstring& xmlData, size_t& pos,
                             std::shared_ptr<Xml> element) const {
    TRY if (xmlData.empty()) return;
    if (xmlData[pos] != L'<')
        THROW_EXCEPTION_MSG(
            ExceptionType::ParserError,
            getErrorMessage(xmlData, pos, L"char is not < but " + std::wstring(1, xmlData[pos])));
    ...
    // content
    if (!parseXMLTagHeader(xmlData, pos, element)) return;
    pos++;
...
void XmlBuilder::parseXml(const std::wstring& xmlData, size_t& pos,
                          std::shared_ptr<Xml> element) const {
    size_t dataLength = xmlData.length();
    ...
    element->_FullText =
        pos < dataLength ? xmlData.substr(startPos, pos - startPos + 1) : xmlData.substr(startPos);
    CATCH
}
...
void XmlBuilder::deserialize(const std::wstring& str, size_t& pos,
                             std::shared_ptr<IDocument> doc) const {
    TRY auto xmlObject = std::dynamic_pointer_cast<Xml>(doc);
