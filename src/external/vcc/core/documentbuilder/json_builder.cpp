...
std::wstring JsonBuilder::serialize(const IDocument* doc) const {
    std::wstring result = L"";
    TRY std::wstring currentNewLineCharacter = _IsBeautify ? _NewLineCharacter : L"";
    ...
    switch (jsonObj->getJsonInternalType()) {
        ...
        case JsonInternalType::String:
            result = getEscapeStringWithQuote(EscapeStringType::DoubleQuote,
                                              jsonObj->getJsonInternalValue());
            break;
        // Object
        case JsonInternalType::Array: {
            _Level++;
            for (auto const& element : jsonObj->getJsonInternalArray()) {
                if (!result.empty()) result += L"," + currentNewLineCharacter;
                result += getCurrentIndent() + serialize(element.get());
            }
            _Level--;
            result = L"[" + currentNewLineCharacter + result + currentNewLineCharacter +
                     getCurrentIndent() + L"]";
            break;
        }
        ...
        case JsonInternalType::Json: {
            ...
            for (auto const& pair : jsonObj->getJsonInternalNameValuePairs()) {
                ...
                result += getCurrentIndent() +
                          getEscapeStringWithQuote(EscapeStringType::DoubleQuote, pair.first);
...
void JsonBuilder::ParseJsonObject(const std::wstring& str, size_t& pos,
                                  std::shared_ptr<Json> doc) const {
    size_t startPos = pos;
    try {
        ...
        if (isStartWith(str, nullStr, pos)) {
            ...
            std::wstring value = getNextQuotedString(
                str, pos, {L",", L"}", L"]"}, {L"\"", L"'", L"{", L"["}, {L"\"", L"'", L"}", L"]"},
                {L"\\", L"\\", L"\\", L"\\"}, {L"\"", L"'"});
                ...
                while (pos < str.length()) {
                    ...
                    if (str[pos] == L']')
                        ...
                        THROW_EXCEPTION_MSG(
                            ...
                            getErrorMessage(str, pos, L"Array elements not end with , or ]"));
                }
            }
        } else {
            std::wstring numStr = getNextQuotedString(
                str, pos, {L",", L"}", L"]"}, {L"\"", L"'", L"{", L"["}, {L"\"", L"'", L"}", L"]"},
                {L"\\", L"\\", L"\\", L"\\"}, {L"\"", L"'"});
            trim(numStr);
            try {
                ...
                THROW_EXCEPTION_MSG(
                    ExceptionType::ParserError,
                    getErrorMessage(str, startPos, L"Unknown json value format: " + numStr));
            }
        ...
        THROW_EXCEPTION_MSG(
            ExceptionType::ParserError,
            getErrorMessage(str, startPos, L"Parse json object error: " + str2wstr(e.what())));
    }
}

void JsonBuilder::deserialize(const std::wstring& str, size_t& pos,
                              std::shared_ptr<IDocument> doc) const {
    TRY auto jsonObj = std::dynamic_pointer_cast<Json>(doc);
    assert(jsonObj != nullptr);
    getNextCharPos(str, pos, true);
    if (str[pos] != L'{')
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            getErrorMessage(str, pos, L"Json Object not start with {"));
    getNextCharPos(str, pos, false);
    while (pos < str.length()) {
        // name
        std::wstring name = getNextQuotedString(str, pos, {L":"}, {L"\"", L"'", L"{", L"["},
                                                {L"\"", L"'", L"}", L"]"},
                                                {L"\\", L"\\", L"\\", L"\\"}, {L"\"", L"'"});
        trim(name);
        ...
        if (str[pos] != L':')
            THROW_EXCEPTION_MSG(
                ExceptionType::ParserError,
                getErrorMessage(str, pos, L"Json Object name " + name + L" not followed by :"));
        getNextCharPos(str, pos, false);
