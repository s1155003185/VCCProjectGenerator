...
std::wstring ConfigBuilder::serialize(const IDocument* doc) const {
    ...
    TRY Config* configObj = dynamic_cast<Config*>(const_cast<IDocument*>(doc));
...
void ConfigBuilder::deserialize(const std::wstring& str, size_t& /*pos*/,
                                std::shared_ptr<IDocument> doc) const {
    TRY auto configObj = std::dynamic_pointer_cast<Config>(doc);
    assert(configObj != nullptr);
