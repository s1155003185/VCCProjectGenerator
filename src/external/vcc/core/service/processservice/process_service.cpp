...
std::wstring ProcessService::_ExecuteLinux(const std::string& command) {
    ...
    // pipe
    if (pipe(pipefd_stdout) < 0) perror("pipe: ");
    if (pipe(pipefd_stderr) < 0) perror("pipe: ");
...
std::wstring ProcessService::execute(const LogConfig* logConfig, const std::wstring& id,
...
std::wstring ProcessService::execute(const LogConfig* logConfig, const std::wstring& id,
                                     const std::wstring& workspace, const std::wstring& command) {
