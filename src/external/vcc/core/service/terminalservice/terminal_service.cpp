...
namespace vcc {
std::wstring TerminalService::execute(const LogConfig* logConfig, std::wstring id,
                                      std::wstring cmd) {
    TRY return execute(logConfig, id, L"", cmd);
    CATCH
    return L"";
}

std::wstring TerminalService::execute(const LogConfig* logConfig, const std::wstring& id,
    ...
    try {
        ...
        if (!isBlank(workspace)) std::filesystem::current_path(workspace);
    } catch (std::exception& e) {
        if (!isBlank(workspace)) std::filesystem::current_path(currentDirectory);
        THROW_EXCEPTION(e);
        return result;
    }
    ...
    try {
        ...
        try {
            while (!feof(p)) {
                if (fgets(buffer, sizeof(buffer), p) != nullptr) result += str2wstr(buffer);
            }
            int status = pclose(p);
#ifdef _WIN32
            if (status != 0)
