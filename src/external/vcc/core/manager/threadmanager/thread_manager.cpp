...
std::vector<std::shared_ptr<Thread>>& ThreadManager::getThreads() const { return _Threads; }
...
void ThreadManager::queue(std::shared_ptr<Thread> thread) const {
    TRY _Threads.push_back(thread);
    trigger();
    CATCH
}

void ThreadManager::urgent(std::shared_ptr<Thread> thread) const {
    TRY _Threads.insert(_Threads.begin(), thread);
    trigger();
    CATCH
}
...
void ThreadManager::trigger() const {
    ...
    if (!_ActiveThreads.empty()) {
        ...
        _ActiveThreads.erase(std::remove_if(
            _ActiveThreads.begin(), _ActiveThreads.end(), [](std::shared_ptr<Thread> thread) {
                return thread->getState() == ProcessState::Complete ||
                       thread->getState() == ProcessState::Stop;
            }));
    }
    if (!_Threads.empty() && _MaxThreadPoolSize > 0 &&
        _ActiveThreads.size() < (size_t)_MaxThreadPoolSize) {
...
void ThreadManager::join(std::shared_ptr<Thread> thread) const {
    TRY ThreadService::join(thread);
    CATCH
}

void ThreadManager::detach(std::shared_ptr<Thread> thread) const {
    TRY ThreadService::detach(thread);
    CATCH
}

bool ThreadManager::isIdle() const {
    return _State == ProcessState::Idle || _State == ProcessState::Complete ||
           _State == ProcessState::Stop;
}

void ThreadManager::suspend() const {
    TRY _State = ProcessState::Suspend;
    CATCH
}

void ThreadManager::resume() const {
    TRY _State = ProcessState::Idle;
    trigger();
    CATCH
}

void ThreadManager::stop() const {
    TRY clearWaitingThread();
    for (auto& thread : _ActiveThreads) thread->setState(ProcessState::Stop);

    // Remove Complete thread
    trigger();
    if (_ActiveThreads.empty()) return;
    ...
    if (_TerminateMode != ThreadManagerTerminateMode::Immediately) {
        if (_TerminateMode == ThreadManagerTerminateMode::Force) {
            for (auto& thread : _ActiveThreads) {
                TRY
#ifdef _WIN32
                    TerminalService::execute(_LogConfig.get(), L"Thread",
                                             L"taskkill /PID " + thread->getPid() + L" /F");
#else
                    TerminalService::execute(_LogConfig.get(), L"Thread",
                                             L"kill " + thread->getPid());
#endif
                // wait 1s
                Sleep(1000);
                CATCH_SLIENT
            ...
            auto tmpThread =
                std::make_shared<Thread>(_LogConfig, L"ThreadTerminate", L"Start", L"Complete",
                                         [this](const Thread* /*thread*/) {
                                             while (true) {
                                                 this->trigger();
                                                 if (this->getActiveThreads().empty()) break;
                                                 Sleep(500);  // wait 0.5s
                                             }
                                         });
