...
namespace vcc {
std::wstring getCurrentFolderPath() { return std::filesystem::current_path().wstring(); }
...
bool isAbsolutePath(const std::wstring& filePath) {
    TRY return PATH(filePath).is_absolute();
    CATCH
    return false;
}
...
std::wstring getParentPath(const std::wstring& filePath) {
    ...
    TRY parentPath = PATH(filePath).parent_path().wstring();
    CATCH
    return parentPath;
}
...
std::wstring getFileName(const std::wstring& filePath) {
    ...
    TRY fileName = PATH(filePath).filename().wstring();
    CATCH
    return fileName;
}
...
std::wstring getRelativePath(const std::wstring& absolutePath, const std::wstring& basePath) {
    TRY if (isBlank(basePath)) return absolutePath;
    return PATH(absolutePath).lexically_relative(PATH(basePath)).wstring();
    CATCH
    return L"";
}

void getFileDifferenceBetweenWorkspaces(const std::wstring& sourceWorkspace,
    ...
    TRY std::vector<std::wstring> srcFileList, tarFileList;
    TRY for (auto& filePath : std::filesystem::recursive_directory_iterator(
                 PATH(!sourceWorkspace.empty() ? sourceWorkspace : L".")))
        srcFileList.push_back(getRelativePath(filePath.path().wstring(), sourceWorkspace));
    CATCH
    TRY for (auto& filePath : std::filesystem::recursive_directory_iterator(
                 PATH(!targetWorkspace.empty() ? targetWorkspace : L".")))
    ...
    std::set_intersection(srcFileList.begin(), srcFileList.end(), tarFileList.begin(),
                          tarFileList.end(), back_inserter(equalFiles));
    ...
    for (auto& str : equalFiles) {
        ...
        if (isFile(srcFile) && isFile(tarFile)) {
            if (!isFileEqual(srcFile, tarFile)) needToModify.push_back(std::wstring(str));
        }
    }
    CATCH
}
...
bool isPathMatchFileFilter(const std::wstring& filePath, const std::wstring& fileFilter) {
    TRY return std::regex_match(getLinuxPath(filePath),
                                std::wregex(getRegexFromFileFilter(getLinuxPath(fileFilter))));
    CATCH
    return false;
}

bool isPathMatchFileFilters(const std::wstring& filePath,
                            const std::vector<std::wstring>& fileFilters) {
    if (fileFilters.empty()) return false;
    TRY std::vector<std::wstring> regexFilters;
...
void copyFile(const std::wstring& srcFilePath, const std::wstring& destFilePath,
              const bool& isForce) {
    TRY validateFile(srcFilePath);
    if (isForce) createDirectory(PATH(destFilePath).parent_path().wstring());
    if (isForce && isFilePresent(destFilePath)) std::filesystem::remove(destFilePath);
...
void removeFile(const std::wstring& filePath) {
    ...
    TRY if (!isFilePresent(filePath)) return;
...
void createDirectory(const std::wstring& path) {
    if (isBlank(path) || isDirectoryExists(path)) return;

    TRY PATH currentPath(path);
    createDirectory(currentPath.parent_path().wstring());
    std::filesystem::create_directories(path);
    CATCH
}

void copyDirectory(const std::wstring& srcDirectory, const std::wstring& destDirectory,
                   const CopyDirectoryOption* option) {
    assert(!isBlank(srcDirectory));
    assert(!isBlank(destDirectory));
    TRY bool isForce = option != nullptr && option->getIsForce();
    ...
    for (auto& filePath : std::filesystem::recursive_directory_iterator(PATH(srcDirectory))) {
        if (option != nullptr && !option->getIsRecursive()) {
            if (filePath.path().parent_path().wstring() != srcDirectory) continue;
        }
        std::wstring relativePath = getRelativePath(filePath.path().wstring(), srcDirectory);
        if (filePath.is_directory() &&
            !(relativePath.ends_with(L"/") || relativePath.ends_with(L"\\")))
            relativePath += L"/";
        std::wstring destAbsolutePath = concatPaths({destDirectory, relativePath});
        if (filePath.is_directory() &&
            !(destAbsolutePath.ends_with(L"/") || destAbsolutePath.ends_with(L"\\")))
            destAbsolutePath += L"/";
        if (option != nullptr) {
            if (!option->getIncludeFileFilters().empty() &&
                !isPathMatchFileFilters(relativePath, option->getIncludeFileFilters()))
                continue;
            if (!option->getExcludeFileFilters().empty() &&
                isPathMatchFileFilters(relativePath, option->getExcludeFileFilters()))
                continue;
        }
        if (filePath.is_directory()) {
            if (!isDirectoryExists(destAbsolutePath)) createDirectory(destAbsolutePath);
        } else {
            if (isFilePresent(destAbsolutePath)) {
                if (isForce)
                    std::filesystem::remove(destAbsolutePath);
                else
                    THROW_EXCEPTION_MSG(ExceptionType::FileAlreadyExist,
                                        L"File " + destAbsolutePath + L" already exists.");
            }
            std::wstring parentPath = PATH(destAbsolutePath).parent_path().wstring();
            if (!isDirectoryExists(parentPath)) createDirectory(parentPath);
...
void removeDirectory(const std::wstring& directory) {
    ...
    TRY std::filesystem::remove_all(PATH(directory));
    CATCH
}
...
std::wstring readFileOneLine(const std::wstring& filePath, int index) {
    ...
    while (std::getline(fileStream, line)) {
        if (cnt == index) {
            result = line;
            if (!result.empty() && result.at(result.length() - 1) == '\r') result.pop_back();
            break;
        }
        cnt++;
    }
...
void writeFile(const std::wstring& filePath, const std::wstring& content, const bool& isForce) {
    TRY PATH _filePath(filePath);
    PATH dir = _filePath.parent_path();
    if (!dir.wstring().empty() && !isDirectoryExists(dir.wstring())) {
        if (!isForce)
            THROW_EXCEPTION_MSG(ExceptionType::DirectoryNotFound,
                                dir.wstring() + L"Directory not found.");
        else if (!std::filesystem::create_directories(dir))
            THROW_EXCEPTION_MSG(ExceptionType::DirectoryCannotCreate,
                                dir.wstring() + L"Directory not found.");
    }
...
void appendFileOneLine(const std::wstring& filePath, const std::wstring& line,
                       const bool& isForce) {
    TRY
