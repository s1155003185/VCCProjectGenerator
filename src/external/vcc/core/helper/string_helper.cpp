...
std::string wstr2str(const std::wstring& wstr) {
    ...
    TRY std::string str(wstr.begin(), wstr.end());
    return str;
    CATCH
    return "";
}
...
void toLower(std::wstring& str) {
    TRY std::transform(str.begin(), str.end(), str.begin(), ::tolower);
    CATCH
}
    ...
    if (str.length() > 1) {
        if (isContain(str, L" ")) {
            ...
            for (auto& token : tokens) {
                ...
                str += std::wstring(1, std::towupper(token[0])) +
                       (token.length() > 1 ? token.substr(1) : L"");
            }
        } else
...
void toUpper(std::wstring& str) {
    TRY std::transform(str.begin(), str.end(), str.begin(), ::toupper);
    CATCH
}

bool isLower(const std::wstring& str) {
    TRY if (str.empty()) return false;
    for (wchar_t c : str) {
...
bool isEmpty(const std::wstring& str) { return str.empty(); }

bool isBlank(const std::wstring& str) {
    TRY for (const wchar_t& c : str) {
...
bool isEqual(const std::wstring& str1, const std::wstring& str2, const bool& isIgnoreCase) {
    TRY if (str1.size() != str2.size()) return false;
...
bool isStartWith(const std::wstring& str, const std::wstring& prefix, const size_t& startFromPos,
                 bool isIgnoreCase) {
    TRY if (str.length() - startFromPos < prefix.size()) return false;
    return isEqual(str.substr(startFromPos, prefix.length()), prefix, isIgnoreCase);
    CATCH
    return false;
}

bool isStartWith(const std::wstring& str, const std::vector<std::wstring>& prefixes,
                 const size_t& startFromPos, bool isIgnoreCase) {
    bool result = false;
    TRY for (auto const& prefix : prefixes) {
...
bool isStartWithTrimSpace(const std::wstring& str, const std::wstring& prefix,
                          const size_t& startFromPos) {
    TRY std::wstring trimPrefix = std::wstring(prefix);
    trim(trimPrefix);
    ...
    try {
        ...
        while (currentPos < str.length()) {
            ...
            if (prefixPos >= prefix.length()) break;
            currentPos++;
        }
    } catch (...) {
        result = false;
...
bool isEndWith(const std::wstring& str, const std::wstring& suffix) {
    TRY return str.ends_with(suffix);
    CATCH
    return false;
}

std::vector<std::wstring> splitString(const std::wstring& str,
                                      const std::vector<std::wstring>& delimiters,
                                      const std::vector<std::wstring>& quoteOpenList,
                                      const std::vector<std::wstring>& quoteCloseList,
                                      const std::vector<std::wstring>& quoteEscapeList) {
    std::vector<std::wstring> results;
    if (str.empty()) return results;

    TRY if (!(quoteOpenList.size() == quoteCloseList.size() &&
              (quoteEscapeList.empty() || quoteCloseList.size() == quoteEscapeList.size())))
        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                            L"Command Open, Close, Escape List having different size.");
    ...
    if (!currentStr.empty() || !results.empty()) results.push_back(currentStr);
    CATCH
    return results;
}
...
std::vector<std::wstring> splitStringByLine(const std::wstring& str) {
    ...
    TRY std::wistringstream iss(str);
    std::wstring line;
    while (std::getline(iss, line)) results.push_back(line);
    CATCH
    return results;
}

std::vector<std::wstring> splitStringByUpperCase(const std::wstring& str, bool splitDigit,
                                                 bool splitSpecialChar) {
    std::vector<std::wstring> results;
    TRY std::wstring currentToken;
    for (auto const& c : str) {
        bool pushCurrentToken = std::iswupper(c) || std::iswspace(c) ||
        ...
        if (pushCurrentToken) {
            trim(currentToken);
            if (!currentToken.empty()) results.push_back(currentToken);
            currentToken = L"";
        }
        if (addCharToCurrentToken) currentToken += std::wstring(1, c);
    }
    if (!currentToken.empty()) results.push_back(currentToken);
    CATCH
    return results;
...
std::string padLeft(const std::string& str, const size_t& length, const char& c) {
    ...
    TRY return std::string(length - std::min(length, str.length()), c) + str;
    CATCH
    return "";
}
...
std::string padRight(const std::string& str, const size_t& length, const char& c) {
    if (length <= str.length()) return str;
    TRY return str + std::string(length - std::min(length, str.length()), c);
    CATCH
    return "";
}
...
std::wstring padLeft(const std::wstring& str, const size_t& length, const wchar_t& c) {
    if (length <= str.length()) return str;
    TRY return std::wstring(length - std::min(length, str.length()), c) + str;
    CATCH
    return L"";
}
...
std::wstring padRight(const std::wstring& str, const size_t& length, const wchar_t& c) {
    if (length <= str.length()) return str;
    TRY return str + std::wstring(length - std::min(length, str.length()), c);
    CATCH
    return L"";
}

void lTrim(std::string& str) {
    TRY str.erase(str.begin(),
...
void lTrim(std::wstring& str) {
    TRY str.erase(str.begin(), std::find_if(str.begin(), str.end(),
...
std::wstring convertNamingStyle(const std::wstring& str, const NamingStyle& from,
                                const NamingStyle& to, const std::wstring& seperator) {
    std::wstring result = L"";
    TRY if (from == to) return str;
...
size_t find(const std::wstring& str, const wchar_t& c, const size_t& pos,
            const bool& isIgnoreCase) {
    if (str.empty()) return std::wstring::npos;
    TRY size_t cnt = pos;
    for (auto it = str.begin() + pos; it != str.end(); it++) {
        if (isIgnoreCase) {
            if (std::towupper(*it) == std::towupper(c)) return cnt;
        } else {
            if (*it == c) return cnt;
        }
        cnt++;
...
size_t find(const std::wstring& str, const std::wstring& subStr, const size_t& pos,
            const bool& isIgnoreCase) {
    if (str.empty() || str.length() - pos < subStr.length()) return std::wstring::npos;
    TRY if (subStr.length() == 1) return find(str, subStr[0], pos, isIgnoreCase);
    ...
    if (isIgnoreCase) {
        ...
        if (str.length() == subStr.length() && str != subStr) return std::wstring::npos;
...
void getNextCharPos(const std::wstring& str, size_t& pos, bool fromCurrentPos) {
    TRY if (!fromCurrentPos) pos++;
    while (pos < str.length()) {
        if (!std::iswspace(str[pos])) return;
        pos++;
    }
    CATCH
}
...
std::wstring getTailingSubstring(const std::wstring& str, const size_t& length) {
    ...
    TRY return str.substr(str.length() - length, length);
    CATCH
    return str;
}
...
std::wstring getNextStringSplitBySpace(const std::wstring& str, size_t& pos,
    ...
    TRY
        ...
        return getNextString(str, pos, {L" ", L"\r\n", L"\n", L"\r", L"\t"}, quoteOpenList,
                             quoteCloseList, quoteEscapeList);
    CATCH
    return str;
}
...
std::wstring getNextString(const std::wstring& str, size_t& pos,
    ...
    std::wstring result = L"";
    TRY getNextCharPos(str, pos, true);
    if (!(quoteOpenList.size() == quoteCloseList.size() &&
          (quoteEscapeList.empty() || quoteCloseList.size() == quoteEscapeList.size())))
        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                            L"Quote Open, Close, Escape List having different size.");
...
std::wstring getNextQuotedString(const std::wstring& str, size_t& pos,
                                 const std::vector<std::wstring>& delimiters,
                                 const std::vector<std::wstring>& quoteOpenList,
                                 const std::vector<std::wstring>& quoteCloseList,
                                 const std::vector<std::wstring>& quoteEscapeList,
    ...
    if (!isContain(quoteOpenList, stringOpenList))
        THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                            L"Quote Open does not contained String Open List.");
    ...
    if (isStartWith(str, quoteOpenList, pos)) {
        ...
        while (pos < str.length()) {
            if (isStartWith(str, delimiters, pos)) break;
            pos++;
        }
    }
    if (startPos != pos) {
...
bool isContain(const std::wstring& str, const std::wstring& subStr, const size_t& pos,
               const bool& isIgnoreCase) {
    TRY return find(str, subStr, pos, isIgnoreCase) != std::wstring::npos;
    CATCH
    return false;
}
...
void replaceRegex(std::wstring& str, const std::wstring& regex, const std::wstring& replacement) {
    TRY str = std::regex_replace(str, std::wregex(regex), replacement,
                                 std::regex_constants::format_first_only);
    CATCH
}

void replaceAll(std::wstring& str, const std::wstring& from, const std::wstring& to) {
    TRY size_t startPos = 0;
    size_t foundPos;
...
void replaceRegexAll(std::wstring& str, const std::wstring& regex,
                     const std::wstring& replacement) {
    TRY str =
        std::regex_replace(str, std::wregex(regex), replacement, std::regex_constants::match_any);
    CATCH
}
...
std::wstring convertSpecialCharacterToEscapeString(const EscapeStringType& type, const wchar_t& c) {
    TRY std::vector<wchar_t> specialChars = getSpecialCharacters(type);
    if (!std::count(specialChars.begin(), specialChars.end(), c)) return std::wstring(1, c);
...
std::wstring getEscapeString(const EscapeStringType& type, const std::wstring& str) {
    std::wstring result = L"";
    TRY for (auto c : str) result += convertSpecialCharacterToEscapeString(type, c);
    CATCH
    return result;
}
...
std::wstring getUnescapeString(const EscapeStringType& type, const std::wstring& str) {
    std::wstring result = L"";
    TRY std::map<wchar_t, std::wstring> escapeMap;
    switch (type) {
        case EscapeStringType::XML:
...
std::wstring getUnescapeStringWithQuote(const EscapeStringType& type, const std::wstring& str) {
    TRY switch (type) {
        case EscapeStringType::DoubleQuote: {
            bool isNeedRemoveHeadAndTail =
                str.starts_with(L"\"") && str.ends_with(L"\"") && str.length() >= 2;
            if (!isNeedRemoveHeadAndTail)
                THROW_EXCEPTION_MSG(
                    ExceptionType::ParserError,
                    L"GetUnescapeStringWithQuote: " + str + L" does not start with \"");
            break;
        }
        case EscapeStringType::SingleQuote: {
            bool isNeedRemoveHeadAndTail =
