...
namespace vcc {
std::shared_ptr<LogConfig> BaseAction::getLogConfig() const { return _LogConfig; }
...
void BaseAction::logRedoStart() const {
    TRY std::wstring message = getRedoMessageStart();
    if (!isBlank(message)) LogService::logInfo(_LogConfig.get(), L"", message);
    CATCH
}

void BaseAction::logRedoComplete() const {
    TRY std::wstring message = getRedoMessageComplete();
    if (!isBlank(message)) LogService::logInfo(_LogConfig.get(), L"", message);
    CATCH
}

void BaseAction::logUndoStart() const {
    TRY std::wstring message = getUndoMessageStart();
    if (!isBlank(message)) LogService::logInfo(_LogConfig.get(), L"", message);
    CATCH
}

void BaseAction::logUndoComplete() const {
    TRY std::wstring message = getUndoMessageComplete();
    if (!isBlank(message)) LogService::logInfo(_LogConfig.get(), L"", message);
    CATCH
}
...
size_t BaseAction::getSeqNo() const { return _SeqNo; }

void BaseAction::setSeqNo(const size_t& seqNo) const { _SeqNo = seqNo; }
