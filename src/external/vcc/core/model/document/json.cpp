...
JsonInternalType Json::getJsonType(const std::wstring& key) const {
    ...
    TRY result = getJsonInternalNameValuePairsAtKey(key)->getJsonInternalType();
    CATCH
    return result;
}
...
std::vector<std::wstring> Json::getKeys(/*bool isRecursive*/) const {
    ...
    TRY for (auto const& pair : _JsonInternalNameValuePairs) {
...
bool Json::isContainKey(const std::wstring& key /*, bool isRecursive*/) const {
    TRY auto keys = getKeys();  // getKeys(isRecursive);
...
size_t Json::getDecimalPlaces(const std::wstring& key) const {
    ...
    decPt = value.rfind(L".");
    if (decPt == std::wstring::npos) decPt = 0;
    CATCH
    return decPt;
}
...
char Json::getChar(const std::wstring& key) const {
    ...
    if (result.length() != 1)
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            L"Json Value of key " + key + L" is not char");
...
wchar_t Json::getWchar(const std::wstring& key) const {
    ...
    if (result.length() != 1)
        THROW_EXCEPTION_MSG(ExceptionType::ParserError,
                            L"Json Value of key " + key + L" is not wchar_t");
    return result[0];
    CATCH
    return L'\0';
}
...
//         getJsonInternalNameValuePairsAtKey(key)->getJsonInternalArray().push_back(std::make_pair(key,
//         object));
//     CATCH
// }
...
void Json::addArrayNull() const {
    TRY auto json = std::make_shared<Json>();
    json->setJsonInternalType(JsonInternalType::Null);
...
bool Json::getArrayElementBool() const {
    TRY return _JsonInternalValue == L"true" ? true : false;
    CATCH
    return false;
}
...
double Json::getArrayElementDouble() const {
    TRY return std::stod(_JsonInternalValue);
    CATCH
    return 0;
}
...
int64_t Json::getArrayElementInt64() const {
    TRY return std::stoi(_JsonInternalValue);
    CATCH
    return 0;
}
...
char Json::getArrayElementChar() const {
    TRY if (_JsonInternalValue.length() != 1)
...
std::shared_ptr<Json> Json::getArrayElementObject() const {
    TRY return _JsonInternalArray.empty() ? nullptr : _JsonInternalArray[0];
    CATCH
    return nullptr;
}

void Json::addArrayObject(std::shared_ptr<Json> object) const {
    TRY object->setJsonInternalType(JsonInternalType::Json);
...
void Json::addArrayArray(std::shared_ptr<Json> array) const {
    TRY array->setJsonInternalType(JsonInternalType::Array);
