...
std::wstring GitService::execute(const LogConfig* logConfig, const std::wstring& command) {
    TRY return ProcessService::execute(logConfig, GIT_LOG_ID, command);
    CATCH
    return L"";
}

std::wstring GitService::getVersion(const LogConfig* logConfig) {
    TRY std::wstring cmdResult = ProcessService::execute(logConfig, GIT_LOG_ID, L"git --version");
    std::wsmatch m;
    if (std::regex_search(cmdResult, m, std::wregex(L"[0-9]+.[0-9]+.[0-9]+"))) return m[0];
    CATCH
    return L"";
}
    ...
    TRY switch (c) {
        ...
        default:
            THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                L"Unknown Git file status " + std::wstring(1, c));
    }
    CATCH
    return GitFileStatus::NA;
}

std::shared_ptr<GitStatus> GitService::getStatus(const LogConfig* logConfig,
                                                 const std::wstring& workspace,
                                                 const GitStatusSearchCriteria* searchCriteria) {
    auto status = std::make_shared<GitStatus>();
    TRY std::wstring optionStr = L"";
    if (searchCriteria != nullptr) {
        if (searchCriteria->getIsWithIgnoreFiles()) optionStr += L" --ignored";
    }

    std::vector<std::wstring> lines = splitStringByLine(
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git status -b -s" + optionStr));
    ...
    for (auto line : lines) {
        ...
        if (isStartWith(line, localBrachPrefix)) {
            ...
            std::vector<std::wstring> tokens =
                splitString(line.substr(remoteBranchPrefix.length()), {L"..."});
            status->setBranch(tokens.at(0));
            if (tokens.size() > 1) status->setRemoteBranch(tokens.at(1));
            continue;
        }
        ...
        trim(filePath);
        if (indexFileStatus == GitFileStatus::Ignored ||
            indexFileStatus == GitFileStatus::Untracked) {
...
void GitService::initializeGitResponse(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git init");
    CATCH
}

void GitService::cloneGitResponse(const LogConfig* logConfig, const std::wstring& workspace,
                                  const std::wstring& url, const GitCloneOption* option) {
    TRY std::wstring optionStr = L"";
    if (option != nullptr) {
        if (!isBlank(option->getBranch()))
            optionStr += L" -b " + getEscapeStringWithQuote(EscapeStringType::DoubleQuote,
                                                            option->getBranch());
        if (option->getDepth() > 0) optionStr += L" --depth " + std::to_wstring(option->getDepth());
        if (option->getIsQuiet()) optionStr += L" --quiet";
    }
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git clone " + url + optionStr);
    CATCH
}

std::vector<std::shared_ptr<GitRemote>> GitService::getRemote(const LogConfig* logConfig,
                                                              const std::wstring& workspace) {
    std::vector<std::shared_ptr<GitRemote>> remotes;
    TRY std::wstring result =
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git remote -v");
    if (isBlank(result)) return remotes;
...
void GitService::AddRemote(const LogConfig* logConfig, const std::wstring& workspace,
                           const std::wstring& name, const std::wstring& url,
                           const GitRemoteMirror& mirror) {
    TRY assert(!isBlank(name));
    assert(!isBlank(url));
    ...
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git remote " + mirrorStr + L" " + name + L" " + url);
    CATCH
}

void GitService::RenameRemote(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& oldName, const std::wstring& newName) {
    TRY assert(!isBlank(oldName));
    assert(!isBlank(newName));
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git rename " + oldName + L" " + newName);
    CATCH
}

void GitService::RemoveRemote(const LogConfig* logConfig, const std::wstring& workspace,
...
void GitService::FetchAll(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git fetch --all");
    CATCH
}

void GitService::Pull(const LogConfig* logConfig, const std::wstring& workspace,
                      const GitPullOption* option) {
    TRY std::wstring optionStr = L"";
    if (option != nullptr) {
        if (option->getIsQuite()) optionStr += L" --quiet";
...
void GitService::Push(const LogConfig* logConfig, const std::wstring& workspace,
                      const GitPushOption* option) {
    TRY std::wstring optionStr = L"";
    if (option != nullptr) {
        if (option->getIsForce()) optionStr += L" --force";
...
std::wstring getGitLogSearchCriteriaString(const GitLogSearchCriteria* searchCriteria) {
    ...
    TRY if (searchCriteria == nullptr) return optionStr;
    ...
    if (!isBlank(searchCriteria->getDateAfter()))
        optionStr +=
            L" --after=\"" +
            getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getDateAfter()) + L"\"";

    if (!isBlank(searchCriteria->getDateBefore()))
        optionStr +=
            L" --before=\"" +
            getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getDateBefore()) + L"\"";

    if (!isBlank(searchCriteria->getAuthor()))
        optionStr += L" --author=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getAuthor()) +
                     L"\"";

    if (!isBlank(searchCriteria->getCommitter()))
        optionStr +=
            L" --committer=\"" +
            getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getCommitter()) + L"\"";

    if (!isBlank(searchCriteria->getGrep()))
        optionStr += L" --grep=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getGrep()) +
                     L"\"";

    if (searchCriteria->getIsGrepAllMatch()) optionStr += L" --all-match";

    if (searchCriteria->getIsGrepInvertGrep()) optionStr += L" --invert-grep";

    if (searchCriteria->getIsPatternRegexpIgnoreCase()) optionStr += L" --regexp-ignore-case";

    if (searchCriteria->getIsPatternExtendedRegexp()) optionStr += L" --extended-regexp";

    if (searchCriteria->getIsPatternFixedStrings()) optionStr += L" --fixed-strings";

    if (searchCriteria->getIsMerges()) optionStr += L" --merges";

    if (searchCriteria->getIsNoMerges()) optionStr += L" --no-merges";
    ...
    if (searchCriteria->getIsFirstParent()) optionStr += L" --first-parent";

    if (searchCriteria->getIsAll()) optionStr += L" --all";

    if (searchCriteria->getIsAllBranches())
        optionStr += L" --branches";
    else if (!isBlank(searchCriteria->getBranches()))
        optionStr += L" --branches=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getBranches()) +
                     L"\"";
    ...
    if (searchCriteria->getIsAllTags())
        ...
        optionStr += L" --tags=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getTags()) +
                     L"\"";
    ...
    if (searchCriteria->getIsAllRemotes())
        ...
        optionStr += L" --remotes=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getRemotes()) +
                     L"\"";
    ...
    if (searchCriteria->getIsAllGlob())
        ...
        optionStr += L" --glob=\"" +
                     getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getGlob()) +
                     L"\"";

    if (!isBlank(searchCriteria->getExcludeGlob()))
        optionStr +=
            L" --exclude=\"" +
            getEscapeString(EscapeStringType::DoubleQuote, searchCriteria->getExcludeGlob()) +
            L"\"";

    // History Simplification
    if (searchCriteria->getIsSimplifyByDecoration()) optionStr += L" --simplify-by-decoration";

    if (searchCriteria->getIsShowPulls()) optionStr += L" --show-pulls";

    if (searchCriteria->getIsFullHistory()) optionStr += L" --full-history";

    if (searchCriteria->getIsSparse()) optionStr += L" --sparse";

    if (searchCriteria->getIsSimplifyMerges()) optionStr += L" --simplify-merges";
...
std::vector<std::shared_ptr<GitLog>> GitService::parseGitLogGraph(const std::wstring& str) {
    ...
    TRY std::vector<std::wstring> lines = splitStringByLine(str);
    for (const std::wstring& line : lines) {
...
time_t GitService::parseGitLogDatetime(const std::wstring& datimeStr) {
    time_t time = -1;
    TRY time = ParseDatetime(datimeStr, L"%a %b %d %H:%M:%S %Y %z");
    CATCH
    return time;
}

void GitService::parseGitLog(const std::wstring& str, std::shared_ptr<GitLog> log) {
    TRY if (!isStartWith(str, L"commit "))
    ...
    for (const std::wstring& line : lines) {
        if (isBlank(line)) {
            ...
            if (!isBlank(log->getHashID()) && log->getHashID() != tokens.at(0))
                THROW_EXCEPTION_MSG(ExceptionType::CustomError,
                                    L"HashID " + log->getHashID() + L" + not match: " + str);
...
std::vector<std::shared_ptr<GitLog>> GitService::getLogs(
    const LogConfig* logConfig, const std::wstring& workspace,
    const GitLogSearchCriteria* searchCriteria) {
    std::vector<std::shared_ptr<GitLog>> logs;
    TRY logs = GitService::parseGitLogGraph(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git log --graph --oneline --pretty=format:\"(%H)(%h)(%T)(%t)(%P)(%p)\" " +
            getGitLogSearchCriteriaString(searchCriteria)));
    ...
    std::vector<std::wstring> lines = splitStringByLine(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git log --pretty=fuller" + getGitLogSearchCriteriaString(searchCriteria)));
    for (const std::wstring& line : lines) {
        if (isStartWith(line, hashIDPrefix)) {
            ...
            if (!logDetail.empty()) logDetail += L"\n";
            logDetail += line;
        }
    }
    if (!logDetail.empty()) {
...
std::shared_ptr<GitLog> GitService::getCurrentLog(const LogConfig* logConfig,
                                                  const std::wstring& workspace) {
    TRY auto searchCriteria = std::make_shared<GitLogSearchCriteria>();
    searchCriteria->setLogCount(1);
    auto logs = GitService::getLogs(logConfig, workspace, searchCriteria.get());
    if (!logs.empty()) return std::dynamic_pointer_cast<GitLog>(logs.at(0)->clone());
    CATCH
    return nullptr;
}

// void GitService::GetLog(const LogConfig *logConfig, const std::wstring &workspace, const
// std::wstring &hashID, std::shared_ptr<GitLog> log)
// {
//     TRY
//         assert(!isBlank(hashID));
//         GitService::parseGitLog(ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git
//         log --pretty=fuller -n 1 " + hashID), log);
//     CATCH
// }

std::vector<std::wstring> GitService::getTags(const LogConfig* logConfig,
                                              const std::wstring& workspace,
                                              const GitTagSearchCriteria* searchCriteria) {
    ...
    std::vector<std::wstring> lines =
        splitStringByLine(ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git tag -l"));
    tags.insert(tags.end(), lines.begin(), lines.end());
    CATCH
    return tags;
}

// void GitService::getTag(const LogConfig *logConfig, const std::wstring &workspace, const
// std::wstring &tagName, std::shared_ptr<GitLog> log)
// {
//     TRY
//         assert(!isBlank(tagName));
//          std::vector<std::wstring> lines = splitStringByLine(ProcessService::execute(logConfig,
//          GIT_LOG_ID, workspace, L"git show --format=fuller " + tagName));
...
std::shared_ptr<GitTagCurrentTag> GitService::getCurrentTag(const LogConfig* logConfig,
                                                            const std::wstring& workspace) {
    TRY auto currentTag = std::make_shared<GitTagCurrentTag>();
    std::wstring tagStr = splitStringByLine(
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git describe --tags"))[0];
...
void GitService::CreateTag(const LogConfig* logConfig, const std::wstring& workspace,
                           const std::wstring& tagName, const GitTagCreateTagOption* option) {
    TRY assert(!isBlank(tagName));
    std::wstring optionStr = L"";
    ...
    if (option != nullptr) {
        ...
        if (option->getIsForce()) optionStr += L" -f";

        if (!isBlank(option->getMessage()))
            optionStr += L" -m \"" +
                         getEscapeString(EscapeStringType::DoubleQuote, option->getMessage()) +
                         L"\"";

        if (!isBlank(option->getHashID())) suffixOptionStr += L" " + option->getHashID();
    }
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git tag" + optionStr + L" " + tagName + L" " + suffixOptionStr);
    CATCH
}

void GitService::Switch(const LogConfig* logConfig, const std::wstring& workspace,
                        const std::wstring& tagName, bool isForce) {
    TRY assert(!isBlank(tagName));
    std::wstring optionStr = isForce ? L" -f" : L"";
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git checkout " + optionStr + L" " + tagName);
    CATCH
}

void GitService::SwitchReverse(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git switch -");
    CATCH
}

void GitService::DeleteTag(const LogConfig* logConfig, const std::wstring& workspace,
...
// void GitService::getCurrentBranch(const LogConfig *logConfig, const std::wstring &workspace,
// std::shared_ptr<GitBranch> branch)
// {
//     TRY
//         std::wstring str = ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git branch
//         -v"); if (str.empty())
//             return;
//         std::vector<std::wstring> lines = splitStringByLine(str);
//         if (!lines.empty()) {
...
std::wstring GitService::getCurrentBranchName(const LogConfig* logConfig,
                                              const std::wstring& workspace) {
    TRY return splitStringByLine(
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git branch --show-current"))[0];
    CATCH
    return L"";
}

std::vector<std::shared_ptr<GitBranch>> GitService::getBranches(const LogConfig* logConfig,
                                                                const std::wstring& workspace) {
    std::vector<std::shared_ptr<GitBranch>> branches;
    TRY std::wstring str =
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git branch -v --all");
    if (str.empty()) return branches;
    std::vector<std::wstring> lines = splitStringByLine(str);
    for (const std::wstring& line : lines) {
...
void GitService::CreateBranch(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& branchName,
                              const GitBranchCreateBranchOption* option) {
    TRY assert(!isBlank(branchName));
    std::wstring optionStr = L"";
    if (option != nullptr) {
        if (option->getIsForce()) optionStr += L" -f";
        ...
        if (option->getIsRecurseSubmodules()) optionStr += L" --recurse-submodules";
    }
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git branch" + optionStr + L" " + branchName);
    CATCH
}

void GitService::SwitchBranch(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& branchName,
                              const GitBranchSwitchBranchOption* option) {
    TRY assert(!isBlank(branchName));
    std::wstring optionStr = L"";
    if (option != nullptr) {
    ...
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git switch" + optionStr + L" " + branchName);
    CATCH
}

void GitService::RenameBranch(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& oldBranchName, const std::wstring& newBranchName,
                              bool isForce) {
    TRY assert(!isBlank(oldBranchName));
    assert(!isBlank(newBranchName));
    std::wstring optionStr = isForce ? L" -M" : L" -m";
    ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git branch" + optionStr + L" " + oldBranchName + L" " + newBranchName);
    CATCH
}

void GitService::CopyBranch(const LogConfig* logConfig, const std::wstring& workspace,
                            const std::wstring& oldBranchName, const std::wstring& newBranchName,
                            bool isForce) {
    TRY assert(!isBlank(oldBranchName));
    assert(!isBlank(newBranchName));
...
void GitService::DeleteBranch(const LogConfig* logConfig, const std::wstring& workspace,
                              const std::wstring& branchName, bool isForce) {
    TRY assert(!isBlank(branchName));
...
std::shared_ptr<GitDifferenceSummary> GitService::getDifferenceSummary(
    const LogConfig* logConfig, const std::wstring& workspace,
    const std::vector<std::wstring>& hashIDs) {
    TRY auto summary = std::make_shared<GitDifferenceSummary>();
    std::vector<std::wstring> lines = splitStringByLine(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace, L"git diff " + concat(hashIDs, L" ") + L" --numstat"));
    for (const std::wstring& line : lines) {
...
std::shared_ptr<GitDifference> GitService::parseGitDiff(const std::wstring& str) {
    TRY auto difference = std::make_shared<GitDifference>();
    ...
    for (const std::wstring& line : lines) {
        if (rowCount > 0) {
            ...
            if (isStartWith(line, filePathOldPrefix)) {
                ...
                rowCount = std::max(difference->getLineCountOld()[currentItem],
                                    difference->getLineCountNew()[currentItem]);
            }
        }
    }
    return difference;
...
std::shared_ptr<GitDifference> GitService::getDifferenceIndexFile(
    const LogConfig* logConfig, const std::wstring& workspace,
    const GitDifferentSearchCriteria* searchCriteria) {
    TRY auto diff = std::make_shared<GitDifference>();
    std::wstring optionStr = L"";
    if (searchCriteria != nullptr) {
    ...
    return parseGitDiff(ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                                                L"git diff --cached" + optionStr));
    CATCH
    return nullptr;
}

std::shared_ptr<GitDifference> GitService::getDifferenceWorkingFile(
    const LogConfig* logConfig, const std::wstring& workspace, const std::wstring& filePath,
    const GitDifferentSearchCriteria* searchCriteria) {
    TRY assert(!isBlank(filePath));
    std::wstring optionStr = L"";
    if (searchCriteria != nullptr) {
        if (searchCriteria->getNoOfLines() > -1)
            optionStr += L" --unified=" + std::to_wstring(searchCriteria->getNoOfLines());
    ...
    return parseGitDiff(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git diff" + optionStr + L" \"" +
            getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\""));
    CATCH
    return nullptr;
}

std::shared_ptr<GitDifference> GitService::getDifferenceFile(
    const LogConfig* logConfig, const std::wstring& workspace, const std::wstring& filePath,
    const GitDifferentSearchCriteria* searchCriteria) {
    TRY assert(!isBlank(filePath));
    std::wstring optionStr = L"";
    ...
    return parseGitDiff(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git diff HEAD" + optionStr + L" \"" +
            getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\""));
    CATCH
    return nullptr;
}

std::shared_ptr<GitDifference> GitService::getDifferenceCommit(
    const LogConfig* logConfig, const std::wstring& workspace, const std::wstring& fromHashID,
    const std::wstring& toHashID, const std::wstring& filePath,
    const GitDifferentSearchCriteria* searchCriteria) {
    TRY assert(!isBlank(filePath));
    std::wstring optionStr = L"";
    if (searchCriteria != nullptr) {
    ...
    return parseGitDiff(ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git diff" + optionStr + L" " + fromHashID + L"..." + toHashID + L" \"" +
            getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\""));
    CATCH
    return nullptr;
}

void GitService::stage(const LogConfig* logConfig, const std::wstring& workspace,
                       const std::wstring& filePath) {
    TRY ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git add \"" + getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\"");
    CATCH
}
void GitService::stageAll(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git add *");
    CATCH
}

void GitService::unstage(const LogConfig* logConfig, const std::wstring& workspace,
                         const std::wstring& filePath) {
    TRY ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git reset \"" + getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\"");
    CATCH
}

void GitService::unstageAll(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git reset");
    CATCH
}

void GitService::Commit(const LogConfig* logConfig, const std::wstring& workspace,
                        const std::wstring& command) {
    TRY ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git commit -m \"" + getEscapeString(EscapeStringType::DoubleQuote, command) + L"\"");
    CATCH
}

void GitService::Amend(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git commit --amend");
    CATCH
}

void GitService::DiscardFile(const LogConfig* logConfig, const std::wstring& workspace,
                             const std::wstring& filePath) {
    TRY ProcessService::execute(
        logConfig, GIT_LOG_ID, workspace,
        L"git checkout \"" + getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\"");
    CATCH
}

void GitService::RestoreFileToParticularCommit(const LogConfig* logConfig,
                                               const std::wstring& workspace,
                                               const std::wstring& filePath,
                                               const std::wstring& hashID) {
    TRY assert(!isBlank(hashID));
    ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                            L"git restore --source=" + hashID + L" \"" +
                                getEscapeString(EscapeStringType::DoubleQuote, filePath) + L"\"");
    CATCH
}

void GitService::ResetCommit(const LogConfig* logConfig, const std::wstring& workspace,
                             const GitResetMode& resetMode, const std::wstring& hashID) {
    TRY std::wstring modeStr = L"";
    switch (resetMode) {
...
void GitService::ReverseCommit(const LogConfig* logConfig, const std::wstring& workspace,
                               const std::wstring& hashID) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git revert " + hashID);
    CATCH
}

bool GitService::IsConfigExists(const LogConfig* logConfig, const std::wstring& workspace,
...
void GitService::getConfig(const LogConfig* logConfig, const std::wstring& workspace,
                           std::shared_ptr<GitConfig> config) {
    TRY GitService::getLocalConfig(logConfig, workspace, config);
    auto globalResult = std::make_shared<GitConfig>();
    GitService::getGlobalConfig(logConfig, globalResult);
    if (config->getUserName().empty()) config->setUserName(globalResult->getUserName());
    if (config->getUserEmail().empty()) config->setUserEmail(globalResult->getUserEmail());
...
std::wstring GitService::getConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                   const std::wstring& key) {
    std::wstring result = L"";
    TRY try { result = GitService::getLocalConfig(logConfig, workspace, key); } catch (...) {
        result = L"";
    }
    if (result.empty()) result = GitService::getGlobalConfig(logConfig, key);
    CATCH
    return result;
}

std::wstring GitService::getUserName(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY return GitService::getConfig(logConfig, workspace, gitConfigUserName);
    CATCH
    return L"";
}

std::wstring GitService::getUserEmail(const LogConfig* logConfig, const std::wstring& workspace) {
    TRY return GitService::getConfig(logConfig, workspace, gitConfigUserEmail);
    CATCH
    return L"";
}
...
void GitService::getGlobalConfig(const LogConfig* logConfig, std::shared_ptr<GitConfig> config) {
    TRY std::wstring cmdResult =
        ProcessService::execute(logConfig, GIT_LOG_ID, L"git config --global --list");
    if (!isBlank(cmdResult)) {
        ...
        for (auto it : element->getConfigs()) {
            if (!element->isValue(it.first)) continue;
...
std::wstring GitService::getGlobalConfig(const LogConfig* logConfig, const std::wstring& key) {
    TRY std::wstring cmdResult =
        ProcessService::execute(logConfig, GIT_LOG_ID, L"git config --global " + key);
    return isBlank(cmdResult) ? L"" : splitStringByLine(cmdResult).at(0);
    CATCH
    return L"";
}

void GitService::setGlobalConfig(const LogConfig* logConfig, const std::wstring& key,
                                 const std::wstring& value) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID,
                                L"git config --global " + key + L" \"" +
                                    getEscapeString(EscapeStringType::DoubleQuote, value) + L"\"");
    CATCH
}

std::wstring GitService::getGlobalUserName(const LogConfig* logConfig) {
    TRY return GitService::getGlobalConfig(logConfig, gitConfigUserName);
    CATCH
    return L"";
}

void GitService::setGlobalUserName(const LogConfig* logConfig, const std::wstring& value) {
    TRY GitService::setGlobalConfig(logConfig, gitConfigUserName, value);
    CATCH
}

std::wstring GitService::getGlobalUserEmail(const LogConfig* logConfig) {
    TRY return GitService::getGlobalConfig(logConfig, gitConfigUserEmail);
    CATCH
    return L"";
}

void GitService::setGlobalUserEmail(const LogConfig* logConfig, const std::wstring& value) {
    TRY GitService::setGlobalConfig(logConfig, gitConfigUserEmail, value);
    CATCH
}

bool GitService::IsLocalConfigExists(const LogConfig* logConfig, const std::wstring& workspace,
                                     const std::wstring& key) {
    bool result = false;
    try {
...
void GitService::getLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                std::shared_ptr<GitConfig> config) {
    TRY std::wstring cmdResult =
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git config --local --list");
    if (!isBlank(cmdResult)) {
        auto element = std::make_shared<Config>();
        auto reader = std::make_unique<ConfigBuilder>();
...
std::wstring GitService::getLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                        const std::wstring& key) {
    TRY std::wstring cmdResult =
        ProcessService::execute(logConfig, GIT_LOG_ID, workspace, L"git config --local " + key);
    return isBlank(cmdResult) ? L"" : splitStringByLine(cmdResult).at(0);
    CATCH
    return L"";
}

void GitService::setLocalConfig(const LogConfig* logConfig, const std::wstring& workspace,
                                const std::wstring& key, const std::wstring& value) {
    TRY ProcessService::execute(logConfig, GIT_LOG_ID, workspace,
                                L"git config --local " + key + L" \"" +
                                    getEscapeString(EscapeStringType::DoubleQuote, value) + L"\"");
    CATCH
}

std::wstring GitService::getLocalUserName(const LogConfig* logConfig,
                                          const std::wstring& workspace) {
    TRY return GitService::getLocalConfig(logConfig, workspace, gitConfigUserName);
    CATCH
    return L"";
}

void GitService::setLocalUserName(const LogConfig* logConfig, const std::wstring& workspace,
                                  const std::wstring& value) {
    TRY GitService::setLocalConfig(logConfig, workspace, gitConfigUserName, value);
    CATCH
}

std::wstring GitService::getLocalUserEmail(const LogConfig* logConfig,
                                           const std::wstring& workspace) {
    TRY return GitService::getLocalConfig(logConfig, workspace, gitConfigUserEmail);
    CATCH
    return L"";
}

void GitService::setLocalUserEmail(const LogConfig* logConfig, const std::wstring& workspace,
                                   const std::wstring& value) {
    TRY GitService::setLocalConfig(logConfig, workspace, gitConfigUserEmail, value);
    CATCH
}
}  // namespace vcc
