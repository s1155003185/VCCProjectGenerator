...
namespace vcc {
GitManager::GitManager(std::shared_ptr<LogConfig> logConfig, const std::wstring& workspace)
    : BaseManager(logConfig) {
    _Workspace = workspace;
}

void GitManager::validate() const {TRY VALIDATE(L"Workspace is empty", !isBlank(_Workspace)) CATCH}

std::wstring GitManager::getVersion() const {
    TRY return GitService::getVersion(_LogConfig.get());
    CATCH
    return L"";
}
...
std::shared_ptr<GitStatus> GitManager::getStatus(
    const GitStatusSearchCriteria* searchCriteria) const {
    TRY validate();
...
void GitManager::AddRemote(const std::wstring& name, const std::wstring& url,
...
std::vector<std::shared_ptr<GitLog>> GitManager::getLogs(
...
void GitManager::CreateBranch(const std::wstring& branchName,
...
void GitManager::SwitchBranch(const std::wstring& branchName,
...
void GitManager::RenameBranch(const std::wstring& oldBranchName, const std::wstring& newBranchName,
                              bool isForce) {
    TRY validate();
...
void GitManager::CopyBranch(const std::wstring& oldBranchName, const std::wstring& newBranchName,
                            bool isForce) {
    TRY validate();
...
std::shared_ptr<GitDifferenceSummary> GitManager::getDifferenceSummary(
...
std::shared_ptr<GitDifference> GitManager::getDifferenceIndexFile(
...
std::shared_ptr<GitDifference> GitManager::getDifferenceWorkingFile(
    const std::wstring& filePath, const GitDifferentSearchCriteria* searchCriteria) {
    TRY validate();
    return GitService::getDifferenceWorkingFile(_LogConfig.get(), _Workspace, filePath,
                                                searchCriteria);
    CATCH
    return nullptr;
}

std::shared_ptr<GitDifference> GitManager::getDifferenceFile(
    const std::wstring& filePath, const GitDifferentSearchCriteria* searchCriteria) {
    TRY validate();
...
std::shared_ptr<GitDifference> GitManager::getDifferenceCommit(
    const std::wstring& fromHashID, const std::wstring& toHashID, const std::wstring& filePath,
    const GitDifferentSearchCriteria* searchCriteria) {
    TRY validate();
    return GitService::getDifferenceCommit(_LogConfig.get(), _Workspace, fromHashID, toHashID,
                                           filePath, searchCriteria);
    CATCH
    return nullptr;
}
