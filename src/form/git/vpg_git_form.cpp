...
VPGGitForm::VPGGitForm() : vcc::BaseForm() {
    TRY _ObjectType = ObjectType::GitForm;
    initialize();
    CATCH
}
...
std::shared_ptr<vcc::Json> VPGGitForm::toJson() const {
    TRY auto json = std::make_unique<vcc::Json>();
    return json;
    CATCH
    return nullptr;
}
...
void VPGGitForm::deserializeJson(std::shared_ptr<vcc::IDocument> document) {
    TRY auto json = std::dynamic_pointer_cast<vcc::Json>(document);
    assert(json != nullptr);
    CATCH
}
...
std::shared_ptr<vcc::IResult> VPGGitForm::doAction(const int64_t& formProperty,
    ...
    TRY switch (static_cast<VPGGitFormProperty>(formProperty)) {
        default:
            assert(false);
            break;
    }
